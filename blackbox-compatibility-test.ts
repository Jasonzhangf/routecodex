...
class BlackboxCompatibilityTest {
  ...
  private async saveTestReport(): Promise<void> {
    try {
      ...
      await writeFile(
        join(this.samplesDir, 'test-report.json'),
        JSON.stringify(report, null, 2),
        'utf8'
      );

      console.log(`📄 测试报告已保存到: ${join(this.samplesDir, 'test-report.json')}`);
    } catch (error) {
...
// 运行测试
async function main() {
  const tester = new BlackboxCompatibilityTest();
...
export { BlackboxCompatibilityTest };