...
} from './router-hotpath/native-servertool-core-semantics.js';
`;
...
import type * as NativeServertoolCoreSemantics from '__NATIVE_SERVERTOOL_CORE_IMPORT__';
...
function pruneServertoolWrapperDeclarations(source, activeRefs) {
  ...
  const pruned = source.replace(
    /^export declare function ([A-Za-z0-9_]+)(?:<[^>]+>)?\([^;]*\): [^;]+;$/gm,
    (line, name) => {
      if (!activeRefs.has(name)) {
        return '';
      }
      kept.add(name);
      if (nativeServertoolCoreTypedExports.has(name)) {
        return `export declare const ${name}: typeof NativeServertoolCoreSemantics.${name};`;
      }
      return line;
    }
  );
...
const distDts = dts.replace(
  "'__NATIVE_SERVERTOOL_CORE_IMPORT__'",
  "'./router-hotpath/native-servertool-core-semantics.js'"
);
const packageTypesDts = dts.replace(
  "'__NATIVE_SERVERTOOL_CORE_IMPORT__'",
  "'../dist/native/router-hotpath/native-servertool-core-semantics.js'"
);

fs.mkdirSync(outDir, { recursive: true });
fs.mkdirSync(path.dirname(typesDtsPath), { recursive: true });
fs.writeFileSync(jsPath, js);
fs.writeFileSync(dtsPath, distDts);
fs.writeFileSync(typesDtsPath, packageTypesDts);
