...
if (!callMapChain) {
  ...
  const expectedCallees = new Map([
    [
      ...
      'build_provider_resp_compat_02_from_v3_provider_resp_inbound_01_sse',
    ],
    [
...
for (const gate of [
  'npm run test:v3-anthropic-relay-runtime-integration',
  'npm run verify:v3-anthropic-relay-runtime-integration',
  'npm run test:v3-anthropic-relay-runtime-integration-red-fixtures',
]) {
  if (!packageJson.scripts['verify:architecture-ci-longtail']?.includes(gate)) {
    failures.push(`${packagePath}: verify:architecture-ci-longtail missing ${gate}`);
  }
}
