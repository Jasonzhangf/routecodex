...
const cases = [
  ...
  ['Relay WebSocket client disconnect kind erased', 'v3/crates/routecodex-v3-server/src/websocket.rs', 'Err(error) if is_v3_client_disconnect_source(&error) => return Err(()),\n            Err(error) => {\n                return send_responses_websocket_error(\n                    socket,\n                    "runtime_stream_error",\n                    error.message,', 'Err(error) if error.code == "client_disconnect" => return Err(()),\n            Err(error) => {\n                return send_responses_websocket_error(\n                    socket,\n                    "runtime_stream_error",\n                    error.message,', /is_v3_client_disconnect_source|forbidden/],
  ['Post-commit SSE reintroduces an embedded HTTP status', 'v3/crates/routecodex-v3-server/src/frame_builders.rs', 'let event = json!({\n        "type": "error",', 'let _event = json!({\n        "type": "error",\n    });\n    return v3_sse_error_event_chunk(projected.status, &code, &message);\n    let event = json!({\n        "type": "error",', /v3_sse_error_event_chunk|forbidden/],
  ...
  ['WebSocket fallback marker', providerWebsocket, 'fn websocket_sse_stream(', 'fn fallback_http_retry() {}\nfn websocket_sse_stream(', /fallback/i],
];
const copied = [
  runtime,
