...
const verifier = resolve(repo, 'scripts/architecture/verify-v3-selected-provider-model-binding.mjs');
const copied = [
  'package.json',
...
const cases = [
  ...
  },
  {
    ...
    path: 'package.json',
    mutate: (source) => source.replace('"build:v3-cli": "npm run verify:v3-selected-provider-model-binding && ', '"build:v3-cli": "'),
