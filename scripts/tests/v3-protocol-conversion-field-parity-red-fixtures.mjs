...
const cases = [
  ...
  {
    name: 'OpenAI Chat client_metadata target-dependent review loses Responses rename',
    file: 'docs/architecture/reviews/v3-protocol-semantic-matrix-review.md',
    from: 'OpenAI Chat wire rejects `client_metadata` before provider send; Responses wire explicitly renames `client_metadata` to `metadata`.',
    to: 'OpenAI Chat wire rejects `client_metadata` before provider send.',
    diagnostic: /client_metadata|Responses wire explicitly renames/,
  },
  {
  ...
  {
    name: 'Chat canonical max_completion_tokens no longer maps to Anthropic max_tokens',
    file: 'v3/crates/routecodex-v3-runtime/src/hub_v1/anthropic_codec.rs',
    from: '.or_else(|| object.get("max_completion_tokens"))',
    to: '',
    diagnostic: /max_completion_tokens|max_tokens/,
  },
  {
