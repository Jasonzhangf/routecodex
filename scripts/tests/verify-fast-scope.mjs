...
const deletedRustOwnerRoot = mkdtempSync(join(repo, 'playground', '.verify-fast-deleted-rust-'));
try {
  const manifestPath = join(deletedRustOwnerRoot, 'v3', 'Cargo.toml');
  const packageManifestPath = join(deletedRustOwnerRoot, 'v3', 'crates', 'known-owner', 'Cargo.toml');
  const packageSourcePath = join(deletedRustOwnerRoot, 'v3', 'crates', 'known-owner', 'src', 'lib.rs');
  const deletedFixturePath = join(deletedRustOwnerRoot, 'v3', 'tests', 'support', 'retired_fixture.rs');
  mkdirSync(join(manifestPath, '..'), { recursive: true });
  mkdirSync(join(packageManifestPath, '..'), { recursive: true });
  mkdirSync(join(packageSourcePath, '..'), { recursive: true });
  mkdirSync(join(deletedFixturePath, '..'), { recursive: true });
  writeFileSync(manifestPath, '[workspace]\nmembers = ["crates/known-owner"]\nresolver = "2"\n');
  writeFileSync(packageManifestPath, '[package]\nname = "known-owner"\nversion = "0.1.0"\nedition = "2021"\n');
  writeFileSync(packageSourcePath, 'pub const KNOWN_OWNER: bool = true;\n');
  writeFileSync(deletedFixturePath, 'pub const RETIRED_FIXTURE: bool = true;\n');
  writeFileSync(join(deletedRustOwnerRoot, 'v3', 'Cargo.lock'), '# This file is automatically @generated by Cargo.\nversion = 3\n');
  symlinkSync(join(repo, 'node_modules'), join(deletedRustOwnerRoot, 'node_modules'), 'dir');
  execFileSync('git', ['init', '-q'], { cwd: deletedRustOwnerRoot });
  execFileSync('git', ['add', 'v3'], { cwd: deletedRustOwnerRoot });
  execFileSync('git', ['-c', 'user.name=Scope Test', '-c', 'user.email=scope@example.invalid', 'commit', '-qm', 'rust fixture'], { cwd: deletedRustOwnerRoot });
  rmSync(deletedFixturePath);
  execFileSync('git', ['add', '-u', 'v3/tests/support/retired_fixture.rs'], { cwd: deletedRustOwnerRoot });
  const result = spawnSync(process.execPath, [verifier], {
    cwd: deletedRustOwnerRoot,
    encoding: 'utf8',
    env: { ...process.env, ROUTECODEX_GATE_DIFF_MODE: 'staged' },
  });
  const output = `${result.stdout || ''}\n${result.stderr || ''}`;
  if (result.status !== 0 || !output.includes('deleted file(s): v3/tests/support/retired_fixture.rs') || output.includes('file(s) are not owned by a V3 Cargo package')) {
    failures.push(`deleted unowned Rust file must not require Cargo owner evidence, got status=${result.status}\n${output}`);
  }
} finally {
  rmSync(deletedRustOwnerRoot, { recursive: true, force: true });
}

const deletedOwnedRustRoot = mkdtempSync(join(repo, 'playground', '.verify-fast-deleted-owned-rust-'));
try {
  const manifestPath = join(deletedOwnedRustRoot, 'v3', 'Cargo.toml');
  const packageManifestPath = join(deletedOwnedRustRoot, 'v3', 'crates', 'known-owner', 'Cargo.toml');
  const packageSourcePath = join(deletedOwnedRustRoot, 'v3', 'crates', 'known-owner', 'src', 'lib.rs');
  const deletedSourcePath = join(deletedOwnedRustRoot, 'v3', 'crates', 'known-owner', 'src', 'retired.rs');
  mkdirSync(join(manifestPath, '..'), { recursive: true });
  mkdirSync(join(packageManifestPath, '..'), { recursive: true });
  mkdirSync(join(packageSourcePath, '..'), { recursive: true });
  writeFileSync(manifestPath, '[workspace]\nmembers = ["crates/known-owner"]\nresolver = "2"\n');
  writeFileSync(packageManifestPath, '[package]\nname = "known-owner"\nversion = "0.1.0"\nedition = "2021"\n');
  writeFileSync(packageSourcePath, 'pub mod retired;\n');
  writeFileSync(deletedSourcePath, 'pub const RETIRED_SOURCE: bool = true;\n');
  writeFileSync(join(deletedOwnedRustRoot, 'v3', 'Cargo.lock'), '# This file is automatically @generated by Cargo.\nversion = 3\n');
  symlinkSync(join(repo, 'node_modules'), join(deletedOwnedRustRoot, 'node_modules'), 'dir');
  execFileSync('git', ['init', '-q'], { cwd: deletedOwnedRustRoot });
  execFileSync('git', ['add', 'v3'], { cwd: deletedOwnedRustRoot });
  execFileSync('git', ['-c', 'user.name=Scope Test', '-c', 'user.email=scope@example.invalid', 'commit', '-qm', 'owned rust source'], { cwd: deletedOwnedRustRoot });
  rmSync(deletedSourcePath);
  execFileSync('git', ['add', '-u', 'v3/crates/known-owner/src/retired.rs'], { cwd: deletedOwnedRustRoot });
  const scopeOutputPath = join(deletedOwnedRustRoot, 'scope-output.txt');
  const result = spawnSync(process.execPath, [verifier], {
    cwd: deletedOwnedRustRoot,
    encoding: 'utf8',
    env: { ...process.env, ROUTECODEX_GATE_DIFF_MODE: 'staged', ROUTECODEX_GATE_SCOPE_ONLY: '1', ROUTECODEX_GATE_SCOPE_OUTPUT: scopeOutputPath },
  });
  const output = `${result.stdout || ''}\n${result.stderr || ''}`;
  if (result.status !== 0 || !output.includes('affected Rust compile deferred to scoped V3 test job')) {
    failures.push(`deleted owned Rust source must retain scoped V3 compile evidence, got status=${result.status}\n${output}`);
  }
  const scopeOutput = readFileSync(scopeOutputPath, 'utf8');
  if (!scopeOutput.includes('v3=true\n')) {
    failures.push(`deleted owned Rust source must retain V3 scope, got ${scopeOutput}`);
  }
} finally {
  rmSync(deletedOwnedRustRoot, { recursive: true, force: true });
}
