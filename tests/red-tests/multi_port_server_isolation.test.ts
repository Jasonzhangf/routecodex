...
it('StatsManager composeBucketKey partitions stats per port when entryPort is provided', async () => {
  ...
  it('ProviderTrafficGovernor scopeKey partitions per-server concurrency buckets', async () => {
    const { ProviderTrafficGovernor } = await import('../../src/server/runtime/http-server/provider-traffic-governor.js');
    const scopeRoot = mkdtempSync(join(tmpdir(), 'rcc-traffic-scope-'));
    const prevServerId = process.env.ROUTECODEX_SERVER_ID;
    process.env.ROUTECODEX_SERVER_ID = 'host-test:5520';
    try {
      const governor = new ProviderTrafficGovernor(scopeRoot);
      const runtime = {
        runtimeKey: 'mock.iso.runtime',
        providerId: 'mock',
        providerKey: 'mock.iso',
        providerFamily: 'mock',
      ...
      } as unknown as import('../../src/providers/core/api/provider-types.js').ProviderRuntimeProfile;

      // Server A fills concurrency for runtimeKey=mock.iso.runtime.
      const port5520 = await governor.acquire({
        runtimeKey: 'mock.iso.runtime',
        providerKey: 'mock.iso',
        requestId: 'a-1',
        runtime,
        scopeKey: 'server:127.0.0.1:5520'
      });
      expect(port5520.permit.stateKey).toContain('server%3A127.0.0.1%3A5520');

      // Server B on a different port must NOT be blocked by server A's lease.
      const port10000 = await governor.acquire({
        runtimeKey: 'mock.iso.runtime',
        providerKey: 'mock.iso',
        requestId: 'b-1',
        runtime,
        scopeKey: 'server:127.0.0.1:10000'
      });
      expect(port10000.permit.stateKey).not.toBe(port5520.permit.stateKey);

      // Second acquire on server A should hit maxInFlight=2 boundary.
      const port5520Second = await governor.acquire({
        runtimeKey: 'mock.iso.runtime',
        providerKey: 'mock.iso',
        requestId: 'a-2',
        runtime,
      ...
      expect(port5520Second.permit.stateKey).toBe(port5520.permit.stateKey);

      // capacity check should still be per-scope
      const capA = governor.isProviderAtConcurrencyCapacitySync('mock.iso.runtime', runtime, 'server:127.0.0.1:5520');
      const capB = governor.isProviderAtConcurrencyCapacitySync('mock.iso.runtime', runtime, 'server:127.0.0.1:10000');
      expect(capA).toBe(true);
      expect(capB).toBe(false);

      await governor.release(port5520.permit);
      await governor.release(port5520Second.permit);
      await governor.release(port10000.permit);
    } finally {
      if (typeof prevServerId === 'string') {
        process.env.ROUTECODEX_SERVER_ID = prevServerId;
      } else {
        delete process.env.ROUTECODEX_SERVER_ID;
      }
