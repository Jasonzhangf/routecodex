...
const ROUTE_AVAILABILITY_HOST = 'src/modules/llmswitch/bridge/route-availability-host.ts';
...
describe('VR route availability floor singleton truth', () => {
  ...
  it('native export surface exposes singleton route-pool exhaustion decision', () => {
    ...
    expect(readSrc(ROUTE_AVAILABILITY_HOST)).toMatch(/evaluateSingletonRoutePoolExhaustionNative/);
  });

  it('TS executor core utils no longer implement singleton hold semantics locally', () => {
    const src = readSrc(EXECUTOR_CORE_UTILS);
    expect(src).toContain('../../../../modules/llmswitch/bridge/route-availability-host.js');
    expect(src).not.toContain('../../../../modules/llmswitch/bridge/native-exports.js');
  ...
  it('Rust primary_exhausted_to_default_pool contract is reachable through the native export', () => {
    ...
    expect(readSrc(ROUTE_AVAILABILITY_HOST)).toMatch(/planPrimaryExhaustedToDefaultPoolNative/);
  });
