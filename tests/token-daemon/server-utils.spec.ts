...
describe('token daemon server-utils non-blocking observability', () => {
  ...
  it('distinguishes config load failures', async () => {
    ...
    jest.unstable_mockModule('../../src/config/config-paths.js', () => ({
      resolveRouteCodexConfigPath: () => '/tmp/config.json'
    }));
  ...
  it('does not warn for routecodex starting health during startup probe', async () => {
    const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => {});
    ...
    jest.unstable_mockModule('../../src/utils/http-health-probe.js', () => ({
      ...
      describeHealthProbeFailure: () => 'starting status=200 body={"status":"starting"}'
    }));
