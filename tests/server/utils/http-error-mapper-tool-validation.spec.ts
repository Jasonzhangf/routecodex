...
describe('http-error-mapper tool validation handling', () => {
  it('surfaces structured tool validation details in HTTP payloads', () => {
    const payload = mapErrorToHttp({
      message: 'Converted provider tool call has invalid client arguments at choices[0].message.tool_calls[0]: exec_command. exec_command requires input.cmd as a string.',
