...
describe('init-v2-builder', () => {
  it('builds weighted route pools without priority mode', () => {
    const routing = buildInitRouting({
      ...
      multimodalTargets: ['deepseek-web.deepseek-v4-vision']
    ...
    expect(routing.multimodal[0].loadBalancing.weights).toEqual({
      'deepseek-web.deepseek-v4-vision': 1
    });
  ...
  it('wraps routing under routingPolicyGroups and preserves existing policy metadata', () => {
    ...
    expect(next.virtualrouter.activeRoutingPolicyGroup).toBeUndefined();
    expect(
  ...
  it('does not emit legacy activeRoutingPolicyGroup in v2 config output', () => {
    const next = buildV2ConfigObject({
      host: '127.0.0.1',
      port: 5555,
      policyId: 'canary',
      routing: buildInitRouting({ defaultTarget: 'openai.gpt-5.2' })
    }) as Record<string, any>;

    expect(next.virtualrouter.routingPolicyGroups.canary).toBeTruthy();
    expect(next.virtualrouter.activeRoutingPolicyGroup).toBeUndefined();
  });
});
