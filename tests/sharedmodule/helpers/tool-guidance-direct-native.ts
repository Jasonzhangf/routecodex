...
export function augmentOpenAIToolsDirectNative(tools: unknown[]): unknown[] {
  return parseNativeJson<unknown[]>(
    nativeFn('augmentOpenAIToolsJson')(JSON.stringify(tools)),
    'augmentOpenAIToolsJson'
  );
}

export function buildSystemToolGuidanceDirectNative(): string {
  return parseNativeJson<string>(
    nativeFn('buildSystemToolGuidanceJson')(),
