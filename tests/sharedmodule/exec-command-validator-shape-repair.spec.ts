import { validateExecCommandToolCallDirectNative } from './helpers/tool-validation-direct-native.js';

describe('exec_command validator shape repair', () => {
  it('unwraps nested input.cmd shape into canonical cmd', () => {
    const result = validateExecCommandToolCallDirectNative(JSON.stringify({ input: { cmd: 'pwd', workdir: '/workspace' } }));
  ...
  it('unwraps nested arguments.command shape into canonical cmd', () => {
    const result = validateExecCommandToolCallDirectNative(
