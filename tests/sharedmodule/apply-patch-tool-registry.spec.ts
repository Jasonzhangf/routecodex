import { validateApplyPatchToolCallDirectNative } from './helpers/tool-validation-direct-native.js';
...
describe('apply_patch direct native validation', () => {
  it('validates apply_patch through the native apply_patch verdict', () => {
    ...
    const result = validateApplyPatchToolCallDirectNative(JSON.stringify({ patch }));
  ...
  it('rejects line-edit payloads missing fileContent through the native apply_patch verdict', () => {
    const result = validateApplyPatchToolCallDirectNative(
      JSON.stringify({
        patch: '+ 2 deadbeef\nhello',
        filePath: 'note.txt'
      })
  ...
  it('accepts newline-escaped raw patch string without first losing request shape to empty object', () => {
    ...
    const result = validateApplyPatchToolCallDirectNative(patch);
    expect(result.ok).toBe(true);
    const parsed = toArgsObject(result);
  ...
  it('repairs arg_key invalid-json artifact through raw request string instead of collapsing request shape to empty object', () => {
    ...
    const result = validateApplyPatchToolCallDirectNative(invalidJson);
    expect(result.ok).toBe(true);
    const parsed = toArgsObject(result);
