#!/usr/bin/env node
// Qwen Tool Calling Analysis: Send request → Capture response → Module-by-module dry-run
...
async function main() {
  console.log('🚀 Starting Qwen Tool Calling Analysis Pipeline...\n');
  ...
  const qwenConf = userConf?.virtualrouter?.providers?.qwen || {};
  const baseUrl = process.env.QWEN_BASE_URL || qwenConf.baseURL || 'http://localhost:8000';
  const apiKey = process.env.QWEN_API_KEY || (Array.isArray(qwenConf.apiKey) ? qwenConf.apiKey[0] : '');
  ...
  console.log('\n📤 Stage 2: Sending Real Request to Qwen');

  const { OpenAIPassthroughLLMSwitch } = await importFromDist('modules/pipeline/modules/llmswitch/openai-passthrough.js');
  const { QwenCompatibility } = await importFromDist('modules/pipeline/modules/compatibility/qwen-compatibility.js');
  const { QwenProvider } = await importFromDist('modules/pipeline/modules/provider/qwen-provider.js');
  ...
  const realCompatibility = new QwenCompatibility({ type: 'qwen-compatibility', config: { toolsEnabled: true } }, dependencies);
  const realProvider = new QwenProvider({ type: 'qwen-provider', config: { baseUrl, auth: { type: 'apikey', apiKey } } }, dependencies);
  ...
  console.log('📤 Sending transformed request to Qwen...');
  ...
  console.log('✅ Real response received from Qwen');
  ...
  writeJSON(path.join(outDir, 'qwen-tool-calling-real-response.json'), realResponse);
  ...
  const compatAnalyzer = new AnalysisModule('qwen-compatibility-analyzer', 'compatibility', realCompatibility, 'response-transformation');
  const llmSwitchAnalyzer = new AnalysisModule('qwen-llm-switch-analyzer', 'llm-switch', realLlmSwitch, 'response-transformation');
  ...
  console.log('\n📊 Analyzing Qwen Compatibility Module...');
  ...
  const compatRequest = {
    data: realResponse,
    route: { providerId: 'qwen', modelId: toolCallingRequest.model, requestId: `qwen_compat_analysis_${Date.now()}`, timestamp: Date.now() },
    metadata: { source: 'qwen-tool-calling-analysis', stage: 'compatibility-analysis' },
    debug: { enabled: true, stages: {} }
  };

  const compatResult = await dryRunPipelineExecutor.executePipeline(compatRequest, 'qwen-tool-calling-compatibility', 'dry-run');
  ...
  console.log('\n📊 Analyzing Qwen LLM Switch Module...');
  ...
  const llmSwitchRequest = {
    data: llmSwitchInput,
    route: { providerId: 'qwen', modelId: toolCallingRequest.model, requestId: `qwen_llmswitch_analysis_${Date.now()}`, timestamp: Date.now() },
    metadata: { source: 'qwen-tool-calling-analysis', stage: 'llm-switch-analysis', previousStage: 'compatibility' },
    debug: { enabled: true, stages: {} }
  };

  const llmSwitchResult = await dryRunPipelineExecutor.executePipeline(llmSwitchRequest, 'qwen-tool-calling-llm-switch', 'dry-run');
  ...
  const comprehensiveReport = {
    ...
    transformationChain: [
      {
        ...
        module: 'qwen-compatibility',
        analysis: compatAnalyzer.getAnalysisData()
      },
      {
        from: 'Compatibility Processed',
        to: 'LLM Switch Final',
        module: 'qwen-llm-switch',
        analysis: llmSwitchAnalyzer.getAnalysisData()
      }
    ],
    overallInsights: {
    ...
    qwenSpecific: {
      ...
      performance: {
        responseTime: realResponse.metadata?.processingTime || 0,
  ...
  if (!comprehensiveReport.qwenSpecific.modelBehavior.usedToolCalling) {
    comprehensiveReport.recommendations.optimization.push('Qwen model did not use tool calling - consider prompt engineering or model selection');
  }

  if (comprehensiveReport.qwenSpecific.performance.responseTime > 5000) {
    comprehensiveReport.recommendations.performance.push('Qwen response time is high - consider optimization');
  }

  // Save all reports
  writeJSON(path.join(outDir, 'qwen-tool-calling-compatibility-result.json'), compatResult);
  writeJSON(path.join(outDir, 'qwen-tool-calling-llm-switch-result.json'), llmSwitchResult);
  writeJSON(path.join(outDir, 'qwen-tool-calling-comprehensive-report.json'), comprehensiveReport);
  writeJSON(path.join(outDir, 'qwen-tool-calling-original-request.json'), toolCallingRequest);

  console.log('\n✅ Qwen Tool Calling Analysis Finished!');
  console.log('📁 Generated Files:');
  console.log('  - Original request: tests/output/qwen-tool-calling-original-request.json');
  console.log('  - Real response: tests/output/qwen-tool-calling-real-response.json');
  console.log('  - Compatibility result: tests/output/qwen-tool-calling-compatibility-result.json');
  console.log('  - LLM Switch result: tests/output/qwen-tool-calling-llm-switch-result.json');
  console.log('  - Comprehensive report: tests/output/qwen-tool-calling-comprehensive-report.json');
  ...
  console.log(`  - Used tool calling: ${comprehensiveReport.qwenSpecific.modelBehavior.usedToolCalling ? '✅' : '❌'}`);
  console.log(`  - Used text response: ${comprehensiveReport.qwenSpecific.modelBehavior.usedTextResponse ? '✅' : '❌'}`);
  console.log(`  - Response time: ${comprehensiveReport.qwenSpecific.performance.responseTime}ms`);
  console.log(`  - Tokens used: ${comprehensiveReport.qwenSpecific.performance.tokensUsed}`);
...
if (process.argv[1] && process.argv[1].endsWith('qwen-tool-calling-analysis.mjs')) {
  main().catch(err => {
    console.error('Failed to run Qwen tool calling analysis:', err?.stack || String(err));
    process.exit(1);
  });
}