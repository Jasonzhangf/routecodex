...
- Provider runtime 不得把 `apply_patch` 映射为未证明等价的 provider-native edit/write 工具。
...
- provider-specific `apply_patch -> native edit/write` bridge。
- 未证明等价的 `exec_command -> provider-native shell` bridge。
...
6. Provider runtime：`apply_patch/exec_command` 不得进入未证明等价的 provider-native 工具映射；需要本地执行时走 RCC servertool/client-visible 工具链。
