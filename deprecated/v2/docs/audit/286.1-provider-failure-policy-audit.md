...
| 字段 | 值 |
|------|------|
| 决策 | 判定错误是否为"阻塞可恢复"（需要进入统一 1s/2s/3s blocking backoff 队列，而非快速重试） |
...
| 字段 | 值 |
|------|------|
| 决策 | 计算重试延迟（2026-06-09 后不得解析 Retry-After 或本地指数退避） |
| 迁移去向 | → 保持为通用 wait 工具，延迟值由统一 error action queue 产出 |
