...
### 统一原则
- 函数化: 单一职责、纯输入纯输出、可独立测试
- 原子化: 功能切至最小单元；组合而非耦合
- 模块化: 高内聚、低耦合；对外仅暴露稳定契约（types/exports）

### 量化约束
- 单文件长度 < 500 行；超过必须拆分并抽公共函数
- 每个模块有独立 README.md（见模板）
- 模块可独立构建/测试（无隐藏全局状态）
- 禁止重复实现：复用优先于新增；抽象优先于粘贴

### 流程（先文档后代码）
1. 功能扫描: 查已有实现/文档（rg/README/导出接口）
2. 设计文档: 在模块 README 中补齐“目的/边界/API/依赖/示例/变更”
3. 接口契约: 明确输入/输出/错误与不变量（Fail Fast）
4. 代码实现: 以函数化/原子化/模块化落地
5. 自测验证: 单测/端到端/快照（针对边界与失败路径）
6. 文档更新: README 与变更记录同步

### Do / Don’t
- Do:
  - 先复用再扩展；发现重复立即抽取公共模块
  - 失败即报（无兜底隐式 fallback）
  - 输出结构化错误（code/type/status/details）
- Don’t:
  - 引入跨模块循环依赖/隐式全局状态
  - 未文档化的导出接口/魔法参数
  - 仅为“复用”而过度抽象（保持可读性优先）
...
### 开发检查清单（PR Gate）
- [ ] 已扫描现有功能与导出接口（无重复）
- [ ] 模块 README 完整（目的/边界/API/依赖/示例/变更）
- [ ] 单一职责与文件 < 500 行
- [ ] 可独立构建/测试，错误 Fail Fast
- [ ] 接口契约稳定（输入/输出/错误/不变量明确）

### 模块 README 模板（建议）
- 名称与目标：本模块解决什么问题？边界/不做什么？
- 公共 API（输入/输出/错误）：函数/类型签名与不变量；失败时的结构化错误
- 依赖与约束：上下游接口、环境/配置、禁用的隐性依赖
- 使用示例：最小可运行示例（含失败路径示例）
- 设计说明：关键设计取舍、扩展点、与相邻模块的边界
- 变更记录：版本/日期/影响范围/迁移指引
```
...
- [ ] **配置外部化**: 配置是否外部化且验证？
- [ ] **模块化设计**: 代码结构是否模块化可维护？
- [ ] **函数化原则**: 是否将功能拆分为独立的纯函数？
- [ ] **原子化设计**: 是否将复杂功能拆分为最小操作单元？
- [ ] **独立化实现**: 模块是否自包含且无循环依赖？
- [ ] **重复功能检查**: 是否扫描并避免了重复实现？
- [ ] **README完整性**: 每个模块是否有完整的README文档？
- [ ] **文档先行**: 是否先写文档再写代码？
- [ ] **推导过程**: 是否提供完整的分析和推导过程？
...
**最后更新**: 2025-11-15
**版本**: 0.81.29
9. **模块化** - 文件<500行，函数化、原子化设计

## 🚨 核心开发方法论：函数化、原子化、独立化

### 三大核心原则

**函数化**: 每个功能实现为独立的纯函数，便于测试、复用和组合
- 单一职责：一个函数只做一件事
- 无副作用：函数输出仅依赖输入参数
- 易于测试：纯函数天然适合单元测试
- 可组合性：小函数可以组合成复杂功能

**原子化**: 将复杂功能拆分为最小的不可分割单元
- 最小操作：每个原子操作完成一个最小的业务单元
- 确定性：原子操作要么完全成功，要么完全失败
- 可追踪：每个原子操作都有明确的输入输出和边界
- 可组合：原子操作可以组合成更大的业务流程

**独立化**: 每个模块自包含，不依赖其他模块的内部实现
- 自包含：模块拥有完整的功能实现
- 清晰接口：通过明确的接口与外界交互
- 无循环依赖：模块之间不允许循环依赖
- 可独立测试：模块可以独立进行功能验证
...
**1. 功能扫描机制**
- 开发前必须扫描现有代码库，确认功能是否已实现
- 使用以下命令扫描：`rg --type ts "function.*功能名称" src/`
- 发现已实现功能时，优先复用而非重复开发
- 在模块README中记录功能清单，避免重复开发

**2. 文档先行的开发流程**
- 先写README.md，定义模块职责、接口、边界
- 再写类型定义(.d.ts)，明确输入输出规范
- 最后写实现代码，严格按照文档执行
- 文档与代码不一致时，以文档为准，及时更新代码

**3. 模块README强制要求**
每个模块目录必须包含README.md，包含以下章节：
- **模块职责**: 明确模块的核心功能和边界
- **依赖关系**: 列出所有外部依赖和被依赖关系
- **接口定义**: 主要函数/类的签名和行为说明
- **使用示例**: 典型用法和配置示例
...
**5. 代码组织原则**
- 文件大小：单个文件不超过500行（必要时拆分）
- 函数大小：单个函数不超过50行（必要时拆分）
- 函数参数：不超过5个参数（必要时使用对象封装）
- 嵌套层级：代码嵌套不超过3层
- 复杂度圈复杂度：不超过10
...
**复用检查工具**：
- 功能扫描：`rg "功能关键词" src/ --type ts`
- 接口对比：检查现有接口是否满足需求
- 性能评估：评估现有实现的性能是否满足要求
- 兼容性检查：确认现有实现的兼容性范围

**重复开发处罚机制**：
- Code Review阶段标记重复功能，要求重构
- CI检查：设置重复代码检测阈值（>80%相似度报错）
- 绩效考核：重复开发记录影响技术债务评估
- 重构要求：发现重复功能必须安排重构计划
