...
import { ensureSnapshotRuntimeMarker, pruneSnapshotRequestDirsKeepRecent, resolveSnapshotKeepRecentRequestDirs, } from '../../utils/snapshot-request-retention.js';
...
import { readRuntimeControlProjection, readRuntimeRequestTruthIdentifiers } from '../../server/runtime/http-server/metadata-center/request-truth-readers.js';
...
function logHookNonBlockingError(operation, error) {
    const reason = error instanceof Error ? error.message : String(error);
    console.warn(`[snapshot-writer] ${operation} failed (non-blocking): ${reason}`);
}
function isExplicitOversizeSnapshotArtifact(payload) {
    ...
    return oversize.kind === 'snapshot_payload_oversize';
}
// --- internal helpers ---
...
function requiresPortScopedSnapshotDir(stage) {
    const normalized = String(stage || '').trim().toLowerCase();
    return normalized.startsWith('client-')
        || normalized.startsWith('provider-');
}
function resolveRequiredEntryPort(stage, entryPort) {
...
function logSnapshotNonBlockingError(operation, error) {
    const reason = error instanceof Error ? error.message : String(error);
    console.warn(`[snapshot-writer] ${operation} failed (non-blocking): ${reason}`);
}
...
function buildSnapshotPayload(input) {
    ...
    if (input.entryEndpoint)
        meta.entryEndpoint = input.entryEndpoint;
    if (typeof input.entryPort === 'number') {
        meta.entryPort = input.entryPort;
        meta.matchedPort = input.entryPort;
    }
    if (input.extraMeta)
        Object.assign(meta, input.extraMeta);
    const result = { meta };
    if (input.url)
        result.url = input.url;
    if (input.headers)
        result.headers = redactSensitiveData(maskHeaders(input.headers));
...
function buildSnapshotRuntimeMetadata(metadata) {
    if (!metadata || typeof metadata !== 'object') {
        return undefined;
    }
    const projected = {};
    for (const key of ['stream', 'userAgent', 'entryEndpoint', 'clientRequestId']) {
...
function resolveSnapshotDir(folder, groupRequestId, entryPort) {
...
// --- write ---
export async function writeUnifiedSnapshot(input) {
    ...
    if (!canWriteSnapshotToLocalDisk(input.requestId, groupRequestId))
        return;
