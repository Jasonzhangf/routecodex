...
const PROVIDER_SNAPSHOT_QUEUE = [];
let providerSnapshotQueueBytes = 0;
let providerSnapshotDrainScheduled = false;
let providerSnapshotDrainInFlight = null;
let providerSnapshotDroppedCount = 0;
let providerSnapshotLastDropLogAt = 0;
function resolveProviderSnapshotQueueMaxItems() {
    return resolvePositiveIntegerFromEnv(['ROUTECODEX_PROVIDER_SNAPSHOT_QUEUE_MAX_ITEMS', 'RCC_PROVIDER_SNAPSHOT_QUEUE_MAX_ITEMS'], DEFAULT_PROVIDER_SNAPSHOT_QUEUE_MAX_ITEMS);
}
function resolveProviderSnapshotQueueMemoryBudgetBytes() {
    return resolvePositiveIntegerFromEnv(['ROUTECODEX_PROVIDER_SNAPSHOT_QUEUE_MEMORY_BUDGET_BYTES', 'RCC_PROVIDER_SNAPSHOT_QUEUE_MEMORY_BUDGET_BYTES'], DEFAULT_PROVIDER_SNAPSHOT_QUEUE_MEMORY_BUDGET_BYTES);
}
function resolveProviderSnapshotQueueBatchSize() {
    return resolvePositiveIntegerFromEnv(['ROUTECODEX_PROVIDER_SNAPSHOT_QUEUE_BATCH_SIZE', 'RCC_PROVIDER_SNAPSHOT_QUEUE_BATCH_SIZE'], DEFAULT_PROVIDER_SNAPSHOT_QUEUE_BATCH_SIZE);
}
function estimateProviderSnapshotQueueBytes(input) {
    try {
...
function logNonBlockingError(operation, error) {
    const reason = error instanceof Error ? error.message : String(error);
    console.warn(`[provider-snapshot] ${operation} failed (non-blocking): ${reason}`);
}
function flushProviderSnapshotDropLog(force = false) {
    ...
    console.warn(`[provider-snapshot] queue overflow: dropped ${dropped} old snapshot task(s) ` +
...
async function drainProviderSnapshotQueue() {
...
function scheduleProviderSnapshotDrain() {
...
function enqueueProviderSnapshotPersist(input) {
    ...
    while (PROVIDER_SNAPSHOT_QUEUE.length > 0
