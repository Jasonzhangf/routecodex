...
export async function writeArtifacts(outDir: string, arts: Artifacts): Promise<void> {
  ...
  const add = (name: string, data: any, alsoWithPort = false) => {
  ...
  const mergedOut = (() => {
    ...
    try {
      if (arts.assemblerConfig && typeof arts.assemblerConfig === 'object') {
        ...
        (enriched as any).pipeline_assembler = arts.assemblerConfig;
        return enriched;
      }
    } catch { /* ignore */ }
    return base;
