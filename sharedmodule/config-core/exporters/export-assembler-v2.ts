...
export function exportAssemblerConfigV2(canonical: any) {
  if (!canonical || typeof canonical !== 'object') {
    throw new Error('exportAssemblerConfigV2: canonical is required');
  }

  let pipelinesIn: any[] = Array.isArray(canonical.pipelines) ? canonical.pipelines : [];
  ...
  if (pipelinesIn.length === 0) {
    const providers: Record<string, any> = (canonical.providers && typeof canonical.providers === 'object') ? canonical.providers : {};
    const keyVault: Record<string, Record<string, any>> = (canonical.keyVault && typeof canonical.keyVault === 'object') ? canonical.keyVault : {};
    const allIds = new Set<string>();
    for (const ids of Object.values(routePoolsIn as Record<string, string[]>)) {
      (ids || []).forEach((id) => { if (typeof id === 'string' && id.trim()) allIds.add(id); });
    }
    const requireProviderType = (pid: string): string => {
      ...
      if (!t) throw new Error(`exportAssemblerConfigV2: providers['${pid}'].type is required`);
      return t;
    };
    ...
    for (const id of Array.from(allIds)) {
      ...
      let compatModuleConfig: Record<string, unknown> = {};
      try {
        const models = (provCfgSrc && typeof provCfgSrc.models === 'object') ? provCfgSrc.models as Record<string, any> : {};
        ...
        if (!t) throw new Error(`exportAssemblerConfigV2: compatibility.type/moduleType required for ${providerId}.${modelId}`);
        compatModuleType = (t.toLowerCase() === 'compatibility') ? String(compat.moduleType || '') : t;
        if (!compatModuleType) throw new Error(`exportAssemblerConfigV2: compatibility.moduleType required for ${providerId}.${modelId}`);
      ...
      const shapeVariant = (() => {
        const c = compatModuleConfig as any;
        ...
        if (prof) return `shape-filters.${prof}.json`;
  ...
  const normalizeCompat = (compat: any, pipelineId: string): any => {
    ...
    const moduleType = String((config as any).moduleType || '').trim();
    if (!moduleType) {
    ...
    return {
      ...
      config: {
        moduleType,
        moduleConfig: (config as any).moduleConfig || {},
      }
    };
  };
  ...
  const pipelines = pipelinesIn.map((p) => {
    ...
    if (!id) throw new Error('exportAssemblerConfigV2: pipeline.id is required');
    const modules = (p && typeof p.modules === 'object') ? p.modules : {};

    // provider 配置：统一放入 openai provider 模块的 config 中
    const providerCfg = (modules as any).provider?.config || {};
    const compatCfg = normalizeCompat((modules as any).compatibility, id);
    ...
    const aliasProviderType = (s: string): string => {
      const t = (s || '').toLowerCase();
      if (t.includes('glm') || t.includes('zhipu') || t === 'glm') return 'glm';
    ...
    const compatAny = (compatCfg.config as any);
    ...
    const llmSwitch = (modules as any).llmSwitch?.type
      ? { type: (modules as any).llmSwitch.type, config: (modules as any).llmSwitch.config || {} }
      : { type: 'llmswitch-conversion-router', config: {} };

    const workflow = (modules as any).workflow?.type
      ? { type: (modules as any).workflow.type, config: (modules as any).workflow.config || {}, enabled: (modules as any).workflow.enabled !== false }
  ...
  try {
    ...
    for (const prov of Object.keys(kv as any)) {
      const keys = (kv as any)[prov] || {};
      for (const [kid, ent] of Object.entries(keys)) {
        const val = (ent as any)?.value;
  ...
  return {
    config: {
      ...
      routeMeta: Object.keys(routeMetaIn).length ? routeMetaIn : (() => {
        ...
        const meta: Record<string, { providerId: string; modelId: string; keyId: string }> = {};
