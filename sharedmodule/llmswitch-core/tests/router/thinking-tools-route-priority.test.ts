...
describe("virtual-router thinking vs declared tools priority", () => {
  ...
  test("routes current user thinking turn to thinking when tools are declared", () => {
    const result = classifyRouteWith(
      [
        {
          role: "user",
          content: "Please think step by step and explain the root cause.",
        },
      ],
      [
        {
  ...
  test("keeps read followup context on thinking when latest turn is assistant tool continuation", () => {
    ...
    const features = buildRoutingFeatures(req, {
      requestId: "req_test_followup",
    } as any);
    const result = new RoutingClassifier({}).classify(features);

    expect(result.routeName).toBe("thinking");
    expect(result.reasoning).toContain("thinking:last-tool-thinking");
  ...
  test("current user turn ignores previous search continuation and still prefers thinking", () => {
          ...
          tool_calls: [
            {
              ...
              function: {
                ...
                arguments: JSON.stringify({
                  cmd: 'rg -n "routing" sharedmodule/llmswitch-core',
                }),
              },
            },
          ],
    ...
    const features = buildRoutingFeatures(req, {
      requestId: "req_test_user_overrides_search",
    } as any);
    const result = new RoutingClassifier({}).classify(features);
  ...
  test("current responses user turn still prefers thinking even when previous turn mixed search and read", () => {
    const req = {
      model: "gpt-test",
      messages: [],
      tools: [
          ...
          context: {
            input: [
              ...
              },
              {
                ...
                name: "exec_command",
                arguments: JSON.stringify({
                  cmd: "cat sharedmodule/llmswitch-core/src/router/virtual-router/classifier.ts",
                }),
              },
              {
    ...
    const features = buildRoutingFeatures(req, {
      requestId: "req_test_responses_search",
    } as any);
    const result = new RoutingClassifier({}).classify(features);
  ...
  test("responses tool continuation after latest user boundary does not route as current user input", () => {
    ...
    const features = buildRoutingFeatures(req, {
      requestId: "req_test_responses_current_tool_turn",
    } as any);
    const result = new RoutingClassifier({}).classify(features);
