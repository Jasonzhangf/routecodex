...
#[napi(js_name = "planProviderResponseStoplessRuntimeControlEffectJson")]
pub fn plan_provider_response_stopless_runtime_control_effect_json(
    input_json: String,
) -> NapiResult<String> {
    hub_pipeline_lib::effect_plan::plan_provider_response_stopless_runtime_control_effect_json(
        input_json,
    )
}
...
#[napi]
pub fn should_allow_direct_responses_prebuilt_sse_passthrough_json(
    input_json: String,
) -> NapiResult<bool> {
    let raw: serde_json::Value = serde_json::from_str(&input_json)
        .map_err(|e| napi::Error::from_reason(format!("parse input: {}", e)))?;
