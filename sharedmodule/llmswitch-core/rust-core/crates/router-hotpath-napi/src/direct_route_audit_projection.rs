...
fn project(payload: &Value) -> Value {
    let observed_fields = payload.as_object().map(|record| {
        OBSERVABLE_FIELDS.iter().filter_map(|field| {
            record.get(*field).map(|value| json!({"field": field, "value": value}))
        }).collect::<Vec<_>>()
    }).unwrap_or_default();
...
pub fn project_direct_route_audit_fields_json(input_json: String) -> NapiResult<String> {
    let input: Value = serde_json::from_str(&input_json)
        .map_err(|error| napi::Error::from_reason(format!("direct audit projection input parse failed: {error}")))?;
    serde_json::to_string(&project(input.get("payload").unwrap_or(&Value::Null)))
        .map_err(|error| napi::Error::from_reason(format!("direct audit projection output serialize failed: {error}")))
}
...
mod tests {
    ...
    fn projects_only_ordered_observable_fields() {
        let output = project(&json!({"thinking":"high","model":"m","messages":[],"max_tokens":5,"reasoning":{"effort":"high"}}));
