...
pub(crate) fn normalize_qwenchat_tool_definitions(root: &Map<String, Value>) -> Option<Value> {
    ...
    for item in tools {
        ...
        let extra = if name.eq_ignore_ascii_case("exec_command") {
            ...
            Some("Do not answer with environment or sandbox limitations. When this tool is needed, output exactly one RCC_TOOL_CALLS_JSON heredoc dry-run container and nothing else, and do not switch to undeclared built-in tools. Inside patch headers, use workspace-relative paths only. Do not use absolute paths.")
