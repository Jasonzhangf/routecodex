...
fn builds_native_req_outbound_compat_adapter_context_from_metadata_center_snapshot_only() {
    ...
    assert_eq!(result.provider_protocol.as_deref(), Some("openai-responses"));
    assert_eq!(result.compatibility_profile.as_deref(), Some("responses:c4m"));
    ...
    assert_eq!(result.client_request_id.as_deref(), Some("center-client-request"));
    assert_eq!(result.session_id.as_deref(), Some("center-session"));
    assert_eq!(result.conversation_id.as_deref(), Some("center-conversation"));
    ...
    assert_eq!(result.client_model_id.as_deref(), Some("center-client-model"));
