...
- 黄金样例存放：`~/.rcc/codex-samples/openai-responses/lmstudio-golden`
...
输出：
- `~/.rcc/codex-samples/exp2-responses-to-chat/*.converted.chat.request.json`
- `~/.rcc/codex-samples/exp2-responses-to-chat/*.chat.events.ndjson`
- `~/.rcc/codex-samples/exp2-responses-to-chat/*.chat.final.json`
...
> **快照与输出路径**
>
> 实验脚本默认写入 `~/.rcc/codex-samples/exp*-*`，若运行环境限制 `$HOME` 写入，可在执行命令前设置：
> ```bash
...
命令：
```bash
# 默认读取 ~/.rcc/codex-samples/openai-responses/lmstudio-golden 下最新的 events
npm run replay:responses:chat-sse

# 指定样本 / 输出文件前缀
npm run replay:responses:chat-sse -- \
  --events ~/.rcc/codex-samples/openai-responses/lmstudio-golden/lmstudio-resp-test.events.ndjson \
  --out lmstudio-resp-test
```

输出：
- `~/.rcc/codex-samples/exp3-responses-to-chat-sse/<label>.responses.json`：
  Responses SSE 聚合后的 JSON。
- `~/.rcc/codex-samples/exp3-responses-to-chat-sse/<label>.chat.response.json`：
  经 `buildChatResponseFromResponses` 转换后的 Chat JSON。
- `~/.rcc/codex-samples/exp3-responses-to-chat-sse/<label>.chat.sse.ndjson`：
  由 `json-to-chat-sse` 合成的 Chat SSE 事件日志（NDJSON）。
- `~/.rcc/codex-samples/exp3-responses-to-chat-sse/<label>.chat.sse.txt`：
...
输出：
- `~/.rcc/codex-samples/exp4-responses-loop/<label>.responses.json`：
  Responses 聚合后的 JSON（与实验 3 共享）。
- `~/.rcc/codex-samples/exp4-responses-loop/<label>.replay.events.ndjson`：
  由 `json-to-responses-sse` 重建的 SSE 事件日志。
- `~/.rcc/codex-samples/exp4-responses-loop/<label>.replay.sse.txt`：
...
- 黄金样例事件：`~/.rcc/codex-samples/openai-responses/lmstudio-golden/*.events.ndjson`
- llmswitch-core 实验事件：`~/.rcc/logs/sse/*.log`
- Pipeline 聚合：`~/.rcc/codex-samples/openai-responses/req_*_pipeline.aggregate.json`

对比方法：
```bash
# 示例：比较黄金样例与当前桥接输出的 event 类型序列
rg -o '"type":"[^"]+"' ~/.rcc/codex-samples/openai-responses/lmstudio-golden/<sample>.events.ndjson
rg -o 'event: [^ ]+' ~/.rcc/logs/sse/<bridge>.log
```
...
命令：
```bash
# 默认读取 ~/.rcc/codex-samples/openai-chat/lmstudio-golden 最新样本
npm run replay:chat:bridge

# 指定样本与 provider 元数据
npm run replay:chat:bridge -- \
  --events ~/.rcc/codex-samples/openai-chat/lmstudio-golden/full-chat-experiment-1-1763865876.events.ndjson \
  --profile lmstudio \
  --provider lmstudio \
...
输出：
- `~/.rcc/codex-samples/exp5-chat-bridge/<label>.chat.response.json`
- `~/.rcc/codex-samples/exp5-chat-bridge/<label>.chat.compat.json`
- `~/.rcc/codex-samples/exp5-chat-bridge/<label>.chat.replay.sse.ndjson`
- `~/.rcc/codex-samples/exp5-chat-bridge/<label>.chat.replay.sse.txt`
