...
export function normalizeExecCommandArgs(args: unknown): ExecCommandNormalizeResult {
  const base = unwrapExecArgsShape(args);
  const cmdCandidate =
  ...
  const workdir = asNonEmptyString(base.workdir) ?? asNonEmptyString(base.cwd) ?? asNonEmptyString(base.workDir);
  ...
  const timeoutMs = asFiniteNumber(base.timeout_ms) ?? asFiniteNumber(base.timeoutMs);
  ...
  const yieldTime = asFiniteNumber(base.yield_time_ms) ?? asFiniteNumber(base.yield_ms) ?? asFiniteNumber(base.wait_ms);
