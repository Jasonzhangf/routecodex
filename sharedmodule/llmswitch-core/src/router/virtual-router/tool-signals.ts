...
const THINKING_TOOL_EXACT = new Set([
  "update_plan",
  "read",
  "read_file",
  "read_text",
...
const SHELL_THINKING_COMMANDS = new Set([
  "cat",
  "head",
  "tail",
  "strings",
...
const SHELL_REDIRECT_WRITE_BINARIES = new Set([
  "cat",
  "printf",
  "python",
  "node",
...
const GIT_SEARCH_SUBCOMMANDS = new Set([
  "grep",
  "log",
  "shortlog",
  "reflog",
...
export function detectVisionTool(request: StandardizedRequest): boolean {
  ...
  return request.tools.some((tool) => {
    ...
    return (
      /vision|image|picture|photo/i.test(functionName) ||
      /vision|image|picture|photo/i.test(description || "")
    );
  });
}
...
export function detectCodingTool(request: StandardizedRequest): boolean {
  ...
  return request.tools.some((tool) => {
    ...
    if (WRITE_TOOL_EXACT.has(functionName) || functionName === "exec_command") {
      return true;
    }
 ...
 */
export function detectWebSearchToolDeclared(
...
export function extractMeaningfulDeclaredToolNames(
  tools: StandardizedRequest["tools"] | undefined,
): string[] {
...
export function detectLastAssistantToolCategory(
...
function classifyToolCall(
  ...
  const isWebSearch = WEB_TOOL_KEYWORDS.some((keyword) =>
    normalizedName.includes(keyword),
  );

  // 2) 基于工具名的初步分类（thinking / coding / search / other）
  const nameCategory = categorizeToolName(functionName);

  // 3) shell_command / exec_command 根据内部命令判断 route 语义
  ...
  // 按优先级合并分类结果：
  //   1. web search
  //   2. coding（任一维度命中修改/编辑语义）
  //   3. thinking（任一维度命中读取/计划语义）
  //   4. 其他搜索（非 web search）
  //   5. 其它工具
  ...
  // Priority 2: Coding（编辑/修改语义）
  if (nameCategory === "coding" || shellCategory === "coding") {
    return { category: "coding", name: functionName, commandSnippet: snippet };
  }

  // Priority 3: Thinking（读取/计划语义）
  if (nameCategory === "thinking" || shellCategory === "thinking") {
    return {
      category: "thinking",
      name: functionName,
      commandSnippet: snippet,
  ...
  if (
    !SHELL_TOOL_NAMES.has(functionName) &&
    functionName !== "exec_command" &&
    commandText
  ) {
    const derivedCategory = classifyShellCommand(commandText);
    if (
      derivedCategory === "coding" ||
      derivedCategory === "thinking" ||
      derivedCategory === "search"
    ) {
      return {
...
function extractToolName(
  tool: StandardizedRequest["tools"] extends Array<infer T> ? T : never,
): string {
  ...
  if (
    fromFunction &&
    typeof fromFunction.name === "string" &&
    fromFunction.name.trim()
  ) {
...
function extractToolDescription(
  ...
  if (
    fromFunction &&
    typeof fromFunction.description === "string" &&
    fromFunction.description.trim()
  ) {
...
function categorizeToolName(name: string): ToolCategory {
  ...
  if (
    ...
    SEARCH_TOOL_KEYWORDS.some((keyword) =>
  ...
  if (
    THINKING_TOOL_EXACT.has(normalized) ||
    THINKING_TOOL_KEYWORDS.some((keyword) =>
      normalized.includes(keyword.toLowerCase()),
    )
  ) {
    return "thinking";
...
function classifyShellCommand(command: string): ToolCategory {
  ...
  for (const segment of segments) {
    ...
    for (const args of normalized.commands) {
      ...
      if (isCodingBinary(alias, rest, normalized.raw)) {
        return "coding";
      }
      if (isThinkingBinary(alias, rest, normalized.raw)) {
        sawThinking = true;
        continue;
      }
...
function normalizeShellSegment(
  segment: string,
): { raw: string; commands: string[][] } | undefined {
...
function isCodingBinary(
  ...
  if (joinedArgs.includes("replace")) {
    return true;
  }
  ...
  if (
    SHELL_REDIRECT_WRITE_BINARIES.has(normalized) &&
    OUTPUT_REDIRECT_PATTERN.test(rawSegment)
  ) {
    return true;
  }
  return false;
...
function isThinkingBinary(
  binary: string,
  args: string[],
  rawSegment: string,
): boolean {
  const normalized = binary.toLowerCase();
  if (SHELL_THINKING_COMMANDS.has(normalized)) {
    return true;
  }
