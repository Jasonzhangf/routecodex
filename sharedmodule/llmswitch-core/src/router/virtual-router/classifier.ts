...
export class RoutingClassifier {
  ...
  constructor(config: VirtualRouterClassifierConfig) {
    this.config = {
      longContextThresholdTokens:
        config.longContextThresholdTokens ?? DEFAULT_LONG_CONTEXT_THRESHOLD,
      thinkingKeywords: normalizeList(config.thinkingKeywords, [
        "think step",
        "analysis",
        "reasoning",
      ]),
      backgroundKeywords: normalizeList(config.backgroundKeywords, [
        "background",
        "context dump",
      ]),
    };
  ...
  classify(features: RoutingFeatures): ClassificationResult {
    ...
    const lastToolCategory = latestMessageFromUser
      ? undefined
      : features.lastAssistantToolCategory;
    const webSearchContinuation = lastToolCategory === "websearch";
    const localToolContinuation =
      lastToolCategory === "thinking" ||
      lastToolCategory === "coding" ||
      lastToolCategory === "search" ||
      lastToolCategory === "other";
    const reachedLongContext =
      features.estimatedTokens >=
      (this.config.longContextThresholdTokens ??
        DEFAULT_LONG_CONTEXT_THRESHOLD);
    const hasToolActivity =
      features.hasTools ||
      (!latestMessageFromUser && features.hasToolCallResponses);
    const thinkingContinuation = lastToolCategory === "thinking";
    ...
    const codingContinuation = lastToolCategory === "coding";
    const searchContinuation = lastToolCategory === "search";
    const toolsContinuation = lastToolCategory === "other";
    const hasRemoteVideoAttachment =
      features.hasVideoAttachment === true &&
      features.hasRemoteVideoAttachment === true;
    ...
    > = {
      ...
      coding: {
        ...
        reason: "coding:last-tool-coding",
      },
      web_search: {
      ...
      tools: {
        ...
        triggered:
          toolsContinuation || (!searchContinuation && hasToolActivity),
        reason: toolsContinuation
          ? "tools:last-tool-other"
          : "tools:tool-request-detected",
      },
      background: {
        triggered: containsKeywords(
          features.userTextSample,
          this.config.backgroundKeywords ?? [],
        ),
  ...
  private buildResult(
    ...
    const reasoningParts = [
      chosenReason,
      ...diagnostics.filter((reason) => reason !== chosenReason),
    ];
    return {
      routeName,
      confidence: 0.9,
