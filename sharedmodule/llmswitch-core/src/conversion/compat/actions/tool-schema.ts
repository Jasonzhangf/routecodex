...
type ToolDefinition = UnknownRecord & { function?: ToolFunction };
type ToolFunction = UnknownRecord & { name?: string; parameters?: ToolParameters; strict?: unknown };
type ToolParameters = UnknownRecord & {
...
type ToolProperty = UnknownRecord & {
  type?: string;
  description?: string;
  items?: UnknownRecord;
  oneOf?: unknown;
...
const sanitizeShellCommandProperty = (properties: Record<string, UnknownRecord>): void => {
...
const sanitizeToolFunction = (toolFn: ToolFunction, isShell: boolean): ToolFunction => {
  ...
  if (isShell) {
    ...
    sanitizeShellCommandProperty(params.properties as Record<string, UnknownRecord>);
...
const sanitizeToolDefinition = (tool: UnknownRecord): UnknownRecord => {
