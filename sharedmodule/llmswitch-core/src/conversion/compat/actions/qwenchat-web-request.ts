...
const readToolProtocol = (value: unknown): ToolProtocol | undefined => {
  if (typeof value !== 'string') {
    return undefined;
  }
  const normalized = value.trim().toLowerCase();
...
function resolveToolTextNode(adapterContext?: AdapterContext): Record<string, unknown> {
...
export function applyQwenChatWebRequestTransform(payload: JsonObject, adapterContext?: AdapterContext): JsonObject {
