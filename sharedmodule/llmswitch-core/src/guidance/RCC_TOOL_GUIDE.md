...
- 注入内容（要点，与 CCR 一致）：
  ...
  - apply_patch：仅发送补丁文本；模型输出时**只允许** canonical internal patch grammar：`*** Begin Patch` / `*** End Patch`，以及 `*** Add File:` / `*** Update File:` / `*** Delete File:` 头。若已使用 `*** Begin Patch`，块内不要再写 `--- a/...` / `+++ b/...`，也不要输出 GNU/git diff、shell heredoc、Markdown fences 或正文解释。注意 `*** Update File` 不会隐式创建文件；创建文件请用 `*** Add File:`。修改同一文件时尽量只提交一段连续补丁，多个不相邻位置请拆成多次调用。
