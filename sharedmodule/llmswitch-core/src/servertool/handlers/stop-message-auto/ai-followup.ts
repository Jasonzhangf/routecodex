...
import {
  ...
  sanitizeStopMessageAutoMessageOutput,
  truncateStopMessageAutoMessagePrompt,
  summarizeStopMessageAutoMessageLog,
  extractStopMessageAssistantText,
  extractStopMessageReasoningText,
  extractResponsesReasoningText,
  extractStopMessageReasoningFromContent,
  extractUnknownText,
  dedupeAndJoinTexts,
  buildStopMessageResponseExcerpt,
  asRecord,
  toNonEmptyText
} from './ai-followup-pure-blocks.js';
export { extractResponsesOutputText, hasToolLikeOutput } from './ai-followup-pure-blocks.js';
...
function buildStopMessageAutoMessagePrompt(args: {
  ...
  const lines: string[] = [
    ...
    '6.1) 只要你输出的 followup 消息要求主模型做 summary、最终总结、停止说明、完成/阻塞汇报，就必须同时要求主模型在回复末尾附一个 JSON 对象。',
    '6.2) 该 JSON 对象必须包含：stopreason(0=finished,1=blocked,2=continue_needed)、reason、has_evidence(0/1)、evidence、issue_cause、excluded_factors、diagnostic_order、done_steps、next_step、next_suggested_path、learned。',
    '6.3) 若缺证据则必须调用工具继续执行；不要接受空泛总结。',
  ...
  const closingInstruction =
    `现在做完成校验：若确认已完成，仅输出 ${approvedMarker}；否则输出“根据当前状态调整后的下一步 followup 消息文本”；若该文本要求 summary/最终总结/停止说明，必须明确要求主模型在回复末尾附一个 JSON 对象。`;
