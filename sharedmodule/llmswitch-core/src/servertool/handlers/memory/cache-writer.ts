...
/**
 * 兼容旧调用点：当前实现没有额外 fallback 分支，
 * 真正的唯一真源仍是 adapterContext / __rt 上的工作目录字段解析。
 */
export function resolveWorkingDirectoryFromAdapterContextOrFallback(
 ...
 */
export function resolveWorkingDirectoryFromAdapterContextOrUndefined(adapterContext: Record<string, unknown>): string | undefined {
  return resolveWorkingDirectoryFromAdapterContext(adapterContext);
}
