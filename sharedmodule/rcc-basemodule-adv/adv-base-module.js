 ...
 */
export class AdvBaseModule extends BaseModule {
    constructor() {
        super(...arguments);
        this.advDryRunConfig = {
            enabled: false,
            mode: 'partial',
            verbosity: 'normal',
        ...
        this.nodeDryRunConfigs = new Map();
    }
    // ---- Public API ----
    setDryRunMode(enabled, config) {
        this.advDryRunConfig.enabled = enabled;
        if (config)
            this.advDryRunConfig = { ...this.advDryRunConfig, ...config };
    }
    getDryRunConfig() {
        return { ...this.advDryRunConfig };
    }
    setNodeDryRunConfig(nodeId, config) {
        this.nodeDryRunConfigs.set(nodeId, config);
    }
    getNodeDryRunConfig(nodeId) {
     ...
     */
    async runWithDryRun(op, input, exec, options) {
    ...
    async executeNodeDryRun(input, ctx) {
    ...
    async simulateError(config) {
        return { message: 'simulated-error', config };
    }
    async estimatePerformance(_input) {
        return { estimatedTime: 5, estimatedMemory: 128, complexity: 1 };
    }
    async generateExpectedOutput(input, _nodeType) {
    ...
    async executeDryRunValidation(input, ctx, nodeCfg) {
        const expected = await this.generateExpectedOutput(input, ctx.nodeType);
    ...
    async executeDryRunErrorSimulation(input, ctx, nodeCfg) {
    ...
    createNodeDryRunResult(ctx, input, expected, status, validations, perf) {
        return {
            nodeId: ctx.nodeId,
            nodeType: ctx.nodeType,
            status,
    ...
    async onDryRunResult(ctx, result) {
    ...
    async pauseForInspection(ctx, result) {
    ...
    async emitDryRunEvent(type, ctx, data) {
        try {
