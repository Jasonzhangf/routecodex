/**
 * RouteCodex Unified Configuration Path Interface
 *
 * This module provides a unified interface for configuration path resolution
 * that can work both within the main application and in sharedmodule packages.
 */
...
/**
 * Configuration path resolution result
 */
...
const ENVIRONMENT_VARIABLES = [
  ...
  'RCC4_CONFIG_PATH',        // RCC4 specific (backward compatibility)
...
/**
 * Lightweight configuration path resolver for sharedmodule
 *
 * This provides a subset of the unified configuration path functionality
 * that can be used by sharedmodule packages without requiring the full
 * main application dependencies.
 */
export class SharedModuleConfigResolver {
  /**
  ...
  static resolveConfigPath(options: ConfigPathOptions = {}): ConfigPathResult {
    ...
    const fallbackPath = configName
      ...
      : path.join(this.getDefaultConfigDirectory(), 'default.json');
