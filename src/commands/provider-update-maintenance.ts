...
export function createAddCommand(): Command {
  return new Command('add')
    ...
    .option('-i, --id <id>', 'Provider id (e.g. my-openai, qwen, gemini, deepseek-web)')
    .option('--root <dir>', 'Override provider root directory')
    .action(async (opts: { id?: string; root?: string }) => {
      let providerId = (opts.id || '').trim();
      if (!providerId) {
        providerId = await ask('Provider id (e.g. my-openai, qwen, gemini, deepseek-web)', 'glm');
      }
      if (!providerId.trim()) {
        console.error('Provider id is required');
        process.exit(1);
      ...
      const authType = await ask('Auth type (e.g. apikey, oauth, qwen-oauth, deepseek-account)', authTypeDefault);
    ...
    .action(async (id: string, opts: { root?: string }) => {
      ...
      const currentAuthType =
        typeof authNode.type === 'string'
          ? authNode.type
          : 'apikey';
      const authType = await ask('Auth type (e.g. apikey, oauth, qwen-oauth, deepseek-account)', currentAuthType);
