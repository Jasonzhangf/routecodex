...
export class RouteCodexHttpServer {
  ...
  private async executeRouterDirectPipelineForPort(
    ...
    let directRetryBeyondAttemptBudget = false;
    ...
    try {
      ...
      onProviderError: async (error, ctx) => {
        ...
        const providerSendFailureResult = await processProviderSendFailure({
          error,
          requestId: input.requestId,
          providerKey: ctx.providerKey,
          providerId: directProviderHandle.providerId,
      ...
      if (
        ...
        && (directAttempt < retryState.maxAttempts || directRetryBeyondAttemptBudget)
      ) {
        return await this.executeRouterDirectPipelineForPort(portConfig, input, retryState, directAttempt + 1);
      }
      throw error;
  ...
  private async executeProviderDirectPipelineForPort(
    ...
    const payload = requireDirectPassthroughPayloadObject(input.body);
