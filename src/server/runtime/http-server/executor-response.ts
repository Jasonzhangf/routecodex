...
} from './executor/servertool-adapter-context.js';
import {
  executeServerToolReenterPipeline
} from './executor/servertool-followup-dispatch.js';
import {
...
export async function convertProviderResponseIfNeeded(
  ...
  try {
    ...
    const convertErrorPlan = finalizeServerToolBridgeConvertError({
      ...
      requestId: options.requestId,
      message,
      isSseDecodeError: providerProtocol === 'gemini-chat' && isSseConvertFailure
    });
    if (convertErrorPlan.handled) {
      deps.logStage('convert.bridge.error', options.requestId, convertErrorPlan.stageDetails);
      throw error;
    }
