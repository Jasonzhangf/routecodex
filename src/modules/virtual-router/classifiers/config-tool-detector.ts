/**
 * Configuration-Driven Tool Detector
 * 基于配置的工具检测器
 */
...
export interface ConfigToolDetectionConfig {
  type: 'pattern';
  patterns: ToolPatternConfig;
}

export interface ConfigToolDetectionResult {
  hasTools: boolean;
  toolCount: number;
  toolTypes: string[];
  toolCategories: {
...
export class ConfigToolDetector {
  private config: ConfigToolDetectionConfig;

  constructor(config: ConfigToolDetectionConfig) {
    this.config = config;
  }
  ...
  detect(
    ...
    return {
      ...
      toolTypes: [...new Set(allToolTypes)],
  ...
  private detectToolDefinitions(tools?: ChatCompletionTool[]): {
    ...
    for (const tool of tools) {
      ...
      if (tool.type === 'function') {
        const detectedType = this.detectToolTypeFromConfig(
          tool.function.name,
      ...
      if (message.role === 'assistant' && message.tool_calls) {
        ...
        for (const toolCall of message.tool_calls) {
          if (toolCall.type === 'function') {
            const detectedType = this.detectToolTypeFromConfig(toolCall.function.name);
            if (detectedType) {
              toolTypes.push(detectedType);
            }
          }
   ...
   * 基于配置检测单个工具的类型
   */
  private detectToolTypeFromConfig(name?: string, description?: string): string | null {
    if (!name && !description) {
      return null;
    }
  ...
  getDetailedAnalysis(
    ...
    detection: ConfigToolDetectionResult;
    insights: {
    ...
    return {
      ...
      insights: {
        ...
        toolUsagePattern,
        configDriven: detection.configBased
      },
      suggestions
    };
  }
  ...
  private generateAlternativeRoutes(detection: ConfigToolDetectionResult): Array<{
    route: string;
    confidence: number;
    reasoning: string;
  }> {
