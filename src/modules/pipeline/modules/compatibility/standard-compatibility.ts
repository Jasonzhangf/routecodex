...
export class StandardCompatibility implements PipelineModule {
  ...
  public async initialize(): Promise<void> {
    ...
    // Expect config.config like: { moduleType: 'glm' | 'qwen' | ..., moduleConfig?: {...}, providerType?: string }
    try {
      ...
      if (moduleType && this.manager) {
        const moduleId = await this.manager.createModule({
          id: `${moduleType}-${Date.now()}`,
          type: moduleType,
          providerType,
          enabled: true,
          priority: 1,
          profileId: `${moduleType}-profile`,
          config: moduleCfg
        });
        this.loadedModuleIds.push(moduleId);
        this.deps.logger?.logModule(this.id, 'module-loaded', { moduleId, moduleType, providerType });
      } else {
        this.deps.logger?.logModule(this.id, 'no-compat-config', { note: 'standard compatibility loaded without submodules' });
      }
      this.isReady = true;
    } catch (error) {
