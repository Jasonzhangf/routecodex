...
/**
  ...
  async sendRequest(request: UnknownObject): Promise<ProviderResponse> {
    ...
    try {
      ...
      if (dryRunEnabled) {
        ...
        // Decide path: real OpenAI SDK for api.openai.com, otherwise generic OpenAI-compatible fetch
        const providerCfg = this.config.config as ProviderConfig;
        ...
        if (isOpenAIDomain) {
          ...
          // Generic OpenAI-compatible HTTP fetch (no vendor-specific branches)
  ...
  // GLM compatibility logic has been moved to glm-compatibility module.
