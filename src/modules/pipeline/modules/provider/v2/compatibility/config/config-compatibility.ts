...
export class ConfigCompatibility implements CompatibilityModule {
  ...
  async initialize(): Promise<void> {
    ...
    if (!pathToJson && this.cfg.providerAlias) {
      ...
      // dist 路径：dist/modules/pipeline/modules/provider/v2/compatibility/config/config-compatibility.js
      // 目标路径：dist/modules/pipeline/modules/provider/v2/compatibility/<providerAlias>/config/shape-filters.json
      try {
