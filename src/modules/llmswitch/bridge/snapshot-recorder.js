...
function clipPreviewText(value, max = 240) {
...
function summarizeContentText(value, max = 220) {
    if (typeof value === 'string') {
    ...
    const text = summarizeContentText(row.content, Math.floor(max / 2))
...
function summarizeTail(value, max = 3200) {
    if (typeof value === 'string') {
        return clipPreviewText(value, max);
    }
    if (Array.isArray(value)) {
...
function readDirectErrorHint(payload) {
...
function shouldInspectToolFailures(stage) {
    if (!stage) {
        return false;
    }
...
function shouldInspectRuntimeErrorFast(stage, payload) {
...
function resolveRequestTailFromPayload(stage, payload) {
...
export async function createSnapshotRecorder(context, endpoint) {
    ...
    const runtimeErrorDedup = new Set();
    const clientToolErrorDedup = new Set();
    const emptyResponseDedup = new Set();
    let clientToolParseConsoleLogged = false;
    const stageTrace = [];
    let latestRequestTail = null;
    return {
        ...recorder,
        record(stage, payload) {
            if (stage && stage.startsWith('chat_process.req.') && payload && typeof payload === 'object') {
                try {
                    const resolved = resolveRequestTailFromPayload(stage, payload);
                    if (resolved) {
            ...
            try {
                ...
                if (toolFailures.length > 0) {
                    const requestId = typeof context?.requestId === 'string' ? String(context.requestId) : '';
                ...
                if (emptySignal) {
                    ...
                    const latestReqTraceEntry = [...stageTrace].reverse().find((entry) => typeof entry.stage === 'string' && entry.stage.startsWith('chat_process.req.'));
