...
async function importCoreDist<TModule extends object = AnyRecord>(
  ...
  try {
    return await import(resolveCoreModuleUrl(subpath, 'ts')) as TModule;
  } catch (error) {
    const detail = error instanceof Error ? error.message : String(error);
    throw new Error(
      `[llmswitch-bridge] Unable to load core module "${subpath}" (ts). 请确认 sharedmodule/llmswitch-core 依赖已安装（npm install）。${detail ? ` (${detail})` : ''}`
    );
  }
}
