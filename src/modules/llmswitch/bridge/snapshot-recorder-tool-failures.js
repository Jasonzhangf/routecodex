...
function clipText(input, max = 320) {
    const text = String(input || '').trim();
    if (!text)
        return '';
    return text.length > max ? `${text.slice(0, max)}...` : text;
}
export function classifyApplyPatchVerificationFailure(content) {
    const raw = String(content || '');
    const lower = raw.toLowerCase();
    const invalidHeaderTokenMatch = raw.match(/invalid hunk at line \d+,\s*'([^']+)' is not a valid hunk header/i);
    ...
    if (lower.includes('update file hunk for path') &&
    ...
    if (lower.includes('unexpected line found in update hunk') &&
        lower.includes("'@@'")) {
        return {
    ...
    if (lower.includes("expected update hunk to start with a @@ context marker, got: '======='") ||
...
export function detectToolExecutionFailures(payload) {
    return detectToolExecutionFailuresNative(payload);
}
export function classifyRuntimeErrorSignalFromText(stage, message) {
...
export function shouldLogClientToolErrorToConsole(failure) {
    if (failure.toolName === 'apply_patch') {
        return (failure.errorType === 'apply_patch_args_missing_input' ||
    ...
    if (failure.toolName === 'exec_command') {
        return (failure.errorType === 'exec_command_args_missing_cmd' ||
