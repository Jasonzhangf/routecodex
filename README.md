...
- Don't
  ...
  - Chat 上游真流式（OpenAI Chat SSE 直通）
    - 开关（默认关闭）：`ROUTECODEX_CHAT_UPSTREAM_SSE=1` 或 `RCC_CHAT_UPSTREAM_SSE=1`
    - 启用后：入口为 `/v1/chat/completions` 且 `stream:true` 时，Provider 发起上游 `text/event-stream` 请求；Pipeline 直接透传至客户端，无隐藏回退；未启用时由服务器基于 JSON 合成 Chat SSE
...
- 流式对齐验证（Chat）：`npm run verify:stream:chat`（对比上游 Chat SSE 与合成 Chat SSE 的事件级等价性）
