...
- Phase 0 generated artifact cleanup：`sharedmodule/llmswitch-core/src/**/*.js` / `.d.ts` / `.js.map` 是 ignored TS emit，不是 runtime source truth；Hub/VR source truth 目录（`src/conversion/hub`、`src/router/virtual-router`）不应保留 side-by-side emit。清理前必须用 `git check-ignore` 证明候选是 ignored artifact，再物理删除并用 residue gate 锁住；禁止把 side-by-side JS 当语义修复点。
