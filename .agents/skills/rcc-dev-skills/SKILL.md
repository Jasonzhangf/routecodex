...
- SSE 错误不可吞但也不可直投客户端：provider body、event codec、timeout、EOF 或 Runtime stream error 必须在 client commit 前进入 typed Error01→05；只要可选池或 default 池仍有目标就切换，只有两池同时耗尽才由 Error06 投影最终 HTTP 502。禁止 Server/SSE handler 把中间 provider 错误拼成 client `event:error`，也禁止用 body `io::Error`、直接 EOF、drop task 或仅写日志静默结束。发现错误发生在 client commit 后，先修 Runtime 的 pre-commit/commit authority 边界，禁止在 Server/outbound 补偿。
