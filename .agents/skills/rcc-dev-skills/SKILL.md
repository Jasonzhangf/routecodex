...
- empty-reply continue 边界（2026-06-08）：`empty_reply_continue` 不是 servertool followup runtime-action/runtime-metadata 切片；发现 `empty_reply_continue_contract`、`planEmptyReply*` native export、或 `handlers/empty-reply-continue.ts` 半链时，先移出当前提交，除非已作为独立 Rust-owned feature 完整实现并验证。
