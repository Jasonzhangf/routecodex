...
- servertool loop-warning / orchestration policy（2026-06-09）：stop-message loop warning text/count injection 属于 `followup-core::inject_loop_warning`，`stop-message-loop-payload-block.ts` 只能 build native seed payload，禁止恢复 `appendStopMessageLoopWarning` TS export、本地 `warningText`、`messages.push`、repeat clamp；servertool timeout parse、clientInjectOnly parse、client inject sanitize、followup error compact、adapter providerKey 解析属于 `servertool-core::orchestration_policy_contract::{parse_servertool_timeout_ms,read_client_inject_only,normalize_client_inject_text,compact_followup_error_reason,resolve_adapter_context_provider_key}`，TS `orchestration-policy-block.ts` 只能读 env/参数并调用 native wrapper，禁止恢复 `parseTimeoutMs`、`parseBooleanLike`、`sanitizeFollowupText`、HTTP/HTML regex compaction、providerKey walker 或 stop-gateway wrapper。
