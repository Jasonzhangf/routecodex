...
- `/v1/responses` Direct 遇到 `provider_response_sse_empty` 且候选池含 `openai_chat` / `anthropic` 等非 Responses provider protocol 时，先查是否把非 Responses provider 误送 `/responses`。正确修复是 provider 发送前的 execution planning 选择 Relay/provider-protocol dispatch；禁止在 SSE transport/server handler 补假事件、吞 EOF、改 provider config 或失败后 fallback 到 Relay。
