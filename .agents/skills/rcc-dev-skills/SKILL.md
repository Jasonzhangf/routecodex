...
- Virtual Router bootstrap 真源（2026-05-12）：若 live 日志出现 `Provider runtime <provider>.key1 not found`，先查 **native Rust bootstrap** `virtual_router_engine/provider_bootstrap.rs`，不要误改 TS `bootstrap/auth-utils.ts`（当前无调用点）；V2 `auth.entries` 里的空记录必须在 Rust `push_auth_entry_from_record` 入口直接忽略，否则会被默认 alias 规则物化成幽灵 `key1` runtime。
...
- stop schema 只校验数字字段 `stopreason` / `has_evidence`；`reason` / `next_step` / `evidence` 只判空。followup prompt 必须质询目标/过程/证据三项；`stopreason=0|1` 且 reason 非空才允许 stop 并 prefix summary；缺 schema 只 followup 不计预算；带 schema 的无效 stop 才计入连续预算，非 stop/工具进展必须 reset；预算耗尽 summary 必须聚合最近三轮追问、三轮回复与最后原始 summary。
