...
- stopless/servertool CLI projection 固定契约：`HubRespChatProcess03Governed -> stopless gate -> buildServertoolCliProjectionForAutoFlow -> exec_command CLI projection -> client`。`stop_message_auto` 不 reenter Hub Pipeline，不走 followup 编排，不调用 `reenterPipeline`。TS 只调用 Rust native gate + Rust client-visible projection shell，禁止判断工具语义、拼装 messages/tools、手写 `exec_command` tool_call shape、清洗工具列表、从 `rawBody` 正常构造请求、用 provider raw/client outbound/SSE payload 判定 stopless。
...
- stopless AI followup 删除（2026-06-09）：`stop-message-auto/ai-followup.ts` 与 `ai-followup-pure-blocks.ts` 已物理删除；`aiFollowup` config / `ROUTECODEX_STOPMESSAGE_AI_FOLLOWUP*` / `ROUTECODEX_STOPMESSAGE_AUTOMESSAGE*` 旧分支禁止恢复。stopless 只能走 Rust-owned CLI projection。
- stopless stop-schema 闭环（2026-06-13）：系统/continuation prompt 必须先注入 schema guidance；gate 必须只枚举缺失字段；`finished/blocked` 且 terminal 字段完整时立即允许停止；`continue_needed` 缺 `next_step` 时必须要求只补 `next_step`；三轮只作为 `no_change_count` loop guard，禁止把普通 `used/repeatCount` 当停止主流程预算。
...
- `/v1/responses` handler single-bridge 收口（2026-06-13）：server `responses-handler.ts` / `handler-response-utils.ts` 只允许 HTTP adapter、dispatcher、headers/status、timeout/abort、日志；SSE transport 实现必须物理隔离在 `handler-response-sse.ts`，共享非协议 helper 在 `handler-response-common.ts`。任何 request/response 协议判定、resume/projection payload shape、tool-history contract errorsample 判定都必须下沉到 `responses-request-bridge.ts` / `responses-response-bridge.ts`，并由 `verify-responses-handler-single-bridge-surface` gate 锁死。
- Responses continuation 三重隔离（2026-06-13）：continuation restore/materialize 必须同时锁 `entry protocol(or endpoint)`、`continuationOwner(direct|relay)`、`session/conversation(+port/group)`。`buildChatRequestFromResponses` 只负责已确认的 Responses 协议转换，不负责 scope 命中判定；看到 chat/messages 入口误命中 responses continuation 时，先修 `responses-conversation-store` / restore owner，不修 bridge 补偿。
- stopless/servertool CLI projection（2026-06-11 更新）：迁移后的 servertool 不走私有 server-side execution + followup/reenter，而是投影为客户端可见 `exec_command: routecodex servertool run <toolName> --input-json <json>`；`stop_message_auto` 必须把被拦截 stop 文本映射到 reasoning，但 CLI input/stdout 只能带 `flowId/repeatCount/maxRepeats` 状态。`continuationPrompt`、stop schema guidance、prompt preview、完整 input 只能作为内部 seed/request-side 注入材料，禁止进入 client-visible `exec_command` 或 CLI stdout。
- stopless CLI projection reasoning（2026-06-11 更新）：reasoning 真源顺序必须是 `execution.context.assistantStopText -> current finalChatResponse assistant text -> explicit default`；continuationPrompt/stop schema guidance 不得放 CLI `--input-json`，不得替换可见 reasoning。client-visible Responses replay history 只能投影 reasoning `summary`，不得生成非空 `reasoning.content` 数组；看到 upstream `input[n].content array too long` 时修 servertool projection owner，不修 direct/relay。测试 repeatCount/prompt 层级前必须清理对应 session/tmux state，避免旧 `stopMessageUsed` 残留伪造后续轮次。
...
- stopless goal-state 单测隔离（2026-06-09）：`syncStoplessGoalStateFromRequest` / routing-state store 测试必须显式把 `ROUTECODEX_SESSION_DIR` 指到临时目录；只改 `RCC_HOME`/`ROUTECODEX_HOME` 在 Jest/native 组合下可能仍读真实 `~/.rcc/state/routing/session-*.json`，导致 Rust `RCC_STOPLESS_GOAL_INVALID_TRANSITION` 假红。
...
- stop-message decision context signals（2026-06-09）：submit_tool_outputs resume、port stopMessage disabled、captured request Plan-mode detection 属于 `servertool-core::stopless_decision_context_signals::plan_stopless_decision_context_signals`；TS `stop-message-auto.ts` / `runtime-utils.ts` 只能传 `adapterContext` / `runtimeMetadata` / `capturedRequest` 给 native 并消费三个 boolean，禁止恢复本地 `toolOutputsDetailed.length`、`routecodexPortStopMessageEnabled` 或 `collaboration mode: plan` 扫描。
- stop-message decision context goal（2026-06-09）：request-scoped/persisted stopless goal 状态归一和 `goal_status` 计算属于 `servertool-core::stopless_decision_context_goal::plan_stopless_decision_context_goal_status`；TS `stop-message-auto.ts` 只能传 `adapterContext` 与 persisted goal state 给 native，禁止恢复 `isDirectStoplessGoalStateSnapshot`、`readRequestScopedGoalState`、`stoplessGoalStateSource` 或 `hasExplicitGoalState` 本地判定。
- stop-message default config（2026-06-09）：default stop-message `enabled/text/maxRepeats` 最终决策属于 `servertool-core::stop_message_default_config::plan_stop_message_default_config`；TS 只能读取 config/env 原始值并传 native，禁止恢复 `resolveStopMessageDefault*Live`、固定 `STOP_MESSAGE_EXECUTION_APPEND`、本地 trim/Number/Math.floor/default repeat 策略。
- stop-message persist plan（2026-06-09）：schema gate 后的 compare budget、next max/used、persist snapshot text/source/stage/default 归一属于 `servertool-core::stop_message_persist_plan::plan_stop_message_persist_snapshot`；TS `stop-message-auto.ts` 只能调用 native plan 并执行 metadata/state IO，禁止恢复 `gateMaxRepeats`、`resolvedMaxRepeats`、`schemaBudgetMaxRepeats`、`nextMaxRepeats`、`nextUsed` 或本地 `stateUpdate` 数字/字符串默认策略。
...
- servertool backend-route runtime/materialization（2026-06-09）：followup `entryEndpoint` 优先级、`payload` / `injection` source 识别、runtime action 与 runtime metadata planning 属于 `servertool-core::backend_route_contract::{plan_followup_materialization,plan_followup_runtime_action,plan_followup_runtime_metadata}`；TS `backend-route-runtime-block.ts` 只能调用 native plan、执行 injection IO builder、按 Rust plan 写 metadata/抛 Rust-described error，禁止恢复 `resolveFollowupEntryEndpoint`、`resolveFollowupPayloadFromPlan`、`resolveFollowupPayloadSource`、本地 `followupPlan.payload/injection/entryEndpoint` 扫描或 `/v1/chat/completions` 默认。
- servertool backend-route reenter/bootstrap（2026-06-09）：followup error envelope / terminal classification 与 bootstrap preflight/replay policy 属于 `servertool-core::backend_route_contract::{plan_followup_error_envelope,plan_bootstrap_replay}`；TS `backend-route-reenter-block.ts` / `backend-route-bootstrap-replay-block.ts` 只能调用 native plan、执行 reenter/file IO、构造 existing error projection，禁止恢复 `extractFollowupErrorEnvelope`、`isTerminalFollowupError`、`readPreflightStatus`、`buildReplayPayload`、本地 4xx/400/429/code/message substring 判定或 captured seed payload 组装。
- servertool skeleton-config（2026-06-09）：derived skeleton config、registration spec normalize、tool name alias、auto-hook phase、priority integer parse、tool spec lookup、auto hook queue、pending injection kinds、progress config、followup profiles、state config 属于 `router-hotpath-napi/src/servertool_skeleton_config.rs::{plan_servertool_skeleton_derived_config_json,normalize_servertool_registration_spec_json,resolve_servertool_tool_spec_json}`；TS `skeleton-config.ts` 只能调用 native wrappers 并返回 typed projection，禁止恢复 `normalizeServerToolName`、`normalizeAutoHookPhase`、`normalizeInteger`、`Number.isFinite` / `Math.floor`、`websearch/web-search` alias 分支、trigger->executionMode policy、`Object.fromEntries` config builders 或 followup profile 本地 normalize。
- servertool flow-presentation / fixture handler 删除（2026-06-09）：progress log flow id normalize、tool-name lookup、gold highlight policy 属于 `router-hotpath-napi/src/servertool_skeleton_config.rs::{resolve_servertool_progress_tool_name_json,should_use_servertool_gold_progress_highlight_json}`；TS `flow-presentation-block.ts` 只能调用 native wrappers，禁止恢复 `normalizeFlowId`、`buildServertoolProgressConfig`、`toolNameByFlowId`、`goldHighlightFlowIds` 或 Set highlight。`servertool_fixture` CLI projection dispatch 属于 Rust `chat_servertool_orchestration.rs`，旧 `src/servertool/handlers/fixture.ts` 空 handler 已删，禁止通过 TS registry handler 恢复。
- servertool pending-session / pending-injection（2026-06-09）：pending file-name sanitize、pending max-age parse/default、pending save payload coerce、load stale/malformed/drop decision、pending injection session dedupe、persist record planning、persistence error envelope 属于 `servertool-core::pending_session_contract::{resolve_pending_file_name,resolve_pending_max_age_ms,plan_pending_session_save,plan_pending_session_load,plan_pending_injection_persist,plan_pending_injection_persist_error}`；TS `pending-session.ts` 只能读 env/session dir、执行 fs/log IO 并调用 native wrappers，TS `pending-injection-block.ts` 只能调用 native persist/error planners 后执行 save IO，禁止恢复 `DEFAULT_PENDING_MAX_AGE_MS`、local `sanitizeSegment`、`coercePending`、`Number.parseInt` / `Number.isFinite` / `Math.floor`、regex segment sanitizer、stale message assembly、malformed drop decision、`aliasSessionIds` 去重、pending payload 组装或 `SERVERTOOL_PENDING_INJECTION_FAILED` envelope 组装。若 Jest 报 pending-session native unavailable，先 rebuild native hotpath，禁止加 TS fallback。
- servertool pre-command hooks（2026-06-09）：pre-command hook config/rule normalization、hook id sanitize、tool-set default/dedupe/normalize、regex source/flags planning、timeout clamp/default、priority parse、runtime pre-command rule planning 属于 `servertool-core::pre_command_hook_contract::{plan_pre_command_hooks_config,plan_runtime_pre_command_rule}`；TS `pre-command-hooks.ts` 只能读 config/env/cache、materialize Rust regex plan、parse current tool args、执行 jq/shell/runtime script IO，禁止恢复 TS normalize/sanitize/timeout/priority/tool-set policy。
- servertool stopless CLI projection seed（2026-06-11 更新）：stopless 闭环优先收口小骨架：`stop_message_auto` internal seed 的 continuationPrompt、reasoningText、repeatCount/maxRepeats、assistant stop text cleanup、finalChatResponse 文本 fallback、followup injection append_user_text 提取属于 `servertool-core::cli_contract::plan_stop_message_cli_projection_seed`；TS `engine.ts` 只能传 `execution/finalChatResponse` 给 `planStopMessageCliProjectionSeedWithNative` 并消费 Rust seed。client-visible `build_client_exec_cli_projection_output` 必须剥离 continuationPrompt，只投状态；禁止恢复 `readStopMessageFollowupText`、assistant/chat 文本扫描、loop number parse、stop-schema strip 或固定中文默认提示到 TS。
- servertool stopless terminal visible payload（2026-06-09）：`stop_message_auto` 当前 assistant stop text 提取、terminal stop 可见 payload 的 stop schema strip、summary prefix/replace、reasoning 字段剥离属于 `servertool-core::stop_visible_text::{extract_current_assistant_stop_text,build_stop_message_terminal_visible_payload}`；TS `handlers/stop-message-auto.ts` 只能调用 `extractCurrentAssistantStopTextWithNative` / `buildStopMessageTerminalVisiblePayloadWithNative`，禁止恢复 `choices/output` 遍历、`collectTextBlocks`、`prefixChatChoiceContent`、`replaceResponsesOutputContent`、`stripVisibleReasoningFields` 或独立 `stripStopSchemaControlPayload` native export。
- servertool stopless orchestration action（2026-06-09）：`stop_message_flow` 的 CLI projection / terminal final / generic followup 分支选择属于 `servertool-core::stopless_orchestration_contract::plan_stopless_orchestration_action`；TS `engine.ts` 只能调用 `planStoplessOrchestrationActionWithNative` 并执行 Rust action，禁止恢复 `flowId === 'stop_message_flow'`、`stopMessageTerminalFinal` 本地判断或 stopless reenter 分支。
- servertool CLI projection output（2026-06-11 更新）：client-visible `exec_command` projection output planning belongs to `servertool-core::cli_contract::plan_client_exec_cli_projection_output`; TS `cli-projection.ts` may only pass Rust seed/status into `buildClientExecCliProjectionOutputWithNative` and consume native output. Stopless `flowId=stop_message_flow -> toolName=stop_message_auto`, repeatCount/maxRepeats extraction/validation, and canonical status-only CLI input are Rust-owned; forbid restoring TS `args.flowId === 'stop_message_flow'`, local `toolName`, local repeat/max parsing, local CLI payload merge, or stdoutPreview/continuationPrompt injection into CLI.
- servertool loop-warning / orchestration policy（2026-06-09）：stop-message loop warning text/count injection 属于 `followup-core::inject_loop_warning`，`stop-message-loop-payload-block.ts` 只能 build native seed payload，禁止恢复 `appendStopMessageLoopWarning` TS export、本地 `warningText`、`messages.push`、repeat clamp；servertool timeout parse、timeout watcher、client disconnect 检测/轮询计划、servertool error payload、clientInjectOnly parse、client inject sanitize、followup error compact、adapter providerKey 解析属于 `servertool-core::orchestration_policy_contract::{parse_servertool_timeout_ms,plan_servertool_timeout_watcher,is_adapter_client_disconnected,plan_client_disconnect_watcher,plan_servertool_client_disconnected_error,plan_servertool_timeout_error,plan_stop_message_fetch_failed_error,read_client_inject_only,normalize_client_inject_text,compact_followup_error_reason,resolve_adapter_context_provider_key}`，TS `orchestration-policy-block.ts` 只能读 env/参数并调用 native wrapper；TS `timeout-error-block.ts` 只能执行 timer/Error 对象 glue，禁止恢复本地 `parseTimeoutMs`、`parseBooleanLike`、disconnect watcher policy、timeout/error payload builder、`sanitizeFollowupText`、HTTP/HTML regex compaction、providerKey walker 或 stop-gateway wrapper。
...
- OpenAI Responses prebuilt SSE stopless 边界：TS 只能把 upstream `__sse_responses` 读成 provider `bodyText` 并在 stopless 条件下禁止 passthrough；SSE terminal events 必须在 Rust `RespInbound` materialize 成 Responses JSON 后进入 `RespChatProcess`。默认非 goal stopless 也必须进入该 gate，禁止用 goal state 作为唯一激活条件。禁止把 `__routecodex_stream_contract_probe_body` / snapshot/debug probe 当正常 provider response 语义输入。live 验证 route-forced stopless 用 header `x-route-hint`，body `metadata.routeHint` 会被 metadata guard 正确拦截；same-protocol direct passthrough 不是 stopless 证据。
- stopless router-direct 入口规则（2026-06-09）：若 live stopless 只执行一轮且 usage 显示 `router-direct:*`，先查 direct entry decision。`stop_message_auto` CLI result、`serverToolFollowupSource/followupSource` 含 stop_message、或 stopless goal active 的 `/v1/responses` 必须在 router-direct 发送前返回 `stopless_servertool_requires_hub_relay` 并走 Hub；普通 `serverToolFollowup=true` 或非 stop_message followup 仍保持 direct/provider-direct 边界。
...
- Phase 0 servertool utility bridge cleanup：若 servertool bundle/profile/web_search/apply_patch helper 只剩 TS wrapper / required-export / standalone NAPI surface，必须删 public bridge；Rust 内部调用应改为 `pub(crate)` helper 直连，禁止 Rust 内部自调 JSON/NAPI 壳或用 parse/default compensation 当 fallback。apply_patch freeform contract 的 public owner 是 `normalizeApplyPatchArgumentsJson` / `validateApplyPatchArgumentsJson`，禁止恢复 `runApplyPatchJson` servertool executor。
- Phase 0 servertool continuation bridge cleanup：若 continue_execution / stop-message-state active / canonical chat completion payload helper 只剩 TS wrapper / required-export / standalone NAPI surface，必须删 public bridge；保留 Rust 主路径内部 helper（如 response-stage shape check、req-process servertool bundle plan）即可，禁止恢复 standalone continuation/directive injection NAPI module。
- Phase 0 req_inbound standalone bridge cleanup：若 req_inbound helper 只剩 TS wrapper / required-export / standalone NAPI surface（如 resume mapping、clientInjectReady、context label、shouldRun、shell-like token、followup snapshot、context augment、tool_call_id style），必须删 public bridge；Rust 内部调用必须用 crate helper 直连，禁止恢复 `resolve_client_inject_ready_json` 自调或 stale `coverage-hub-req-inbound-semantic-lift.mjs` stage-dist 覆盖脚本。
- Phase 0 req_inbound reasoning / semantic-lift bridge cleanup（2026-06-10）：`normalizeReqInboundReasoningPayloadWithNative` / `normalizeReasoningPayloadV2WithNative` / `shouldNormalizeReasoningPayloadWithNative` / `applyReqInboundSemanticLiftWithNative` 与匹配 NAPI 表面 `normalize_req_inbound_reasoning_payload_json` / `should_normalize_reasoning_payload_json` / `normalize_reasoning_payload_v2_json` / `apply_req_inbound_semantic_lift_json` 是 aggregate 旧 req_inbound reasoning 的外露；active req_inbound reasoning 唯一真源是 `hub_bridge_actions/reasoning.rs` 的 lower-level `normalize_reasoning_in_{chat,responses,gemini,anthropic}_payload` / `normalize_resp_inbound_reasoning_payload_json`，semantic-lift 唯一真源是 `hub_req_inbound_semantic_lift::apply_req_inbound_semantic_lift`，由 `hub_pipeline_lib::engine` crate-internal 调用。禁止恢复 `apply_req_inbound_semantic_lift_json` 这类公开 NAPI 外壳，禁止再通过 aggregate wrapper 把 4 个协议 normalize 合成一个 `normalizeReasoningPayloadV2`。
...
Provider-owned continuation/tool-result 请求的 ErrorErr05 决策必须保持 provider ownership：provider.send 失败时禁止 `exclude_and_reroute` 跨 provider 重放工具结果；只能同 provider retry（若策略允许）或 fail-fast 暴露原错误，禁止通过 payload 裁剪/转文本/重建上下文补偿。

Responses SSE client-close 挂起排障（2026-06-10）：看到 `response.sse.client_close` 且 `streamEnded=false`、`sawTerminalEvent=false`、`closeBeforeStreamEnd=true`，同时 Responses store 计数不降时，优先查 `handler-response-utils.ts` 的 client-close cleanup 是否被 `clientWriteQueue` / native projection 卡住。非 continuation `/v1/responses` 断连必须立即清 abandoned store；只有 required_action/tool-call continuation 可等待投影队列后持久化 continuation。
...
## Hub Pipeline Rust closeout 分片原则（新增）

- HubPipeline Rust 化只允许按 owner-map 分片推进；若 diff 同时卷入 servertool/VR/http-server/direct-passthrough，先恢复无关 owner 文件，再重切 slice。
- 复用规则：先看 `feature_id`/`owner`/`required tests`/`required gates`，再决定唯一修改点；禁止先改一圈再靠门禁筛。
- 触发信号：本次改动若超过 hub core 真源且扩散到旁支 owner，说明 slice 过激，必须收缩。
- 边界：单次 commit 只允许改动一个 owner-family；多 owner slice 必须拆 commit。
...
14. Provider 模型名双轨契约（2026-06-12）：`provider.models.<modelId>` 是唯一 upstream/provider wire model 名；`aliases` 只用于客户端展示和客户端模型输入匹配。排查 DeepSeek/DF/alias 问题时，必须同时验证两层：`/v1/models` 暴露 alias 或 `modelId`；`provider-request.json` 与 live provider body 中的 `model` 必须回写为 `modelId`，不能把 alias 直接发给上游。
...
- Responses direct continuation owner + payload 边界（2026-06-12）：`previous_response_id/responseId` 若来自 remote/upstream 响应，就必须记 `continuationOwner=direct` 并沿 direct 续接；若是本地 relay/materialize 生成的 responseId，则必须记 `continuationOwner=relay` 并只走 relay。`store:false` 不能阻止同一 response 的 tool continuation 持久化；response 真相里只要还有 pending tool call，就必须把 `allowContinuation=true`。同时 `providerKey` 只能留在 resume/meta carrier 里做路由 pin，**绝不能写回 resumed/materialized provider payload**，否则 same-protocol direct `submit_tool_outputs` 会被 upstream 以 `Unsupported parameter: providerKey` 直接打回 400。
- Responses direct SSE keepalive gate（2026-06-12）：如果 5520/5555 direct `/v1/responses` live 样本在长工具/图像子流里 `finish_reason=unknown`，先查 `~/.rcc/logs/server-<port>.log` 是否出现 `direct passthrough SSE emitted non-Responses event "keepalive"`。`event: keepalive` 是 transport-only 帧，必须在 `handler-response-utils.ts` direct guard 内丢弃；禁止把它当业务事件放行给客户端，也禁止为了解这个样本放宽其它 provider-specific 非标准 SSE 事件。
- Responses direct image partial event（2026-06-12）：修完 `keepalive` 后若同类 direct `/v1/responses` 样本继续卡住，再查是否变成 `response.image_generation_call.partial_image`。这是标准 Responses 图像流事件，证据可直接看本机 `node_modules/openai/resources/responses/responses.d.ts`；应补到 direct allowlist，并继续保留对 `codex.rate_limits` 这类 provider-specific 事件的 fail-fast。
- Responses direct SSE standard-event closeout（2026-06-12）：不要按单个报错逐个补 allowlist。直接用本地 OpenAI SDK `node_modules/openai/resources/responses/responses.d.ts` 做差集，确保 direct passthrough 允许完整标准 `response.*` 事件集合；`keepalive` 这类 transport-only 帧仍应丢弃，`codex.rate_limits` 等 provider-specific 事件仍必须 fail-fast。回归至少包含一个“SDK 事件全集可透传”的请求级黑盒。
...
- direct 不做清理，也不进 relay：direct 只 passthrough 当前 body + hooks；任何“清洗后再发”“shape 不合就 relay”都属于违规双路径。若 direct 上游拒绝，显式暴露并修 direct/provider contract 或历史 owner。
- same-protocol direct 不能因 payload/history shape 改走 relay；`input[].type="reasoning"` 带 `content`、chat-style tool 变体、client-standard Responses history 都不能作为 direct->relay eligibility 条件。若上游在 direct 下返回 400，必须按 direct 错误链显式暴露，并回到客户端历史 owner / provider contract 修复，禁止在 router/direct eligibility 里补一版 Hub 清洗绕路。
...
- direct provider error 的允许路径（2026-06-08 修正）：记录 `router-direct.send.error` → 进入统一 ErrorErr05 plan (`resolveRequestExecutorProviderFailurePlan`) → 仅在 plan 返回 request-local transient retry 时继续 direct-only 执行。429/concurrency 本请求临时 `excludedProviderKeys` 当前 provider 后重新问同一个 VR route/routingPolicyGroup；其它 recoverable 同 provider retry 一次，同错重复后再临时排除。禁止 reenter 标准 executor/relay，禁止 `executePipeline()`，禁止 payload sanitizer/rebuilder，禁止改 health/cooldown/config/pool 持久状态；special_400/400/404 等确定性错误仍 fail-fast 暴露。
...
- `stop_message_auto` migrated path 必须同时锁 projection 和 CLI executor：projection 生成 `exec_command routecodex servertool run stop_message_auto --input-json <json>`；CLI executor 必须强制 `flowId=stop_message_flow`、`repeatCount`、`maxRepeats`，但禁止 `continuationPrompt` / schemaGuidance / prompt preview / 完整 input 出现在 client-visible command/stdout。真实 continuation prompt 由请求端按状态注入。
...
- stopless schema 闭环 Rust owner：schema gate 和 request-side prompt seed 属于 Rust；`schemaGuidance` 不得进入 `stop_message_auto` CLI stdout。
- projection schema Rust owner：`build_client_exec_cli_projection_output()` 构建 status-only execCommand + repeatCount + maxRepeats；旧 `--ticket` / `stcli_` / `rcc_cli_` 标记在 Rust unit test 中被显式拒绝。
...
## 2026-06-10 Responses SSE backpressure hang 精华
...
## 2026-06-11 Responses standard tool SSE 精华

- OpenAI Responses 流式工具调用客户端事件必须使用标准序列：`response.output_item.added -> response.function_call_arguments.done -> response.output_item.done -> response.completed -> response.done`；禁止把 JSON 合约里的 `required_action` 直接发成 `event: response.required_action` SSE。
- 响应侧 terminal repair 真源是 Rust `build_responses_terminal_sse_frames_from_probe_json`；它必须能从 `required_action` 或 pending `output[].function_call` 合成标准工具帧。TS handler 只做写流、超时、诊断日志和错误投影，不能发明第二套工具事件。
- live 验证时不要用 forbidden client metadata（如 `metadata.routeHint`）硬改路由；入口会 fail-fast。工具 SSE 验证必须直接断言客户端收到标准工具事件、`response.completed`、`response.done`，且无 `response.required_action`、无 `event:error`、无挂起。
- 若 raw SSE 事件名已经标准但客户端仍不执行工具，继续检查 terminal `response.completed/response.done` 的 data payload：不得含 `required_action` / `status:"requires_action"`，terminal `output[].function_call.status` 必须是 `completed`。修点是 Rust `project_responses_sse_frame_for_client`，不是 TS writer 或 provider。

## 2026-06-11 LMStudio stringify residue 精华

- `chat:lmstudio` 兼容只应保留 tool id / shape 归一，不应再注入 `lmstudio_responses_input_stringify` 这类把 Responses 结构化 `input` 打平的旧动作；一旦发现 action/profile/native capability/rt 开关同时存在，必须物理删除四处，不要只删其一。
- 验证信号：`openai-responses` 的 `input` 保持数组/对象结构不变，`cargo test -p router-hotpath-napi lmstudio_profile_preserves_openai_responses_structured_tool_input -- --nocapture` 必须为绿；若 profile 仍引用旧 action，residue audit 应直接红。

## 2026-06-11 Stopless provider-continuity 精华

- Stopless `stopMessageUsed` 只允许在同一 session/tmux scope 且同一 routed provider 上连续；provider key 变化必须 reset budget 后继续第一轮。修点属于 Rust `stop-message-core` decision + `servertool-core` persisted snapshot/providerKey carrier，TS 只传当前 provider 与执行 state IO。
- Gate 必须成对：provider mismatch resets, provider match preserves；同时锁 `stopMessageProviderKey` 在 routing-state serialize/deserialize/merge 中不丢失，避免跨 provider 继承旧 budget。
