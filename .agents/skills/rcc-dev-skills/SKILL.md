...
- servertool pending-session（2026-06-09）：pending file-name sanitize、pending max-age parse/default、pending save payload coerce、load stale/malformed/drop decision 属于 `servertool-core::pending_session_contract::{resolve_pending_file_name,resolve_pending_max_age_ms,plan_pending_session_save,plan_pending_session_load}`；TS `pending-session.ts` 只能读 env/session dir、执行 fs/log IO 并调用 native wrappers，禁止恢复 `DEFAULT_PENDING_MAX_AGE_MS`、local `sanitizeSegment`、`coercePending`、`Number.parseInt` / `Number.isFinite` / `Math.floor`、regex segment sanitizer、stale message assembly 或 malformed drop decision。若 Jest 报 pending-session native unavailable，先 rebuild native hotpath，禁止加 TS fallback。
