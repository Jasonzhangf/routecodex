...
English contract tokens (gate-locked): same-turn schema guidance; activation marker is forbidden as truth; No activation / inactive state means pass-through.

stopless 拦截的先决条件不是“看见 stop/end_turn”本身，而是**StoplessCenter MetadataCenter 状态机处于同轮 `schema_guidance_active=true`**：Req04 合法注入 schema guidance 后，把 scoped StoplessCenter 状态迁移到本轮 active state；Resp03 只能读取同一 scoped turn 的 StoplessCenter 状态机，不能从 provider/client payload、SSE、handler、continuation store、debug snapshot、CLI stdout、visible text 或临时 marker 推断。

判定矩阵：

| StoplessCenter same-turn `schema_guidance_active` | stop/end_turn | accepted summary | accepted stop_schema | Resp03 行为 |
| --- | --- | --- | --- | --- |
...
## MetadataCenter StoplessCenter 状态机机制
...
- StoplessCenter 是本 SOP 的唯一控制真源；它只存在于 MetadataCenter/runtime_control，不存在于 CLI、payload、continuation store、SSE、handler、debug/snapshot、provider body 或 client body。
- 这个状态机不是“看见 stop 就拦一下”的规则集合，而是一个按同一 scoped turn 读写的控制状态机：Req04 写入，Resp03 读取，Req04 下一轮再消费。
- 状态机的 activation truth 不是 loose marker；唯一真源是 `schema_guidance_active` 这个状态字段及其同轮 scoped state。Resp03 只能按这个状态判断是否允许续杯拦截。
- 只有相邻迁移合法：`Idle -> ProviderTurnInFlight -> RespStopObserved -> CliNoopProjected -> CliNoopObserved -> ContinuationGuidancePrepared -> ProviderTurnInFlight`，以及 `TerminalCompleted/TerminalBlocked/GuardTerminal` 的显式终态或 `Idle(reset)`。
- `Req04` 负责把合法 schema guidance 转成 active state；`Resp03` 负责把 stop/end_turn + summary/schema evidence 转成终态、续杯、或 guard。
- `Resp03` 只会在 active state 下对 stop/end_turn 做拦截判定；inactive state 必须原样放行，不得静默补偿，不得从 visible text、fenced JSON、CLI stdout、continuation store 反推状态。
- `guard_exhausted` 不是任务完成，只是停止继续拦截当前 stop；`GuardTerminal` 必须显式放行，不得合成内部状态诊断。
- 这套状态机机制必须在本文、manifest、function map、verification map 与 generated review surface 同步出现；缺一处就是 contract drift。
...
- Resp03 只在 StoplessCenter 同轮 `schema_guidance_active` state 存在时拦截提前 stop 或 provider/model-visible `reasoningStop` event；inactive state 必须放行。
- Resp03 先限定 stop/end_turn，再检查 canonical reasoning summary 与同一 reasoning item 的 sibling `stop_schema` / `stopSchema`；没有 summary 且没有 schema 时，只有 StoplessCenter active state 存在才当提前 stop 续轮。visible/fenced schema 仍不是状态源。
...
7. Direct/relay activation conditional：Responses 协议 direct 或 relay 的 response-side stopless 只能在同轮 StoplessCenter `schema_guidance_active` state 存在时激发；inactive state 的 no-summary terminal stop 必须放行；有 summary 或 accepted schema 必须按合同放行/判定。provider-direct/非 Responses direct 不启用 relay StoplessCenter。
...
11. Summary/stop_schema activation：只有同轮 StoplessCenter `schema_guidance_active` 时，无 summary 且无 schema 的 stop/end_turn 才必须续轮；inactive state 必须放行。summary + `stop_schema.finished=true` 必须通过且不计数；summary + unfinished `nextStep` 必须续轮并让 Req04 prompt 带 nextStep；summary + blocked reason 必须把 reason 写入 summary 并通过；visible text/fenced schema 仍必须红锁为非状态源。
...
- 新 summary/schema completion gate 只在同轮 schema guidance active 时拦截自然 stop；有 accepted summary 放行，有 sibling `stop_schema` 时按 schema 判定 finished / blocked / unfinished nextStep；没有 summary/schema 的自然 stop 只有在 StoplessCenter active state 存在时继续续轮。
