...
Fix Lifecycle v2 began with the v0.1.3 single-worker graph. AppSDK v0.1.4 adds the atomic `multi_worker_collaboration` + `multi_worktree_merge_queue` scenario pair and AppSDK v0.1.5 adds pre-review whitebox/deployed-blackbox validation and evidence-backed install/restart receipts. Supported legacy projects may use ordinary `verify`/`compile` while adopting the current CLI; binary, compiler, and bundle identity belong only to explicit migration or SDK-release metadata. Release admission still requires genuine lifecycle evidence. Do not synthesize PASS records when historical evidence is unavailable.
