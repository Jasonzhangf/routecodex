...
fn request_control(
    ...
    // During a managed handoff macOS may transiently report EAGAIN while the
    // child replaces its listener. Treat only that bounded window as retryable;
    // all other socket failures remain fail-closed.
