...
pub fn read_record(paths: &V4LifecyclePaths) -> Result<Option<ManagedInstanceRecord>, LifecycleError> {
    if !paths.record_path.exists() {
        return Ok(None);
    }
    let bytes = fs::read(&paths.record_path)
...
fn request_control(paths: &V4LifecyclePaths, command: &str) -> Result<serde_json::Value, LifecycleError> {
...
fn write_record_atomic(
    ...
    let temporary = paths.state_root.join(format!(".instance.{}.tmp", std::process::id()));
    let mut bytes = serde_json::to_vec_pretty(record)
        .map_err(|error| LifecycleError::Record(error.to_string()))?;
    bytes.push(b'\n');
    fs::write(&temporary, bytes).map_err(|error| io_error(&temporary, error))?;
    fs::rename(&temporary, &paths.record_path)
        .map_err(|error| io_error(&paths.record_path, error))
}
