//! V4 CLI plugin — admission baseline dispatcher.
//!
//! This is a thin read-only CLI that exposes the standard plugin library
//! descriptor/registry/resource surface. It never starts a server, never
//! binds a port, never reads or writes `~/.rcc/`, never manages processes,
//! PID files, sockets, or locks. Runtime admission (`start` / `restart` /
//! `stop` / lifecycle) is owned by other crates; this binary exists so the
//! standard plugin library has a deterministic CLI surface in the V4
//! admission baseline.
...
enum Command {
    ...
    DescribePlugin {
        /// The plugin id to describe (e.g. `v4.std.response.protocol_decode`).
        plugin_id: String,
    },
