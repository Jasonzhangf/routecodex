...
use routecodex_v4_runtime::{
    assert_no_control_leak, execution_binding, project_runtime_fault, ExecutionBinding,
    ExecutionContext, NodePluginPlan, SkeletonRuntime,
};
...
fn positive_request_chain_produces_wire_and_stable_binding() {
    ...
    assert!(scope_value.starts_with("scope:chat:none:session-r-request-1"));
}
