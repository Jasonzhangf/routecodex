...
fn start(intent: StartIntent) -> Result<String, String> {
    ...
    // Top-level `rccv4 start` is the operator observation entrypoint.  Keep
    // the current shell attached by default, matching V3; managed lifecycle
    // ownership remains explicit on `rccv4 restart`/`rccv4 server start`.
    let manifest = compile_runtime_config_file(&config).map_err(|error| error.to_string())?;
