...
use routecodex_v4_router::ProviderCandidate;
use routecodex_v4_runtime::{
    parse_responses_provider_payload, project_runtime_fault, RequestPluginRuntime,
...
fn main() {
    ...
    let request_runtime = match RequestPluginRuntime::new(
        serde_json::to_value(&manifest).expect("compiled manifest is serializable"),
    ) {
        Ok(runtime) => runtime,
        Err(error) => {
            eprintln!("rccv4 request plugin runtime failed: {error}");
            std::process::exit(78);
        }
    };
...
struct AdmissionHandler<'a> {
    ...
    request_runtime: RequestPluginRuntime,
}
    ...
    fn handle(&mut self, request: HttpRequest) -> HttpResponse {
        match (request.method.as_str(), request.path.as_str()) {
            ...
            ("GET", "/v1/models") => json_response(
                200,
                serde_json::json!({
                    "object": "list", "data": self.manifest.candidates.iter().flat_map(|candidate| {
                        candidate.entry_models.iter().map(|entry_model| serde_json::json!({
                            "id": entry_model, "object": "model", "owned_by": candidate.provider_id
                        }))
                    }).collect::<Vec<_>>()
                }),
            ),
            ("POST", "/v1/responses") => match handle_responses(&self.request_runtime, &request) {
...
fn handle_responses(
    runtime: &RequestPluginRuntime,
    request: &HttpRequest,
) -> Result<HttpResponse, HttpResponse> {
    let execution = runtime
        .execute_responses(&request.body)
        .map_err(|fault| project_fault(request, fault, 400))?;
    let target = execution.target;
    let wire_body = execution.provider_wire;
    let stream_mode = execution.stream;
    if stream_mode {
        ...
        .map_err(|fault| project_fault(request, fault, 502))?
    {
        ...
        ResponsesProviderPayload::Sse(bytes) => {
            Ok(HttpResponse::new(raw.status, "text/event-stream", bytes))
        }
    }
}
