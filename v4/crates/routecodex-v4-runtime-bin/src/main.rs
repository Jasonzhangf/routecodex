...
fn restart(intent: RestartIntent) -> Result<String, String> {
    ...
    // Each CLI invocation is a fresh process. Re-establish the project-owned
    // Cordis host before admission instead of relying on environment inherited
    // from the original `start` shell.
    let cordis_child = ensure_cordis_host_socket(&paths.manifest_path, &paths)?;
...
fn ensure_cordis_host_socket(
    ...
    if let Some(existing) = std::env::var_os("RCCV4_CORDIS_HOST_SOCKET") {
        ...
        // A managed restart inherits the previous child's environment. If
        // that child owned the Cordis host, its shutdown also invalidated the
        // inherited socket; rebuild only that project-owned host. Never
        // replace an unreachable external socket.
