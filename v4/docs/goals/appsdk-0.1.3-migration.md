...
The record contracts are declared inputs of every frozen V4 library. Their change is therefore a contract change, not an in-place lock edit. Open and publish new versions while preserving the old immutable versions:
...
5. Merge the exact candidate tree to `main`; only then create the merge, promotion, regression, and freeze records.
6. Compile, freeze, publish the four new Active versions, run global-binary verify/admission, build/install/restart RouteCodex, and replay an online sample before DSH Review.
