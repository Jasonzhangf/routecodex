...
4. 构建门禁统一：`cargo test --workspace --manifest-path v4/Cargo.toml` 覆盖全部
   ...
   verification-map 的 `v4_cargo_workspace_build` 与 CI `test` job（V4 canonical `verify:ci`）不遗漏任何模块。
...
- CI：`test` job 安装 v4 依赖并调用 V4 canonical `verify:ci`，覆盖新 crate
...
6. 构建门禁统一：`v4_cargo_workspace_build` 覆盖全部模块，CI `test` job 经
...
3. **门禁接线**：verification-map 注册 `v4_debug_l2_regression` /
   ...
   resource-binding 红自测；CI `test` job 经 V4 canonical `verify:ci` 覆盖
   4 个 test-consumer
