...
状态：`phase4_real_runtime_admission_active`（M0-M7 与真实 Cordis Host -> Rust NodeContainer 生命周期绑定已落地；M8 独立 `rccv4` 从编译 manifest 冷启动，并通过真实 provider transport 提供 Responses JSON/SSE）
