 ...
 * Pipeline Module Registry Implementation
 */
export class PipelineModuleRegistryImpl {
    constructor() {
        ...
        this.enhancementFactory = null;
        this.registryMetrics = new Map();
     ...
     */
    async createModule(config, dependencies) {
    ...
    async cleanup() {
        ...
        const cleanupPromises = Array.from(this.instances.values()).map(instance => this.cleanupInstance(instance));
     ...
     */
    validateModuleInstance(instance, expectedType) {
    ...
    initializeDebugEnhancements() {
        try {
            ...
            this.enhancementFactory = new ModuleEnhancementFactory(debugCenter);
    ...
    subscribeToRegistryEvents() {
        if (!this.debugEventBus)
            return;
        this.debugEventBus.subscribe('registry-subscription', (event) => {
     ...
     */
    handleRegistryDebugEvent(event) {
     ...
     */
    recordRegistryMetric(operationId, value) {
     ...
     */
    publishRegistryEvent(type, data) {
        if (!this.isEnhanced)
            return;
