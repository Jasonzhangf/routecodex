 ...
 * Qwen Compatibility Module
 */
export class QwenCompatibility {
    constructor(config, dependencies) {
        this.dependencies = dependencies;
        this.type = 'qwen-compatibility';
     ...
     */
    recordCompatibilityMetric(operation, data) {
     ...
     */
    addToTransformationHistory(transformation) {
        ...
        try {
            ...
            if (this.isDebugEnhanced) {
                this.recordCompatibilityMetric('response_transform_complete', {
                    ...
                    hasToolCalls: transformed.choices?.some((c) => c.message?.tool_calls?.length > 0)
                });
                this.addToTransformationHistory({
                    responseId,
                    request: response,
    ...
    initializeTransformationRules() {
        const compatibilityConfig = this.config.config;
     ...
     */
    transformQwenResponseToOpenAI(response) {
     ...
     */
    transformProviderResponse(data) {
        const transformed = {
        ...
        if (data.choices && Array.isArray(data.choices)) {
            transformed.choices = data.choices.map((choice) => ({
     ...
     */
    transformFinishReason(finishReason) {
        const reasonMap = {
            'stop': 'stop',
            'length': 'length',
    ...
    convertToQwenRequest(request) {
        ...
        if (Array.isArray(request.messages)) {
            ...
            qwenRequest.input = request.messages.map((message) => {
