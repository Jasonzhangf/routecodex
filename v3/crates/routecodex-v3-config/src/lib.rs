...
mod tests {
    ...
    #[test]
    fn secret_file_auth_discovery_is_provider_scoped_and_fail_fast() {
        let content = "opencode-go.key1 = one\nopencode-go.key3 = three\nother.key1 = hidden\n";
        assert_eq!(
            discover_v3_secret_file_auth_handles(content, "opencode-go").unwrap(),
            vec![
                ("key1".to_string(), "opencode-go.key1".to_string()),
                ("key3".to_string(), "opencode-go.key3".to_string()),
            ]
        );
        assert!(discover_v3_secret_file_auth_handles("malformed", "opencode-go").is_err());
        assert!(discover_v3_secret_file_auth_handles(
            "opencode-go.key1 = one\nopencode-go.key1 = duplicate\n",
            "opencode-go",
        )
        .is_err());
        assert!(discover_v3_secret_file_auth_handles(content, "missing").is_err());
    }
