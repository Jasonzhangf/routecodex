...
use routecodex_v3_hooks::{
    AnyAppServerTransport, AppServerTransport, ControlRequest, ControlResponse, ControlServer,
    HooksSidecarCore, MessageIntent, Namespace, NativeAppServerTransport, ScheduledMessage,
    SendMode, SessionTarget,
};
...
struct MockAppServerOptions {
    ...
    timeline_error: Option<Value>,
    items_error: Option<Value>,
    turns_error: Option<Value>,
    read_turns_from_accepted: bool,
    queue_start_already_dispatched: bool,
    history_unavailable_until_accepted: bool,
    queue_start_error: Option<Value>,
}
...
#[test]
fn native_transport_accepts_queue_start_auto_dispatch_race() {
    let socket_path = unique_socket("queue-start-race");
    let listener = UnixListener::bind(&socket_path).unwrap();
    let state = Arc::new(Mutex::new(MockAppServerState::default()));
    let stop = Arc::new(AtomicBool::new(false));
    let options = MockAppServerOptions {
        queue_start_already_dispatched: true,
    ...
    let intent = MessageIntent {
        intent_id: "queue-start-race-intent".to_string(),
    ...
    assert_eq!(
        accepted.message_id,
        "queued-queue-start-race-intent".to_string()
    );
...
#[test]
fn any_transport_preserves_native_send_baseline() {
    let socket_path = unique_socket("any-preserves-baseline");
    let listener = UnixListener::bind(&socket_path).unwrap();
    let state = Arc::new(Mutex::new(MockAppServerState::default()));
    let stop = Arc::new(AtomicBool::new(false));
    let server = spawn_mock_app_server_with_options(
    ...
    let mut transport =
        AnyAppServerTransport::Native(NativeAppServerTransport::new(socket_path.to_string_lossy()));
    let intent = MessageIntent {
        intent_id: "any-preserves-baseline-intent".to_string(),
        source: target("source"),
        target: target("target"),
        body: "probe".to_string(),
        send_mode: SendMode::WorkingAllowed,
    };
    let attempt = transport.send_message_with_baseline(&intent).unwrap();
    assert!(
        attempt.baseline.is_some(),
        "AnyAppServerTransport must not drop the native send baseline"
    );
...
#[test]
fn native_transport_keeps_acceptance_when_queue_start_errors_after_queue_add() {
    let socket_path = unique_socket("queue-start-error");
    let listener = UnixListener::bind(&socket_path).unwrap();
    let state = Arc::new(Mutex::new(MockAppServerState::default()));
    let stop = Arc::new(AtomicBool::new(false));
    let options = MockAppServerOptions {
        queue_start_error: Some(json!({
            "code": -32603,
            "message": "internal start failure"
        })),
        ..MockAppServerOptions::default()
    };
    let server = spawn_mock_app_server_with_options(
    ...
    let transport = NativeAppServerTransport::new(socket_path.to_string_lossy());
    let intent = MessageIntent {
        intent_id: "queue-start-error-intent".to_string(),
        source: target("source"),
        target: target("target"),
        body: "probe".to_string(),
        send_mode: SendMode::WorkingAllowed,
    };
    let mut core = HooksSidecarCore::new(transport);
    let outcome = core.forward(intent.clone()).unwrap();
    assert!(
        matches!(outcome, routecodex_v3_hooks::DispatchOutcome::Sent(_)),
        "queue/add acceptance must be preserved after queue/start error"
    );
    let record = core.intent_evidence(&intent.intent_id).unwrap();
    assert!(
        record
            .error
            .as_deref()
            .is_some_and(|error| error.contains("internal start failure")),
        "start_error must be preserved on the accepted intent: {record:?}"
    );
    assert!(
        record
            .evidence
            .first()
            .and_then(|evidence| evidence.start_error.as_deref())
            .is_some_and(|error| error.contains("internal start failure")),
        "accepted delivery evidence must retain the start_error: {record:?}"
    );
    assert_eq!(
        state.lock().unwrap().accepted,
        vec![("target".to_string(), "queue-start-error-intent".to_string())]
    );
    assert!(state.lock().unwrap().started.is_empty());
...
#[test]
fn native_transport_does_not_promote_unavailable_baseline_when_history_becomes_readable() {
    let socket_path = unique_socket("unavailable-baseline");
    let listener = UnixListener::bind(&socket_path).unwrap();
    let state = Arc::new(Mutex::new(MockAppServerState::default()));
    let stop = Arc::new(AtomicBool::new(false));
    let options = MockAppServerOptions {
        history_unavailable_until_accepted: true,
        queue_start_already_dispatched: false,
        ..MockAppServerOptions::default()
    };
    let server = spawn_mock_app_server_with_options(
        listener,
    ...
    let mut transport = NativeAppServerTransport::new(socket_path.to_string_lossy());
    let intent = MessageIntent {
        intent_id: "unavailable-baseline-intent".to_string(),
        source: target("source"),
        target: target("target"),
        body: "probe".to_string(),
        send_mode: SendMode::WorkingAllowed,
    ...
    let accepted = transport.send_message(&intent).unwrap();
    assert_eq!(accepted.state, routecodex_v3_hooks::DeliveryState::Accepted);
    let error = transport.delivery_evidence(&intent, &[]).unwrap_err();
    assert!(
        matches!(
            error,
            routecodex_v3_hooks::AppServerError::DeliveryUnresolved(_)
        ),
        "unavailable baseline must not be promoted after history becomes readable: {error:?}"
    );
    let error = transport
        .delivery_evidence(&intent, &["fabricated-baseline".to_string()])
        .unwrap_err();
    assert!(
        matches!(
            error,
        ...
        "caller baseline must not bypass cached unavailable baseline: {error:?}"
    );
...
#[test]
fn native_transport_keeps_unavailable_baseline_bound_against_caller_baseline() {
    let socket_path = unique_socket("unavailable-baseline-caller");
    let listener = UnixListener::bind(&socket_path).unwrap();
    let state = Arc::new(Mutex::new(MockAppServerState::default()));
    let stop = Arc::new(AtomicBool::new(false));
    let options = MockAppServerOptions {
    ...
    let intent = MessageIntent {
        intent_id: "unavailable-baseline-caller-intent".to_string(),
        source: target("source"),
        target: target("target"),
        body: "probe".to_string(),
        send_mode: SendMode::WorkingAllowed,
    ...
    assert!(
        matches!(
            error,
            routecodex_v3_hooks::SidecarDispatchError::AppServer(
                routecodex_v3_hooks::AppServerError::DeliveryUnresolved(_)
            )
        ),
        "caller baseline must not promote an unavailable persisted baseline: {error:?}"
    );
...
fn native_transport_resolves_receipt_and_reply_over_unix_websocket() {
    ...
    let accepted = transport.send_message(&intent).unwrap();
    assert_eq!(accepted.state, routecodex_v3_hooks::DeliveryState::Accepted);
    assert_eq!(accepted.message_id, "queued-intent-native-1");
    assert_eq!(
        state.lock().unwrap().started,
        vec![("target".to_string(), "queued-intent-native-1".to_string())]
    );
...
fn native_transport_allows_unmaterialized_first_send_with_empty_baseline() {
    ...
    let options = MockAppServerOptions {
        ...
        timeline_error: Some(json!({
            "code": -32601,
            "message": "thread/timeline/list is not supported yet"
        })),
        ...
        queue_start_already_dispatched: false,
        history_unavailable_until_accepted: false,
        queue_start_error: None,
    };
    let server = spawn_mock_app_server_with_options(
        listener,
        Arc::clone(&state),
            ...
            match listener.accept() {
                Ok((stream, _)) => {
                    ...
                    let options = MockAppServerOptions {
                        ...
                        timeline_error: options.timeline_error.clone(),
                        ...
                        queue_start_already_dispatched: options.queue_start_already_dispatched,
                        history_unavailable_until_accepted: options
                            .history_unavailable_until_accepted,
                        queue_start_error: options.queue_start_error.clone(),
                    };
...
fn serve_mock_app_server(
    ...
    loop {
        ...
        let response = match method {
            ...
            "thread/queue/start" => {
                let thread_id = request["params"]["threadId"].as_str().unwrap().to_string();
                let queued_submission_id = request["params"]["queuedSubmissionId"]
                    .as_str()
                    .unwrap()
                    .to_string();
                if let Some(error) = options.queue_start_error.clone() {
                    let mut response = json!({ "error": error });
                    response["id"] = json!(id);
                    write_server_frame(&mut stream, &response.to_string());
                    continue;
                }
                if options.queue_start_already_dispatched {
                ...
                state
                    ...
                    .unwrap()
                    .started
                    .push((thread_id, queued_submission_id));
                json!({
                    "result": {
                        "turn": {
                            "id": "turn-1",
            ...
            "thread/timeline/list" => {
                if let Some(error) = options.timeline_error.clone() {
                    json!({ "error": error })
                } else if options.history_unavailable_until_accepted
                    && state.lock().unwrap().accepted.is_empty()
                {
                    json!({
                        "error": {
                            "code": -32601,
                            "message": "thread/timeline/list is unavailable until accepted"
                        }
                    })
                } else {
                    let accepted = state.lock().unwrap().accepted.clone();
            ...
            "thread/items/list" => {
                    ...
                    && state.lock().unwrap().accepted.is_empty()
                {
                    json!({
                        "error": {
                            "code": -32601,
                            "message": "thread/items/list is unavailable until accepted"
                        }
                    })
                } else {
                    let accepted = state.lock().unwrap().accepted.clone();
            ...
            "thread/turns/list" => {
                    ...
                    && state.lock().unwrap().accepted.is_empty()
                {
                    json!({
                        "error": {
                            "code": -32601,
                            "message": "thread/turns/list is unavailable until accepted"
                        }
                    })
                } else {
