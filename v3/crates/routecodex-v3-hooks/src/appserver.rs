                    ...
                    Err(turns_error) => {
                        let reason = match read_turns_error {
                            Some(read_turns_error) => format!(
                                "thread history read is unsupported: read_turns={read_turns_error}; items={items_error}; turns={turns_error}"
                            ),
                            None => format!(
                                "thread history read is unsupported: items={items_error}; turns={turns_error}"
                            ),
                        };
                        if is_unmaterialized_history_read_error(&reason, &status) {
                            ...
                            Err(AppServerError::Transport(reason))
                        }
                    }
                }
            }
    ...
    pub fn delivery_evidence_from_native(
        ...
        let baseline_for_intent = if baseline.is_empty() {
            self.baseline_cache
                ...
                .map(NativeBaselineSnapshot::item_ids)
                .ok_or_else(|| {
...
impl AppServerTransport for NativeAppServerTransport {
    ...
    fn send_message_with_baseline(
        ...
        Ok(SendAttemptEvidence {
            delivery: DeliveryEvidenceRecord {
                ...
                message_id: intent.intent_id.clone(),
    ...
    fn delivery_evidence_with_baseline(
        ...
        let baseline_for_intent = match baseline {
            Some(snapshot) => snapshot.item_ids(),
            None => self
                .baseline_cache
                .get(&intent.intent_id)
                .map(NativeBaselineSnapshot::item_ids)
