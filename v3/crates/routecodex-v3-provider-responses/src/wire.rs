...
/// assistant 文本消息和 reasoning 块移动到该 run 最后一个 output 之后，保持相对顺序；
/// calls 与 outputs 原序不动。Console Go 的 Chat 降级不接受 reasoning 插在
/// function_call 与 function_call_output 之间，否则仍会触发 400。
