...
fn stream_failure_cools_provider_immediately_and_probe_failure_keeps_excluded() {
    ...
    store
        .complete_provider_cooldown_probe_failure(
            ...
            Some("key-a"),
            Some("model-a"),
            100 + 900_000 + 15 * 60_000 + 1,
        )
        .unwrap();
    assert!(
        store
            .provider_cooldown_probe_keys_due(100 + 900_000 + 2 * 15 * 60_000)
            .unwrap()
            .is_empty(),
    ...
    assert!(
        store
            .provider_cooldown_probe_keys_due(100 + 900_000 + 2 * 15 * 60_000 + 1)
            .unwrap()
            .contains(&("provider-a".to_string(), Some("key-a".to_string()), Some("model-a".to_string())))
    );
    assert!(
