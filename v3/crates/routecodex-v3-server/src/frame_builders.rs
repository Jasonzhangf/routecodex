...
pub(crate) fn v3_is_sse_target_pool_exhaustion_parts(
    ...
    let (code, message) = v3_error_body_code_message(body);
    // Error06 marks client projection and is also emitted for a provider
    // failure while another target remains. Only the typed Error04 exhaustion
    // witness proves that no provider target remains for an SSE disconnect.
    code == "network_error"
