...
auth = {{ type = "api_key", entries = [{{ alias = "key", env = "V3_OPENAI_CHAT_TEST_KEY" }}] }}
[providers.chat.models."glm-5.2"]
wire_name = "glm-5.2"
aliases = ["client-chat"]
...
[route_groups.default.pools.client_chat]
selection = {{ strategy = "priority" }}
match = {{ precedence = 10, entry_protocol = "responses", models = ["client-chat"] }}
targets = [{{ kind = "provider_model", provider = "chat", model = "glm-5.2", key = "key", priority = 1 }}]
[route_groups.default.pools.default]
selection = {{ strategy = "priority" }}
targets = [{{ kind = "provider_model", provider = "chat", model = "glm-5.2", key = "key", priority = 1 }}]
"#
...
async fn controlled_openai_chat_sse_upstream(
    State(state): State<Arc<ProviderState>>,
    headers: HeaderMap,
    axum::Json(body): axum::Json<Value>,
) -> Response<Body> {
    ...
    Response::builder()
        ...
        .body(Body::from(concat!(
            "data: {\"id\":\"chatcmpl_routecodex_chat_sse\",\"object\":\"chat.completion.chunk\",\"created\":1784865608,\"model\":\"glm-5.2\",\"choices\":[{\"delta\":{\"role\":\"assistant\",\"content\":\"ROUTECODEX_OPENAI_CHAT_SSE_OK\"},\"finish_reason\":null,\"index\":0}],\"usage\":null}\n\n",
            "data: {\"id\":\"chatcmpl_routecodex_chat_sse\",\"object\":\"chat.completion.chunk\",\"created\":1784865608,\"model\":\"glm-5.2\",\"choices\":[{\"delta\":{},\"finish_reason\":\"stop\",\"index\":0}],\"usage\":null}\n\n",
            "data: {\"id\":\"chatcmpl_routecodex_chat_sse\",\"object\":\"chat.completion.chunk\",\"created\":1784865608,\"model\":\"glm-5.2\",\"choices\":[],\"usage\":{\"prompt_tokens\":3,\"completion_tokens\":1,\"total_tokens\":4,\"input_tokens\":3,\"output_tokens\":1}}\n\n",
            "data: [DONE]\n\n"
        )))
        .unwrap()
}

async fn controlled_empty_responses_sse_upstream(
    State(state): State<Arc<ProviderState>>,
    headers: HeaderMap,
    axum::Json(body): axum::Json<Value>,
) -> Response<Body> {
...
async fn start_controlled_openai_chat_sse_upstream() -> (
    String,
    mpsc::UnboundedReceiver<ProviderCapture>,
    oneshot::Sender<()>,
) {
    ...
    let app = Router::new()
        .route(
            "/v1/chat/completions",
            post(controlled_openai_chat_sse_upstream),
        )
        .route(
            "/v1/responses",
            post(controlled_empty_responses_sse_upstream),
        )
        .with_state(Arc::new(ProviderState {
            captures: captures_tx,
        }));
...
#[tokio::test]
async fn responses_direct_binding_uses_relay_for_openai_chat_provider_sse() {
    let _test_guard = TEST_LOCK.lock().await;
    let (provider_base_url, mut captures, shutdown) =
        start_controlled_openai_chat_sse_upstream().await;
    std::env::set_var("V3_OPENAI_CHAT_TEST_KEY", "secret-chat");
    ...
    let response = client
        ...
        .json(&json!({
            "model":"client-chat",
            "input":"responses entry should use openai chat provider protocol",
            "stream":true
        }))
        .send()
        .await
    ...
    handle.shutdown().await;
    shutdown.send(()).unwrap();
    std::env::remove_var("V3_OPENAI_CHAT_TEST_KEY");

    assert_eq!(status, 200, "unexpected response body: {body}");
    assert_eq!(content_type, "text/event-stream");
    assert!(trace.contains("V3HubReqInbound01ClientRaw"), "{trace}");
    assert!(
        trace.contains("V3ServerRespOutbound06ClientFrame"),
        "{trace}"
    );
    assert!(
        !trace.contains("V3ResponsesDirect11Policy"),
        "mixed provider protocol /v1/responses must not enter Direct/P6: {trace}"
    );
    assert!(body.contains("event: response.completed"), "{body}");
    assert!(body.contains("event: response.done"), "{body}");
    assert!(body.contains("ROUTECODEX_OPENAI_CHAT_SSE_OK"), "{body}");
    assert!(body.contains("data: [DONE]"), "{body}");
    assert_eq!(capture.body["model"], "glm-5.2");
    assert!(
        capture.body.get("messages").is_some(),
        "OpenAI Chat provider must receive chat/completions wire body: {:?}",
        capture.body
    );
    assert!(
        capture.body.get("input").is_none(),
        "OpenAI Chat provider must not receive Responses /responses wire body: {:?}",
        capture.body
    );
}
...
async fn debug_endpoints_project_shared_runtime_state_and_dry_run_no_send() {
    ...
    let live_snapshots: serde_json::Value = client
        .get(format!(
        ...
        .await
        .unwrap();
    let serialized_live_snapshots = serde_json::to_string(&live_snapshots).unwrap();
    assert!(serialized_live_snapshots.contains("V3Server03HttpRequestRaw"));
    assert!(serialized_live_snapshots.contains("\"live\":true"));
    assert!(!serialized_live_snapshots.contains("sk-v3-secret"));

    let vr_dry_run: serde_json::Value = client
        .post(format!(
            "http://{}/_routecodex/diagnostics/virtual-router/dry-run",
            listener.addr
        ))
        .json(&serde_json::json!({
            "request": {
        ...
        .await
        .unwrap();
    assert_eq!(vr_dry_run["ok"], true);
    assert_eq!(vr_dry_run["localPort"], listener.addr.port());
    assert_eq!(vr_dry_run["routingPolicyGroup"], "default");
    assert_eq!(
        vr_dry_run["diagnostics"]["decision"]["selectedProviderKey"],
        "test:key:test"
    );
    assert_eq!(
        vr_dry_run["diagnostics"]["decision"]["wouldReturnProviderNotAvailable"],
        false
    );
    assert_eq!(
        vr_dry_run["diagnostics"]["diagnosticInput"]["entryEndpoint"],
        "/v1/responses"
    );
    ...
    let snapshots: serde_json::Value = client
        ...
        .await
        .unwrap();
    let retained_dry_run_snapshots = snapshots["snapshots"]
        .as_array()
        .unwrap()
        .iter()
        .filter(|snapshot| {
            snapshot["session_id"]
                .as_str()
                .is_some_and(|session_id| session_id.starts_with("snap-dry-run-"))
        })
        .collect::<Vec<_>>();
    assert!(
