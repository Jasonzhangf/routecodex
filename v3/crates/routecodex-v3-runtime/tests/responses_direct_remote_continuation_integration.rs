...
async fn direct_json_stopless_metadata_center_projects_noop_and_continues_on_remote_direct_locator()
{
    ...
    assert_eq!(
        state.len().unwrap(),
        0,
        "Direct pass-through must not commit a local bridge continuation"
    );
    assert_eq!(
        stopless.len().unwrap(),
        1,
        "Direct pass-through may retain scoped StoplessCenter control without exposing it in payload"
    );
    ...
    assert_eq!(
        ...
        0,
        "Direct request must not inject provider-visible stopless tools: {first_request}"
    );
}
...
async fn direct_json_stopless_noop_continuation_uses_native_reasoning_stop_call_id_not_local_cli_id(
    ...
    assert_eq!(requests.len(), 1);
    let first_request = serde_json::to_string(&requests[0]).unwrap();
    assert!(!first_request.contains("call_stopless_reasoning"));
}
...
async fn direct_stopless_real_user_turn_after_stale_noop_resets_guard_count() {
    ...
    assert!(
        ...
        "direct real user turn must not synthesize a local stopless bridge: {serialized}"
    );
    assert_eq!(stopless.len().unwrap(), 1);
}
...
async fn direct_stopless_fifth_activation_guards_clears_and_next_request_reactivates() {
    ...
    for round in 1..=5 {
        ...
        assert!(
            stopless.len().unwrap() <= 1,
            "Direct StoplessCenter control must remain scoped to one active entry"
        );
        assert!(
            state.len().unwrap() <= 1,
            "Direct remote continuation locator must not grow beyond one active entry"
        );
    }
...
async fn direct_sse_stopless_metadata_center_projects_terminal_frames_without_sse_owning_semantics()
{
    ...
    assert_eq!(
        ...
        0,
        "Direct SSE provider request must not inject provider-visible stopless tools: {provider_request}"
    );
}
...
#[derive(Default)]
struct PendingThenFailedSseAttemptThenSuccessTransport {
    requests: Mutex<Vec<Value>>,
}
...
#[async_trait]
impl ResponsesTransport for PendingThenFailedSseAttemptThenSuccessTransport {
    async fn send(
        &self,
        request: V3Transport13ResponsesHttpRequest,
    ) -> Result<V3ProviderResp14Raw, V3ProviderError> {
        ...
        match requests.len() {
            ...
            2 => Ok(V3ProviderResp14Raw::from_sse(
                ...
                Box::pin(stream::iter(vec![
                    Ok(concat!(
                        "event: response.output_text.delta\n",
                        "data: {\"type\":\"response.output_text.delta\",\"response_id\":\"resp_pinned_attempt\",\"delta\":\"partial\"}\n\n",
                    )
                    .as_bytes()
                    .to_vec()),
                    Ok(concat!(
                        "event: response.failed\n",
                        "data: {\"type\":\"response.failed\",\"response\":{\"id\":\"resp_pinned_attempt\",\"status\":\"failed\",\"error\":{\"code\":\"HTTP_502\",\"message\":\"pinned provider failed after delta\"}}}\n\n",
                    )
                    .as_bytes()
                    .to_vec()),
                ])),
...
async fn sse_two_turn_remote_continuation_commits_and_finishes_on_the_same_exact_pin() {
    ...
    assert_eq!(
        state.len().unwrap(),
        0,
        "live Direct SSE must not wait for provider EOF before returning"
    );
    let first_body = collect_sse_body_text(first.client_payload.body).await;
    assert!(first_body.contains("resp_sse_1"));
    assert!(first_body.contains("call_sse_1"));
    assert!(first_body.contains("[DONE]"));
    assert_eq!(state.len().unwrap(), 1, "terminal SSE closeout commits continuation truth");
    ...
    match body {
        V3ClientBody::Sse(mut stream) => {
            ...
            while let Some(chunk) = stream.next().await {
                bytes.extend_from_slice(&chunk.expect("live Direct SSE must not fail in success test"));
            }
            String::from_utf8(bytes).expect("controlled SSE must remain UTF-8")
        }
        V3ClientBody::CommittedSse(stream) => collect_committed_sse_text(stream).await,
        V3ClientBody::Json(_) | V3ClientBody::Bytes(_) => {
            panic!("successful SSE response must remain an SSE stream")
        }
    }
}
...
#[tokio::test]
async fn direct_sse_error_after_first_frame_stays_on_client_stream_and_fresh_request_remains_independent() {
    let routing_group = "direct_malformed_attempt";
    let manifest = http_only_manifest_without_remote_continuation_for_group(routing_group);
    let state = V3ResponsesDirectContinuationState::default();
    let first = execute_v3_responses_direct_runtime_kernel_with_continuation(
        &state,
    ...
    let V3ClientBody::Sse(mut stream) = first.client_payload.body else {
        panic!("the first semantic frame must admit the live Direct stream")
    };
    let partial = stream
        .next()
        .await
        .expect("partial frame must reach the client")
        .expect("partial frame must be valid");
    assert!(String::from_utf8_lossy(&partial).contains("partial"));
    let error = stream
        .next()
        .await
        .expect("malformed provider event must not become silent EOF")
        .expect_err("malformed provider event must remain a typed stream error");
...
#[tokio::test]
async fn direct_failed_terminal_after_first_frame_stays_on_client_stream() {
    let routing_group = "direct_failed_terminal_attempt";
    let manifest = http_only_manifest_without_remote_continuation_for_group(routing_group);
    let state = V3ResponsesDirectContinuationState::default();
    let first = execute_v3_responses_direct_runtime_kernel_with_continuation(
        &state,
    ...
    let error = stream
        .next()
        .await
        .expect("provider terminal failure must not become silent EOF")
        .expect_err("provider terminal failure must remain a typed stream error");
...
async fn terminal_sse_success_seals_replay_without_blocking_a_fresh_request() {
    ...
    let V3ClientBody::Sse(mut failed_stream) = failed.client_payload.body else {
        panic!("the first semantic frame must admit the live Direct stream")
    };
    assert!(String::from_utf8_lossy(
        &failed_stream
            .next()
            .await
            .expect("failed attempt partial frame")
            .expect("failed attempt partial frame must be valid")
    )
    .contains("partial"));
    assert!(failed_stream
        .next()
        .await
        .expect("failed attempt error must not become silent EOF")
        .is_err());
    ...
    assert_eq!(fresh.client_payload.status, 200, "{fresh:?}");
    let terminal_text = collect_sse_body_text(terminal.client_payload.body).await;
    assert!(
        terminal_text.contains("response.completed"),
        "{terminal_text}"
    );
    assert!(terminal_text.contains("[DONE]"), "{terminal_text}");
}
...
async fn sse_stream_error_after_restore_releases_locator_after_terminal_error05() {
    ...
    assert_eq!(
        ...
        "provider diagnostics must remain on the error chain, not the client payload"
    );
    assert!(output
        .node_trace
        .contains(&"V3HubRespContinuation04Committed"));
...
#[tokio::test]
async fn pinned_failed_sse_attempt_stays_live_without_provider_retry_or_front_leak() {
    let routing_group = "direct_pinned_failed_attempt";
    let manifest = manifest_for_group(routing_group);
    let state = V3ResponsesDirectContinuationState::default();
    let transport = PendingThenFailedSseAttemptThenSuccessTransport::default();
    prime_pending_with_id(
        &state,
        &manifest,
        scope_for_group(routing_group),
        &transport,
        1_000,
        "req-pinned-failed-attempt-prime",
    )
    .await;
    let recovered = continuation_turn(
        ...
        &transport,
        "resp_pinned_attempt",
        "req-pinned-failed-attempt-recovery",
        2_000,
    )
    .await;
    ...
    assert_eq!(
        ...
        0,
        "pinned provider-attempt retry must not re-enter or scan the Router"
    );
    assert_eq!(count(&recovered.node_trace, "V3TargetLocalReselected"), 0);
    let V3ClientBody::Sse(mut stream) = recovered.client_payload.body else {
        panic!("the admitted provider attempt must remain a live SSE stream")
    };
    let partial = stream
        .next()
        .await
        .expect("pinned attempt partial frame")
        .expect("pinned attempt partial frame must be valid");
    assert!(String::from_utf8_lossy(&partial).contains("partial"));
    assert!(stream
        .next()
        .await
        .expect("pinned provider failure must not become silent EOF")
        .is_err());
    assert!(stream.next().await.is_none());
    assert_eq!(state.len().unwrap(), 1, "post-admission failure must not rewrite continuation state");
    let requests = transport.requests.lock().unwrap();
    assert_eq!(requests.len(), 2, "post-admission failure must not retry provider");
}
