...
use routecodex_v3_runtime::{
    ...
    V3ResponsesRelayHealthSource, V3ResponsesRelayLocalContinuationScope,
...
async fn json_stopless_center_persists_without_local_continuation_store() {
    ...
    let result = execute_v3_responses_relay_runtime(&manifest(), V3ResponsesRelayRuntimeInput {
        ...
        }, V3ResponsesRelayExecutionEnv::new(&transport, V3ResponsesRelayHealthSource::Shared(&provider_health)).with_stopless_control(&stopless_control, scope.clone()))
    .await
    .unwrap();
...
async fn json_stopless_center_noop_cli_roundtrip_preserves_provider_tools() {
    ...
    let second = execute_v3_responses_relay_runtime(&manifest(), V3ResponsesRelayRuntimeInput {
        ...
        }, V3ResponsesRelayExecutionEnv::new(&transport, V3ResponsesRelayHealthSource::Shared(&provider_health)).with_local_stopless_control(V3ResponsesRelayLocalStoplessControlInput::new(
            &state,
            stopless_control.as_ref(),
            scope,
            31_000,
...
async fn sse_runtime_runs_stopless_center_through_json_hub_pipeline_before_client_sse() {
    ...
    let output = execute_v3_responses_relay_runtime(&manifest(), V3ResponsesRelayRuntimeInput {
        ...
        }, V3ResponsesRelayExecutionEnv::new(&transport, V3ResponsesRelayHealthSource::ManifestLocal).with_local_continuation(&state, scope, 50_000)).await.unwrap();
...
async fn provider_request_dry_run_uses_live_local_continuation_state() {
    ...
    let first = execute_v3_responses_relay_runtime(
        ...
        V3ResponsesRelayExecutionEnv::new(&transport, V3ResponsesRelayHealthSource::ManifestLocal)
            .with_local_continuation(&state, scope.clone(), 70_000),
    )
    .await
    .unwrap();
    let first_body = match first.client_body {
...
async fn json_two_turn_restores_tool_call_pairs_output_and_preserves_tools() {
    ...
    let first = execute_v3_responses_relay_runtime(
        ...
        V3ResponsesRelayExecutionEnv::new(&transport, V3ResponsesRelayHealthSource::ManifestLocal)
            .with_local_continuation(&state, scope.clone(), 1_000),
    )
    .await
    .unwrap();
    match first.client_body {
    ...
    let second = execute_v3_responses_relay_runtime(
        ...
        V3ResponsesRelayExecutionEnv::new(&transport, V3ResponsesRelayHealthSource::ManifestLocal)
            .with_local_continuation(&state, scope, 2_000),
    )
    .await
    .unwrap();
    match second.client_body {
...
async fn json_two_turn_preserves_responses_additional_tools_surface_and_tool_result_pairs() {
    ...
    let first = execute_v3_responses_relay_runtime(
        ...
        V3ResponsesRelayExecutionEnv::new(&transport, V3ResponsesRelayHealthSource::ManifestLocal)
            .with_local_continuation(&state, scope.clone(), 2_100),
    )
    .await
    .unwrap();
    match first.client_body {
    ...
    let second = execute_v3_responses_relay_runtime(
        ...
        V3ResponsesRelayExecutionEnv::new(&transport, V3ResponsesRelayHealthSource::ManifestLocal)
            .with_local_continuation(&state, scope, 2_200),
    )
    .await
    .unwrap();
    match second.client_body {
...
async fn json_two_turn_apply_patch_uses_freeform_projection_and_error_feedback() {
    ...
    let first = execute_v3_responses_relay_runtime(
        ...
        V3ResponsesRelayExecutionEnv::new(&transport, V3ResponsesRelayHealthSource::ManifestLocal)
            .with_local_continuation(&state, scope.clone(), 20_000),
    )
    .await
    .unwrap();
    match first.client_body {
    ...
    let second = execute_v3_responses_relay_runtime(&manifest(), V3ResponsesRelayRuntimeInput {
        ...
        }, V3ResponsesRelayExecutionEnv::new(&transport, V3ResponsesRelayHealthSource::ManifestLocal).with_local_continuation(&state, scope, 21_000))
    .await
    .unwrap();
    match second.client_body {
        V3ResponsesRelayClientBody::Json(body) => assert_eq!(body["status"], "completed"),
...
async fn wrong_tool_output_id_fails_before_provider_send_and_keeps_saved_context() {
    ...
    execute_v3_responses_relay_runtime(
        ...
        V3ResponsesRelayExecutionEnv::new(&transport, V3ResponsesRelayHealthSource::ManifestLocal)
            .with_local_continuation(&state, scope.clone(), 10_000),
    )
    .await
    .unwrap();
    assert_eq!(state.len().unwrap(), 1);
...
async fn responses_relay_selected_openai_chat_provider_uses_chat_wire_tools_and_schema() {
    ...
    let result = execute_v3_responses_relay_runtime(&manifest_openai_chat_wire(), V3ResponsesRelayRuntimeInput {
        ...
        }, V3ResponsesRelayExecutionEnv::new(&transport, V3ResponsesRelayHealthSource::ManifestLocal).with_local_continuation(&state, scope, 12_000))
    .await;
...
async fn responses_openai_chat_stopless_submit_restores_additional_tools_for_provider_wire() {
    ...
    let first = execute_v3_responses_relay_runtime(&manifest_openai_chat_wire(), V3ResponsesRelayRuntimeInput {
        ...
        }, V3ResponsesRelayExecutionEnv::new(&transport, V3ResponsesRelayHealthSource::ManifestLocal).with_local_continuation(&state, scope.clone(), 12_100))
    .await
