...
fn saturate_capacity_key(provider_key: &str) -> Vec<V3AdaptiveConcurrencyLease> {
    ...
    assert!(
        controller.try_acquire_business(provider_key).is_none(),
        "one normal lease at max_in_flight=1 must make the capacity pool busy"
    );
    vec![held]
}
...
#[tokio::test]
async fn provider_concurrency_all_busy_waits_once_then_rescans_after_release() {
    let first = "conc_all_busy_release_first";
    let second = "conc_all_busy_release_second";
    let held_first = saturate_capacity_key(&format!("{first}:key"));
    let held_second = saturate_capacity_key(&format!("{second}:key"));
    let manifest = provider_concurrency_two_provider_manifest(first, second, 200);
    let provider_health = V3ProviderFailureRuntimeHealth::from_manifest(&manifest);
    let transport = ProviderConcurrencyRecordingTransport::default();
    let raw = test_responses_raw(
        "default",
    ...
    let runtime = execute_v3_responses_direct_runtime_kernel_core(
        V3ResponsesDirectRuntimeCoreState::new()
            .with_provider_health(provider_health)
            .with_now_epoch_ms(10),
        &manifest,
    ...
    tokio::select! {
        ...
        output = &mut runtime => {
            ...
            release_capacity_leases(held_second);
            panic!("runtime completed before all-busy release: {output:?}");
        }
    }
    let output = tokio::time::timeout(Duration::from_millis(300), &mut runtime)
        .await
        .expect("release before deadline must wake the all-busy wait");
    release_capacity_leases(held_first);
...
#[tokio::test]
async fn provider_concurrency_all_busy_times_out_without_provider_failure_health() {
    let first = "conc_all_busy_timeout_first";
    let second = "conc_all_busy_timeout_second";
    let held_first = saturate_capacity_key(&format!("{first}:key"));
    let held_second = saturate_capacity_key(&format!("{second}:key"));
    let manifest = provider_concurrency_two_provider_manifest(first, second, 25);
    let provider_health = V3ProviderFailureRuntimeHealth::from_manifest(&manifest);
    let transport = ProviderConcurrencyRecordingTransport::default();
    let raw = test_responses_raw(
        "default",
    ...
    assert!(
        ...
        "all-busy timeout must not send a provider request"
    );
    assert!(
        !output
            .node_trace
...
async fn provider_concurrency_all_busy_uses_due_scheduled_probe_for_admission() {
    ...
    assert!(
        snapshot.saturated,
        "fake runtime transport proves admission selection, not probe recovery completion"
    );
...
#[tokio::test]
async fn provider_concurrency_relay_all_busy_times_out_without_provider_failure_health() {
    let first = "conc_relay_all_busy_timeout_first";
    let second = "conc_relay_all_busy_timeout_second";
    let held_first = saturate_capacity_key(&format!("{first}:key"));
    let held_second = saturate_capacity_key(&format!("{second}:key"));
    let manifest = provider_concurrency_two_provider_relay_manifest(first, second, 25);
    let provider_health = V3ResponsesRelayProviderHealthHandle::from_manifest_without_persistence(&manifest);
    let transport = ProviderConcurrencyRecordingTransport::default();
