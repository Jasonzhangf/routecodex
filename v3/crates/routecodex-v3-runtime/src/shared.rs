...
mod tests {
    ...
    #[tokio::test]
    async fn diagnosis_direct_sse_empty_output_item_then_failed_must_fail_before_client_commit() {
        let raw = V3ProviderResp14Raw::from_sse(
            "req".to_string(),
            "provider".to_string(),
            201,
            ...
            Box::pin(stream::iter(vec![
                ...
                Ok::<Vec<u8>, V3ProviderError>(
                    b"event: response.failed\ndata: {\"type\":\"response.failed\",\"response\":{\"status\":\"failed\",\"error\":{\"code\":\"provider_failed\",\"message\":\"cc-sol failed after empty lifecycle frame\"}}}\n\n".to_vec(),
                ),
            ])),
        );
    ...
    #[tokio::test]
    async fn diagnosis_direct_sse_empty_output_item_then_eof_must_fail_before_client_commit() {
        let raw = V3ProviderResp14Raw::from_sse(
            "req".to_string(),
            "provider".to_string(),
            201,
