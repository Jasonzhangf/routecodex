...
async fn guard_initial_direct_sse_provider_failure_with_timeout(
    ...
    // Client-commit authority: SSE failures stay precommit Error01->05 for switching, never client SSE error events.
    // Error06 projects HTTP 502 only after candidate and default pools are both exhausted; otherwise preserve frames.
...
mod tests {
    ...
    include!("shared_direct_sse_precommit_tests.rs");
