...
include!("shared_direct_sse_precommit.rs");
...
mod tests {
    ...
    #[tokio::test]
    async fn guard_initial_direct_sse_rejects_empty_completed_before_client_commit() {
        let wire = concat!(
            "event: response.created\n",
            "data: {\"type\":\"response.created\",\"response\":{\"id\":\"resp_empty\",\"status\":\"in_progress\"}}\n\n",
            "event: response.output_item.added\n",
            "data: {\"type\":\"response.output_item.added\",\"output_index\":0,\"item\":{\"type\":\"message\",\"status\":\"in_progress\",\"content\":[{\"type\":\"output_text\",\"text\":\"\"}]}}\n\n",
            "event: response.content_part.added\n",
            "data: {\"type\":\"response.content_part.added\",\"output_index\":0,\"item_id\":\"msg_empty\",\"content_index\":0,\"part\":{\"type\":\"output_text\",\"text\":\"\"}}\n\n",
            "event: response.completed\n",
        ...
        .await;
        let Err(error) = result else {
            panic!("empty response.completed must remain precommit and enter Error01");
        };

        assert_eq!(error.source_kind, V3ErrorSourceKind::ProviderFailure);
        assert_eq!(error.code, "provider_response_sse_empty");
        assert!(error.message.contains("no content or tool output"));
    }
