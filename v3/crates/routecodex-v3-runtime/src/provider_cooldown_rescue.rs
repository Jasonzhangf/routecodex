...
fn v3_provider_busy_wait_timeout_ms(
    expanded: &V3Target09CandidateSetExpanded,
    busy_candidates: &BTreeSet<String>,
) -> u64 {
    expanded
        .candidates
        .iter()
        .filter(|candidate| busy_candidates.contains(&v3_relay_provider_candidate_key(candidate)))
        .map(|candidate| candidate.concurrency_acquire_timeout_ms)
        .min()
        .unwrap_or(0)
}
    ...
    loop {
        ...
        .await
        {
            V3TargetSelectionAfterRescue::Selected(selected) => {
                ...
                busy_capacity_keys.insert(capacity_key.clone());
            ...
            V3TargetSelectionAfterRescue::Exhausted(exhausted) => {
                ...
                wait_consumed = true;
                let timeout_ms = v3_provider_busy_wait_timeout_ms(&expanded, &busy_candidates);
                let wait_result = if timeout_ms == 0 {
                    Err(())
                } else {
                    controller
                        .wait_for_capacity_change(
                            busy_capacity_keys.iter().map(String::as_str),
                            Duration::from_millis(timeout_ms),
                        )
                        .await
                };
                busy_candidates.clear();
                busy_capacity_keys.clear();
                busy_selected_candidates.clear();
                if wait_result.is_err() {
                    // Rescan once after the bounded wait deadline before
                    // projecting all-busy exhaustion; this closes the race
                    // where a release lands exactly as the timer expires.
                    wait_consumed = true;
                }
            }
        }
