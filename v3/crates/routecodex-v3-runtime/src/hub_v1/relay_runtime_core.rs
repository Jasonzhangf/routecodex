...
/// Relay SSE 首帧超时：provider SSE 首帧挂起时 fail-fast 进入错误链。
/// 短于常见客户端（zterm/codex/reasonix ~8-12s）超时重试窗口，避免 client 无限重试。
const V3_RELAY_SSE_FIRST_FRAME_TIMEOUT: std::time::Duration =
    std::time::Duration::from_secs(8);

/// Relay transport 响应头超时：provider 在窗口内未返回响应头（上游挂起）时
/// 归一化为 Transport 错误进入错误链（记录 provider failure + reselect 切 provider），
/// 与 responses 入口的失败处理一致，避免 chat 入口 client 无限重试且 console 静默。
    ...
    T: ResponsesTransport,
{
    ...
    loop {
        ...
        .await
        {
            ...
            Err(_elapsed) => {
                // provider 响应头在窗口内未返回（上游挂起）：归一化为 Transport
                // 错误进入错误链（记录 provider failure + reselect 切 provider）。
