...
/// Responses SSE 结算尾部帧判定：`response.completed` 之后 provider 可能继续发送
/// 非语义 keepalive/成本帧（如 opencode.ai/zen/go 的 `{"type":"ping","cost":"0"}`）。
/// 这些帧不携带输出语义，必须容忍（跳过），否则会把良性协议尾误判为流错误并
/// abort 已向客户端提交 [DONE] 的连接。语义事件（输出增量/工具调用参数）在
/// completed 之后出现仍是异常，必须保持 fail-fast。
fn is_v3_responses_settlement_tail_frame(data: &str) -> bool {
    let Ok(value) = serde_json::from_str::<serde_json::Value>(data) else {
        return false;
    };
    let Some(event_type) = value.get("type").and_then(serde_json::Value::as_str) else {
        // 无 type 的裸帧（纯 usage/cost 结算帧）视为结算尾。
        return true;
    };
    match event_type {
        "response.completed" | "response.incomplete" | "response.failed" => false,
        _ if event_type.contains("output")
            || event_type.contains("function_call")
            || event_type.contains("tool_call") =>
        {
            false
        }
        // ping / usage / 未知 keepalive 类帧 → 结算尾。
        _ => true,
    }
}
...
include!("openai_chat_relay_runtime_sse.rs");
