...
#[test]
fn anthropic_outbound_rejects_unmapped_responses_only_reasoning_policy_fields() {
    let chat = json!({
        "model": "MiniMax-M3",
    ...
    let error = encode_v3_responses_semantic_as_anthropic_request(chat)
        .expect_err("unmapped Responses reasoning policy fields must fail at Anthropic owner");
    let display = error.to_string();
    assert!(
        display.contains("$.request.reasoning_context_policy"),
        "{display}"
    );
    assert!(display.contains("$.request.reasoning_mode"), "{display}");
    assert!(
        display.contains("$.request.reasoning_include_thoughts"),
        "{display}"
    );
}
