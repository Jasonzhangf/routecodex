...
const WEB_SEARCH_SCORE_THRESHOLD: f32 = 0.6;
...
impl TermStrength {
    fn weight(self) -> f32 {
        match self {
            TermStrength::Strong => 1.0,
            TermStrength::Medium => 0.6,
            TermStrength::BrandWeak => 0.4,
        }
    }
}
...
const WEB_SEARCH_TERMS: &[TermHit] = &[
    TermHit { term: "search the web", strength: TermStrength::Strong },
    TermHit { term: "web search", strength: TermStrength::Strong },
    TermHit { term: "browse the web", strength: TermStrength::Strong },
    TermHit { term: "联网搜索", strength: TermStrength::Strong },
    TermHit { term: "请搜索", strength: TermStrength::Strong },
    TermHit { term: "引用来源", strength: TermStrength::Strong },
    TermHit { term: "with sources", strength: TermStrength::Strong },
    TermHit { term: "search online", strength: TermStrength::Medium },
    TermHit { term: "look it up", strength: TermStrength::Medium },
    TermHit { term: "上网搜索", strength: TermStrength::Medium },
    TermHit { term: "上网搜", strength: TermStrength::Medium },
    TermHit { term: "网页搜索", strength: TermStrength::Medium },
    TermHit { term: "搜一下", strength: TermStrength::Medium },
    TermHit { term: "搜一搜", strength: TermStrength::Medium },
    TermHit { term: "查一下", strength: TermStrength::Medium },
    TermHit { term: "查一查", strength: TermStrength::Medium },
    TermHit { term: "look up", strength: TermStrength::BrandWeak },
    TermHit { term: "搜网页", strength: TermStrength::BrandWeak },
    TermHit { term: "查网页", strength: TermStrength::BrandWeak },
    TermHit { term: "搜个", strength: TermStrength::BrandWeak },
    TermHit { term: "搜点", strength: TermStrength::BrandWeak },
    TermHit { term: "google it", strength: TermStrength::BrandWeak },
    TermHit { term: "google 一下", strength: TermStrength::BrandWeak },
    TermHit { term: "bing 一下", strength: TermStrength::BrandWeak },
    TermHit { term: "百度一下", strength: TermStrength::BrandWeak },
    TermHit { term: "谷歌一下", strength: TermStrength::BrandWeak },
];

const WEB_SEARCH_NEGATIVE_PATTERNS: &[&str] = &[
    "do not web search",
    "don't web search",
    "don't search",
    "do not search",
...
fn has_web_search_negative_context(text: &str) -> bool {
    let normalized = text.to_lowercase();
    WEB_SEARCH_NEGATIVE_PATTERNS
        .iter()
        .any(|p| normalized.contains(p))
}

pub fn has_web_search_intent(text: &str) -> bool {
    if has_web_search_negative_context(text) {
        return false;
    }
    let normalized = text.to_lowercase();
    let mut total = 0.0_f32;
    let mut strong_hit = false;
    for hit in WEB_SEARCH_TERMS {
        if normalized.contains(hit.term) {
            total += hit.strength.weight();
            if matches!(hit.strength, TermStrength::Strong) {
                strong_hit = true;
            }
        }
    }
    let score = total.min(1.0);
    strong_hit || score >= WEB_SEARCH_SCORE_THRESHOLD
}
