...
const verifier = resolve(v3Root, 'scripts/architecture/verify-v3-normalization-payload-logic-boundary.mjs');
const fixtures = [
  {
...
for (const fixture of fixtures) {
  ...
  try {
    cpSync(resolve(v3Root, 'crates/routecodex-v3-runtime/src'), join(root, 'crates/routecodex-v3-runtime/src'), { recursive: true });
    cpSync(resolve(v3Root, 'crates/routecodex-v3-provider-responses/src'), join(root, 'crates/routecodex-v3-provider-responses/src'), { recursive: true });
    cpSync(resolve(admissionRoot, 'docs/architecture/manifests'), join(root, 'docs/architecture/manifests'), { recursive: true });
    const target = join(root, fixture.file.replace(/^v3\//u, ''));
    ...
    const result = spawnSync(process.execPath, [verifier], {
      ...
      env: {
        ...
        ROUTECODEX_V3_FIXTURE_ROOT: root,
      },
    });
