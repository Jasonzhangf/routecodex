...
const relaySharedPath = 'v3/crates/routecodex-v3-runtime/src/hub_v1/relay_runtime_shared.rs';
...
const endpointHandlersPath = 'v3/crates/routecodex-v3-server/src/endpoint_handlers.rs';
const frameBuildersPath = 'v3/crates/routecodex-v3-server/src/frame_builders.rs';
...
const relayCore = read(relayCorePath);
const relayShared = read(relaySharedPath);
...
const serverExecutors = read(serverExecutorsPath);
const endpointHandlers = read(endpointHandlersPath);
const frameBuilders = read(frameBuildersPath);
...
]) requireText(runtimeSurface, `${runtimePath}+${relayCorePath}+${relaySharedPath}`, phrase);
...
for (const phrase of [
  ...
  'validated Gemini attempt must complete after the controlled terminal',
  'metadata_center',
...
for (const phrase of [
  'execute_v3_gemini_relay_runtime_with_default_transport',
  'V3Server16Body::Sse(client_stream)',
  'Body::from_stream(',
  'entry_protocol_binding_for_endpoint(&path)',
  'entry_protocol == "gemini"',
  'execute_v3_gemini_generate_content_request',
]) requireText(serverSurface, `${serverPath}+${serverExecutorsPath}+${endpointHandlersPath}`, phrase);
for (const phrase of [
...
const geminiServerProjection = slice(serverExecutors, 'fn gemini_relay_output_response', 'fn anthropic_relay_output_response');
forbid(geminiServerProjection, serverExecutorsPath, [
...
forbid(runtimeSurface, `${runtimePath}+${relayCorePath}+${relaySharedPath}`, [
  /fallback/i,
