...
const root = process.env.ROUTECODEX_V3_ADMISSION_WORKSPACE === '1'
  ? process.cwd()
  : path.join(process.cwd(), 'build-contracts', 'architecture-admission', 'repo');
const v3Root = process.env.ROUTECODEX_V3_ADMISSION_WORKSPACE === '1'
  ? path.join(root, 'v3')
  : process.cwd();
const failures = [];
const paths = {
...
function abs(rel) {
  if (rel === 'package.json') return path.resolve(v3Root, rel);
  return rel.startsWith('v3/')
    ? path.resolve(v3Root, rel.slice(3))
