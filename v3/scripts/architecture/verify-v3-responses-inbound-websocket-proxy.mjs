...
const v3Root = explicitRootIsV3Root || runningFromV3Root
  ? explicitRootIsV3Root
    ? sourceRoot
    : resolve(process.cwd())
  : process.env.ROUTECODEX_V3_ADMISSION_WORKSPACE === '1' || explicitSourceRoot
    ? resolve(sourceRoot, 'v3')
    : installedV3Root;
const architectureRoot = explicitRootIsV3Root || runningFromV3Root
  ? explicitRootIsV3Root
    ? resolve(sourceRoot, '..')
    : resolve(process.cwd(), '..')
  : runningFromV3Root
    ? resolve(process.cwd(), '..')
    : sourceRoot;
const resolveFile = (file) => file.startsWith('v3/')
  ? resolve(v3Root, file.slice(3))
  : resolve(architectureRoot, file);
const text = Object.fromEntries(
  Object.entries(files).map(([key, file]) => [key, readFileSync(resolveFile(file), 'utf8')]),
);
...
if (wsStart < 0 || wsEnd <= wsStart) {
  ...
  const wsSection = text.websocket.slice(wsStart, wsEnd);
  forbid(files.websocket + ': WebSocket section', wsSection, [
    /connect_async\s*\(/,
    /WebSocketStream</,
