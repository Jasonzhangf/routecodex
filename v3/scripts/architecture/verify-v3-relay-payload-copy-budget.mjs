...
const paths = {
  ...
  providerResponsesWebsocketVerifier: 'v3/scripts/architecture/verify-v3-responses-websocket-v2-transport-hardening.mjs',
...
const text = Object.fromEntries(
  Object.entries(paths).map(([key, path]) => [key, readSurface(path)]),
);
const runtime = [
  text.response,
  text.responseCommon,
...
forbid(text.request, paths.request, /fn forbidden_roundtrip/, 'JSON serialization round-trip clone');
forbid(text.responsesRelayRuntime, paths.responsesRelayRuntime, /fn forbidden_sse_materialize/, 'unowned full SSE materialization');
forbid(
  runtime,
...
const packageJson = JSON.parse(readFileSync(resolve(root, 'v3/package.json'), 'utf8'));
for (const script of [
...
function readSurface(relative) {
  const parts = [readFileSync(resolve(root, relative), 'utf8')];
  const siblingDirectory = resolve(root, relative.replace(/\.rs$/, ''));
  try {
    if (statSync(siblingDirectory).isDirectory()) {
      for (const entry of readdirSync(siblingDirectory).sort()) {
        ...
        if (statSync(child).isFile() && entry.endsWith('.rs')) {
          parts.push(readFileSync(child, 'utf8'));
        }
      }
    }
  } catch {
    // A split sibling directory is optional; the owning source file is mandatory.
  }
  return parts.join('\n');
}
