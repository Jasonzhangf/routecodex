...
const lifecycleFiles = [
  ...
  'v3/crates/routecodex-v3-lifecycle/src/control_plane.rs',
  'v3/crates/routecodex-v3-lifecycle/src/exec_restart.rs',
  'v3/crates/routecodex-v3-lifecycle/src/fs_locks.rs',
  'v3/crates/routecodex-v3-lifecycle/src/pid_scan.rs',
  'v3/crates/routecodex-v3-lifecycle/src/reap.rs',
  'v3/crates/routecodex-v3-lifecycle/src/tests.rs',
];
const lifecycle = lifecycleFiles.map(read).join('\n');
...
requireText(configStore, 'source_closure_sha256(&canonical_path, &source.raw_toml, &parsed.provider_sources)', 'config store source identity owner');
...
requireText(cliManagedTests, 'top-level start must stream standard human startup status with version/binary evidence even when config log_console=false', 'managed CLI human startup console assertion');
...
requireText(cliManagedTests, 'plain_start_stdout.contains("▶ [/v1/responses]")', 'managed CLI request monitor assertion');
...
requireText(cliManagedTests, 'strip_ansi', 'managed CLI ANSI-stripped semantic assertions');
...
requireText(cliManagedTests, '!plain_start_stderr.contains("❌ [unknown]")', 'managed CLI unknown path endpoint assertion');
requireCount(cliManagedTests, '!plain_start_stderr.contains("request pre-request failed")', 2, 'managed CLI no pre-request assertion');
requireText(cliManagedTests, 'assert_ne!(\n        first_color, second_color', 'managed CLI different session color assertion');
requireText(cliManagedTests, 'plain_start_stdout.contains("[virtual-router-hit]")', 'managed CLI V2 virtual-router-hit assertion');
requireText(cliManagedTests, 'plain_start_stdout.contains("sessionID:console-alpha")', 'managed CLI V2 session id assertion');
requireText(cliManagedTests, 'plain_start_stdout.contains("reason=provider-request-dry-run")', 'managed CLI V2 route reason assertion');
requireText(cliManagedTests, 'plain_start_stdout.contains("[provider-request-dry-run]")', 'managed CLI V2 usage route assertion');
requireText(cliManagedTests, 'plain_start_stdout.contains("[test.wire-test")', 'managed CLI V2 usage model assertion');
requireText(cliManagedTests, 'plain_start_stdout.contains("finish_reason=")', 'managed CLI V2 finish_reason assertion');
requireText(cliManagedTests, 'log_text.contains("\\u{1b}[")', 'managed CLI persisted ANSI server log assertion');
requireText(cliManagedTests, 'plain_log_text.contains("[virtual-router-hit]")', 'managed CLI persisted V2 route log assertion');
requireText(cliManagedTests, 'plain_start_stdout.contains("✅ [/v1/responses]")', 'managed CLI completion monitor assertion');
requireText(cliManagedTests, 'plain_start_stdout.contains("[usage]")', 'managed CLI usage monitor assertion');
requireText(cliManagedTests, 'plain_start_stderr.contains("error=V3E")', 'managed CLI compact error number assertion');
requireText(cliManagedTests, '!plain_start_stderr.contains("errorChain=")', 'managed CLI no foreground error chain assertion');
...
requireText(cli, `configure_v3_snapshot_flags(
...
requireText(cli, 'configure_v3_snapshot_flags(', 'CLI snap override dispatch');
...
requireText(server, 'allocate_v3_console_request_identity', 'Server independent foreground request identity resolver');
