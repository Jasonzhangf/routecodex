...
const websocketPath = 'v3/crates/routecodex-v3-provider-responses/src/transport/websocket.rs';
...
const packagePath = 'v3/package.json';
...
const explicitRootIsV3Root = explicitSourceRoot
  && existsSync(resolve(explicitSourceRoot, 'crates'))
  && existsSync(resolve(explicitSourceRoot, 'package.json'));
const runningFromV3Root = !explicitSourceRoot
  && existsSync(resolve(process.cwd(), 'crates'))
  && existsSync(resolve(process.cwd(), 'package.json'));
const architectureRoot = explicitSourceRoot
  ? explicitRootIsV3Root
  ...
  : process.env.ROUTECODEX_V3_ADMISSION_WORKSPACE === '1'
    ...
    : runningFromV3Root
      ? resolve(process.cwd(), '..')
      : resolve(installedV3Root, 'build-contracts', 'architecture-admission', 'repo');
const v3Root = explicitRootIsV3Root
  ? resolve(explicitSourceRoot)
  : explicitSourceRoot || process.env.ROUTECODEX_V3_ADMISSION_WORKSPACE === '1'
    ? resolve(architectureRoot, 'v3')
    : runningFromV3Root
      ? resolve(process.cwd())
      : installedV3Root;
const resolveFile = (path) => path.startsWith('v3/')
  ? resolve(v3Root, path.slice(3))
  : resolve(architectureRoot, path);
const read = (path) => readFileSync(resolveFile(path), 'utf8');
const transport = read(transportPath);
const websocket = read(websocketPath);
const transportSurface = `${transport}\n${websocket}`;
...
for (const [owner, text, phrases] of [
  [`${transportPath} + ${websocketPath}`, transportSurface, [
...
const wsEnd = transport.indexOf('fn content_type_header(', wsStart);
const wsSseStart = websocket.indexOf('fn websocket_sse_stream(');
if (wsStart < 0 || wsEnd <= wsStart || wsSseStart < 0) {
  failures.push(`${transportPath}: missing WebSocket transport owner boundary`);
} else {
  const wsJsonOwner = transport.slice(wsStart, wsEnd);
  ...
  const wsOwner = `${wsJsonOwner}\n${websocket.slice(wsSseStart)}`;
  ...
  for (const phrase of [
    ...
    if (!wsOwner.includes(phrase)) {
      failures.push(`${transportPath} + ${websocketPath}: missing Codex WebSocket error parsing phrase ${phrase}`);
    }
  }
}
