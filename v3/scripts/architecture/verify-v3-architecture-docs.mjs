...
const rewriteRel = (rel) => {
  ...
  if (!sourceRoot && !admissionWorkspace) {
    if (rel.startsWith('scripts/') || rel === 'scripts') return path.join(v3Root, rel);
  }
  return path.join(root, rel);
};
const exists = (rel) => fs.existsSync(rewriteRel(rel));
const failures = [];
const requiredFiles = [
...
const abs = (file) => rewriteRel(file);
const read = (file) => {
...
const readSourceWithSiblingModule = (file) => {
  ...
  const stemAbs = abs(stem);
  if (!fs.existsSync(stemAbs) || !fs.statSync(stemAbs).isDirectory()) return primary;
  const parts = [primary];
  for (const entry of fs.readdirSync(stemAbs)) {
...
for (const file of requiredFiles) if (!exists(file)) fail(`${file}: missing`);
