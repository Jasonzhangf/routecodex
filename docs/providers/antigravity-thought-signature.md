...
当前 active code 不应恢复这些 TS compat action shell。若需要重新接入 Antigravity thought-signature cache/prepare flow，必须落到 Rust `router-hotpath-napi` / compat owner，并只通过 Host N-API 薄壳暴露。
...
- llmswitch-core（恢复模式：strip + recovery prompt；旧 TS action 已删除，不能作为当前入口恢复）：
  - `sharedmodule/llmswitch-core/src/conversion/compat/actions/antigravity-thought-signature-prepare.ts`
...
另见（recoverable / cooldown 的标准行为）：`docs/antigravity-routing-contract.md`
