...
| resp_process stage1 tool governance | `sharedmodule/llmswitch-core/src/conversion/hub/pipeline/stages/resp_process/resp_process_stage1_tool_governance/index.ts` | 当前 stage shell 只做：1) `resolveRequestedToolNamesWithNative()`；2) `prepareRespProcessToolGovernancePayloadWithNative()`；3) `applyRespProcessToolGovernanceWithNative()`；4) `recordStage`。此前 TS 侧 `attachRequestedToolNames()` / `markTextHarvestApplied()` / `__rcc_tool_governance` sidecar 写入已移除。 | `thin-shell` | `tests/sharedmodule/resp-process-chat-normalize-from-responses.spec.ts`、`tests/sharedmodule/resp-process-tool-allowlist-contract.spec.ts`、本目录 `__tests__/resp-process-stage1-tool-governance-native.test.ts`、`tests/sharedmodule/hub-pipeline-stage-residue-audit.spec.ts` | stage shell residue 已拔掉；当前缺口转为 Rust 文件 `resp_process_stage1_tool_governance.rs` 内部还有非本轮焦点的未收口分支与未全绿 cargo slice，不能扩大宣称为“resp_process stage1 全闭环完成”。 | 保持 stage shell 薄壳；后续只在 Rust 真源继续收口剩余语义，不允许 TS stage 回流 sidecar/payload 改写。 |
...
1. `tests/sharedmodule/hub-pipeline-stage-residue-audit.spec.ts`
   ...
   - 红测二：`resp_process_stage1_tool_governance/index.ts` 不得重新引入 `__rcc_tool_governance` / `attachRequestedToolNames` / `markTextHarvestApplied`

2. 后续 deletion gate（待 Rust 收口后补）
   - 删除 req_process stage 内 heartbeat / clock / sanitizer TS residue 后，原行为回归必须保持全绿。
   - resp_process stage1 当前以“禁止回流 TS sidecar 语义”为门禁，不再以 stage-shell 删除为目标。
