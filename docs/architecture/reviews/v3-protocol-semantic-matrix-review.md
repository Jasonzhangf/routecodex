...
| --- | ---: | --- |
| `covered` | 162 | Runtime/test owner currently implements the audited semantic for the named path. |
...
| `runtime_conformance_pending` | 3 | Design and owner are fixed, but positive/negative runtime and live evidence have not passed. |
| `partial` | 112 | Some runtime path or direction is covered, but cross-protocol or value-shape parity is incomplete. |
| `extension_declared` | 219 | The OpenAI Chat extension field and semantic owner are declared, but runtime conversion closeout is not claimed. |
...
| `gap.anthropic_response_projection_completion` | source closed; live closeout pending | `covered` / 3 | Anthropic stop reasons, typed tool-result error status, and the duplicate terminal classification row are locked by the 74/7 red baseline, 81/0 focused green suite, and JSON/SSE shared-owner regressions. | Keep the only implementations in the adjacent Anthropic codec and generic typed terminality owner; install, aggregate restart, online replay, and DSH Review are still required before delivery. |
