...
After `V3Target10ConcreteProviderSelected`, Runtime immediately evaluates `V3Execution11ProtocolDecision`: `provider.responses.process` remains provider-private truth and does not override a same-protocol responses entry into Relay, protocol mismatch with Relay allowed must enter Hub Relay, and mismatch without Relay fails before provider send.
