...
- For `stopreason=2`, the provider-facing continuation text must be derived from the model's own `current_goal` plus `next_step` as a completion-check prompt. For `blocked + needs_user_input=true`, the finalized client response must include the summary plus the user decision question and stop with `finish_reason=stop`.
