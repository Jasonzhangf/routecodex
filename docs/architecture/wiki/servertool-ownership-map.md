...
- Stop schema fields are conditionally required, not globally required: `simple_question=true` allows natural stop for very simple user inputs without `stopreason`; otherwise `stopreason` is unconditional; `stopreason=0` requires `has_evidence=1` plus non-empty `evidence`; `stopreason=1` requires non-empty `reason` and may stop with reason only; `stopreason=2` requires `next_step`; diagnostic fields are optional unless a future rule explicitly makes them conditional.
