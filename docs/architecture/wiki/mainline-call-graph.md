...
| native.provider_response_servertool_retirement_effect | `plan_provider_response_servertool_retirement_effect_json` | `hub.provider_response_servertool_retirement_effect_plan`<br/>Rust-owned provider-response legacy servertool retirement effect plan | Rust selects continue/reject and optional stop-gateway write for retired provider-response servertool actions; TS executes only the returned effects. |
...
| native.provider_response_stream_pipe_effect | `plan_provider_response_stream_pipe_effect_json` | `hub.provider_response_stream_pipe_effect_plan`<br/>Rust-owned provider-response stream-pipe effect plan | Rust validates streamPipe and selects no_pipe/use_pipe with canonical codec/requestId/payload; TS consumes the returned pipe only for Node stream IO. |
