...
- `scripts/architecture/verify-responses-request-compat-rust-only.mjs`
  - 检查 `responses:temperature-unsupported` request compat 与通用 Responses function/instructions normalization 的唯一真源仍在 Rust `req_outbound_stage3_compat`，TS 只能通过 `runReqOutboundStage3CompatJson` 调用
