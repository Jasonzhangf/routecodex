...
| **Process Stage (host)** | RouteCodex workflow 对 canonical JSON 做路由与 Rust/native effect plan 执行；写入 `providerId/providerProtocol/processMode/stream`。| Rust Virtual Router / Hub Pipeline owner + `sharedmodule/llmswitch-core/src/conversion/compat/*.json` authoring data |
