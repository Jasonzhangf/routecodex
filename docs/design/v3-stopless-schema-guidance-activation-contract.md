...
9. **Path scope**: The rule applies to Responses-protocol provider turns whether they are direct or relay. Relay owns StoplessCenter state. Direct may use the same response evidence gate only when same-turn StoplessCenter activation exists, but it must not write relay StoplessCenter or invent non-native remote continuation state.
