...
```text
provider/runtime/send/convert error
  -> ProviderFailurePolicy.classify(error, stage, context)
  -> ProviderFailurePolicy.decide(classification, route context, retry context)
  -> decision {
       recoverability,
       affectsHealth,
       action,
  ...
  ->
     A. RequestExecutor 执行 decision（wait / retry / direct return）
     B. ProviderErrorReporter 仅上报 decision 结果
     C. Virtual Router 仅消费 decision-affecting event / policy projection
```
