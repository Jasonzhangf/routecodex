...
## 目标 / 非目标
- 目标：servertool followup 只能从 origin snapshot 构造一个正常请求，并重新进入完整 Hub Pipeline。
- 目标：servertool 只代客户端执行本地工具或发起 followup；不拥有专用响应协议、不投影客户端 shape、不修补 provider shape。
- 目标：followup 请求与正常请求同构，保留原请求工具列表、模型参数、entry endpoint 与协议语义。
...
- `ServertoolReq04FollowupBuilt`：从 origin snapshot 生成正常 followup 请求；禁止从当前污染 payload 猜测补偿。
...
5. 禁止清洗工具列表、重写历史、多轮合并历史来“修复” followup。
...
5. followup body 必须保留 origin tools 与参数，不允许清洗工具列表。
