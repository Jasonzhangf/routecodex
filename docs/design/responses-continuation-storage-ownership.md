...
2. **relay store restore 同步** — `route-aware-responses-continuation.ts` 中 `syncProviderKeyToRoutingPin()` 在 `consult_scope_store` 模式下 restore/materialize 成功后，将 store 中保存的 `providerKey` 同步写入 `normalizedMetadata.__shadowCompareForcedProviderKey`。
