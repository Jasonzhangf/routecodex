...
- CLI input 只允许包含状态字段：`flowId`、`repeatCount`、`maxRepeats`；不得携带 `continuationPrompt`、schema guidance、prompt preview 或完整内部 input。
- 真实续轮提示词由请求端根据 stopless 状态和 schema gate reason 注入；`continuationPrompt` 只可作为内部 seed/request-side 注入材料，不得进入 client-visible `exec_command` 或 CLI stdout。
...
- 状态记录在 runtime/followup metadata scope 下：
  ...
  - `stopMessageUsed`
- CLI input 中 `repeatCount` / `maxRepeats` 必须始终存在，缺 runtime state 时使用 `repeatCount=0`、`maxRepeats=3`；CLI stdout 只能返回执行状态，不回显注入提示词。
