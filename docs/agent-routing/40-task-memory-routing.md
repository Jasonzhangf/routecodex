...
## Skill 精华沉淀（强制）
1. 先查现成 skill：当检查一个问题时，先在相关 local skill / global skill 中找是否已有现成流程和经验，再决定是否需要新增或更新。
2. 探索后若出现“新精华”或“旧规则失效/需修正”，才更新对应 local skill（`.agents/skills/*/SKILL.md`）；无新增信息时不强制写。
3. 记录的是“经验精华”，不是流水账。至少提炼以下 4 项：
   - 触发信号：什么日志/症状说明要走这条路径；
   - 关键判断：如何区分真因与假因；
   - 可复用动作：下次应直接执行的最小动作序列；
   - 边界/反模式：哪些做法会导致复发，明确禁止。
4. 单条沉淀建议 1-8 行（允许一两句短结论），要求“可执行 + 可复用 + 可验证”；禁止堆叠无结论的时间线。
5. 技能沉淀与 `MEMORY.md` 职责分离：
   - `SKILL.md`：方法论、排障路径、动作模板（面向下次执行）；
   - `MEMORY.md`：项目事实、决策结论、状态真相（面向知识留存）。
6. 若当前项目尚无对应 local skill，先在 `.agents/skills/` 新建后再沉淀。
7. 任务结束时必须回查相关 skills：若本次形成了可复用动作、反模式或验证口径，但 skills 里还没有，就补进去。
8. 推荐统一模板（写入 local skill）：
   - `触发信号`
   - `关键判断`
   - `可复用动作（最小闭环 3-4 步）`
   - `反模式/边界`
