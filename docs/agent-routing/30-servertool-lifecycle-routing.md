...
## stopless 生命周期
1. 当前 stopless 默认开启，默认注入三轮递进目标/过程/证据检查提示，默认次数 3；旧默认 `继续执行` 只作为 legacy exact-match 输入并在 Rust 中升级。
2. `/goal active` 时收到 `finish_reason=stop`：不自动续轮。
3. `/goal non-active` 时收到 `finish_reason=stop`：自动注入目标/过程/证据检查提示；任一项无证据必须调用工具，已完成/阻塞必须给证据。
4. 非 `/goal` 时收到 `finish_reason=stop`：自动注入目标/过程/证据检查提示；任一项无证据必须调用工具，已完成/阻塞必须给证据。
