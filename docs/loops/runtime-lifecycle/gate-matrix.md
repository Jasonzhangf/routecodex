...
| --- | --- |
| Owner | `scripts/install-release.sh`, `scripts/install-release-snapshot.mjs`, `scripts/verify-rcc-release-install.mjs`, `package.json`, `package-lock.json`. |
...
| Whitebox | `npm run verify:function-map-build-wiring`; version sync check for `package.json`, lock root, and lock package root; focused test for any touched script/helper. |
