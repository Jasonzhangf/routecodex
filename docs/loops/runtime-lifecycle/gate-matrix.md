...
| Blackbox | Managed `rcc start --snap`, `rcc start --restart --port <port>`, `rcc stop --port <port>`, or `rcc restart --port <port>` only with explicit human approval; then `/health`, expected port-group behavior, and process lifecycle log evidence. |
