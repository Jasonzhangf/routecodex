...
### 目标
在已物理删除错误本地 gRPC/LS 双实现的前提下，仅保留 `src/providers/core/runtime/windsurf-chat-provider.ts` 这一条本地实现，按“解析语义真源”重建 Windsurf provider：把真实上游返回正确解析为 cascade assistant tool call / tool result / semantic roundtrip，并将其直接接回当前 provider 的发送-收口主链；禁止恢复旧本地 gRPC/LS 主线，禁止新增第二套实现。

### 验收标准
1. 仓库内不存在本地 gRPC/LS 第二套活实现，Windsurf provider 只剩单一路径。
...
4. send/response parse 主链若恢复，必须直接建立在当前语义解析真源上；不得再引回本地 gRPC/LS、本地 session/workspace panel 初始化链。
5. 实机链路恢复后，5520 same-shape smoke 不再出现旧本地实现错误形态（如 `InitializeCascadePanelState`、本地 `grpc/index.js`、本地 LS/cascade 启动链）。
6. summary 必须明确说明：哪些测试先红、哪些修改点转绿、为何这些修改点是唯一正确真源。
...
6. **物理删除错误实现**：确认错误的旧实现、旧依赖、旧桥接必须物理移除，不能仅闲置。
...
#### Slice C：send / response parse 主链接回
- 恢复后的 send path 只能调用当前 semantic parser 真源
...
## 5. 风险与规避
1. **误把旧 transport 当参考真源**
   - 规避：只参考其“上游返回语义”，不恢复其本地 gRPC/LS 结构。
2. **只把测试跑绿，但主链仍走旧分支**
...
5. **删不干净，仓库残留第二套实现**
   - 规避：grep + build + targeted smoke 验证无本地 gRPC/LS 依赖残留。
...
### 第三优先：same-shape 实机 smoke（仅在 send path 恢复后）
- 用 5520 发同形请求
...
4. 再跑定向 jest，转绿。
5. 若 send path 已达到恢复条件，再把真实 send/response parse 主链直接接到当前 semantic parser。
...
## 8. 完成定义（DoD）
1. Windsurf provider 只剩单一路径；仓库内不再存在本地 gRPC/LS 第二套错误实现。
2. cascade assistant tool call / tool result / continuity / fail-fast 解析测试全部转绿。
3. send path 若恢复，必须直接建立在当前语义解析真源上。
4. temporary text carrier 不再作为主语义判断路径；剩余依赖已删除或明确退居过渡观察位并有后续清理计划。
5. 定向测试、类型检查、构建、全局安装、restart、same-shape smoke 全部通过并留证据。
6. summary 明确给出唯一性论证：为什么修改点只能落在 `windsurf-chat-provider.ts` 的解析/收口真源，而不能改 Host/Router、不能恢复旧本地 gRPC/LS、不能保留双实现。
