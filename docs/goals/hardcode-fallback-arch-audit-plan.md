...
2. `provider-failure-policy-impl.ts` `UNRECOVERABLE_CODE_SET` / `NETWORK_ERROR_CODE_SET` / `BLOCKING_RECOVERABLE_CODE_SET` 等 Set 的 `WINDSURF_*` / `DEEPSEEK_*` / `HTTP_*` 错误码迁到 `provider-error-catalog.ts` 作为唯一字典；删除 `windsurf.managed.*` / `windsurf. ws-pro-1` 等 `startsWith` 字符串硬编码。Windsurf / DeepSeek provider 差异只允许在 provider runtime 内。
3. `rust-core/crates/router-hotpath-napi/src/virtual_router_engine/health.rs` 的 `clear_windsurf_managed_persisted_503_family` 迁入按 `reason` 匹配的通用 persisted 503 family 清理逻辑，删 `windsurf.managed.` 前缀特判。
...
- 静默 catch 清理：`src/providers/core/runtime/{base-provider,windsurf-chat-provider,deepseek-file-upload,transport/*}.ts`、`src/server/handlers/responses-handler.ts`。
...
### Out of Scope
- 不改 Virtual Router 选路语义（除非 windsurf 特例删除导致需要调整）。
...
2. **Fail-fast + no fallback**：禁止 catch 后 `return null/false/undefined`；禁止 `// ignore` / `// best-effort` 但不触发报警。删除旧 fallback 路径必须连同 `isWindsurfStreamCanceled` 这类 `if` 特例一起移除（不保留兜底）。
3. **Provider 特例只能在 Provider runtime**：Hub Pipeline / Virtual Router / RequestExecutor 不得写 `windsurf.managed.` / `deepseek` / `qwen` 字符串前缀判断；改用 `providerFamily` / `runtimeKey` 抽象。
...
- `src/providers/core/runtime/windsurf-chat-provider.ts` —— `WINDSURF_*_URL` 常量（行 82-87）作为 provider contract 字符串保留在 `windsurf-provider-contract.ts`，不要散落到业务函数。
...
- 新增红测：`provider-failure-policy-impl.red.spec.ts`：当 `WINDSURF_CASCADE_NO_PROGRESS` 等迁移后，断言三个 Set 的 `has(...)` 行为不变。
- `pnpm run build:min` 通过。

### Phase 3：Provider key / runtime key 字符串硬编码清理（Hub/VR 边界）

**文件**：
- `src/server/runtime/http-server/executor/request-executor-pipeline-attempt.ts`（行 85-86）—— 删 `startsWith('windsurf.managed.')` 特判，改为 `target.providerFamily === 'windsurf' && target.compatibilityProfile === 'chat:windsurf'` 抽象判断。
- `src/server/runtime/http-server/request-executor.ts`（行 949-950）—— 同样改 provider family 抽象。
- `src/server/runtime/http-server/http-server-runtime-providers.ts`（行 214/233/303/432）—— 删 `startsWith('windsurf.')` 字符串，改为 runtime profile 字段判断（如 `runtimeProfile.providerFamily === 'windsurf'`）。
- `src/server/runtime/http-server/executor/provider-response-converter.ts`（行 910-921）—— `options.providerFamily === 'windsurf'` 已是抽象，但若 `bridge.windsurf_native_chat_direct` 阶段名仍带 windsurf 字面，加白名单审计即可，行为不强制改。
...
**Rust 侧**：
- `sharedmodule/llmswitch-core/rust-core/crates/router-hotpath-napi/src/virtual_router_engine/health.rs`（行 220-243）—— `clear_windsurf_managed_persisted_503_family` 改名为 `clear_persisted_503_family_for_provider`，循环逻辑保留但不再 `starts_with("windsurf.managed.")`，改用传入 `provider_key` 自身的 `reason.as_deref() == Some(PERSIST_REASON_HTTP_503_DAILY)` 判断；若有兄弟 provider key 通过同 family 联动，则 family 概念下沉到 `provider_family` 字段（新增 `provider_family: String` 到 health state）。
- `sharedmodule/llmswitch-core/rust-core/crates/router-hotpath-napi/src/virtual_router_engine/load_balancer.rs`（行 279）—— 测试字符串 `windsurf.ws-pro-{idx}.gpt-5.4-none` 改为 fixture 函数生成的别名，不写死 windsurf 前缀（仍在 `#[cfg(test)]` 内，仅做命名清理）。
- `sharedmodule/llmswitch-core/rust-core/crates/router-hotpath-napi/src/virtual_router_engine/provider_bootstrap.rs`（行 1685-1730）—— `"baseURL": "https://server.self-serve.windsurf.com"` 测试 fixture 保持 `#[cfg(test)]` 标记；用 `bootstrap_test_fixture` 常量统一拼装。
...
- 新增红测：`http-server-runtime-providers.red.spec.ts` —— 当 `runtimeKey` 不以 `windsurf.` 开头但 `providerFamily === 'windsurf'`，旧代码会漏处理；新代码命中。
...
- `src/providers/core/runtime/windsurf-chat-provider.ts`（行 1932-1933）—— `// best-effort logging, never throw` 改为 throw + 上层 logger；其他 `// best-effort` 注释（行 1304/1832/2611/3288-3380/3731）按 `silent-failure-audit.mjs` 白名单保留，但日志必须显式 warn/error。
...
**文件**：
- `scripts/ci/silent-failure-audit.mjs` —— 增加 provider prefix 字符串白名单：禁止在 `src/server/**` / `sharedmodule/llmswitch-core/src/**` 出现 `windsurf\.managed\.` / `windsurf\.ws-pro-` / `windsurf-cascade` 这类运行期特判（`#[cfg(test)]` / `*.test.ts` / `*.spec.ts` 例外）。
...
1. **风险 1**：删 `windsurf.managed.` 字符串特判导致 runtime 漏处理 windsurf 503 family 联动。
   - **规避**：在 health.rs 新增 `provider_family` 字段（按 providerKey 前缀推断 family，保留"windsurf.managed" → "windsurf" 的归一规则），red test 覆盖 503 + 健康恢复路径。
...
5. **风险 5**：rust 侧 `clear_persisted_503_family_for_provider` 改名后旧测试引用断。
   - **规避**：先 `pub(crate) fn` 双名（保留旧 `pub(crate) fn clear_windsurf_managed_persisted_503_family` 转发到新函数），逐测试改完后再物理删除旧名。
...
- `pnpm jest tests/providers/core/runtime/windsurf-chat-provider*`
...
- `routecodex restart --port 5520` + 走 windsurf managed 路径观察 503 family cleanup 行为
...
3. **Step 3 — Phase 3 provider key 字符串清理**：
   ...
   - Rust：`health.rs` `clear_windsurf_managed_persisted_503_family` 改名 + 改抽象。
   - 跑 `cargo test health::tests` + `pnpm jest http-server-runtime-providers*`。
4. **Step 4 — Phase 4 静默 catch 清理**：
   - `base-provider.ts` / `windsurf-chat-provider.ts` / `responses-handler.ts` / `provider-http-executor-utils.ts` / `oauth-header-preflight.ts` 改显式 warn。
...
3. `http-server-runtime-providers.ts` / `request-executor.ts` / `request-executor-pipeline-attempt.ts` 无 `windsurf.managed.` / `windsurf.` 字符串特判；`health.rs` 无 `windsurf.managed.` 前缀硬编码（除 `#[cfg(test)]` fixture）。
4. `base-provider.ts` / `windsurf-chat-provider.ts` / `responses-handler.ts` 静默 catch 全部改为显式 warn，silent-failure-audit 命中数 < 当前基线。
...
7. `routecodex restart --port 5555` + `--port 5520` live smoke 验证 windsurf 503 family cleanup 行为不变。
