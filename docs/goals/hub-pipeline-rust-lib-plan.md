...
- 2026-06-04 correction: `servertoolRuntimeAction` stopless/stop gateway 判定位置必须是 `HubRespChatProcess03Governed` chat 标准态，不是 provider raw，也不是 client remap / `RespOutboundSseStream` 后的 payload。Rust effect 必须携带 chat-process payload；TS effect executor 缺 payload fail-fast，禁止 fallback 到 client payload。验证：`anthropic_end_turn_stopless_effect_uses_chatprocess_payload`、focused Jest、0.90.2787 5555 health + log `source=chat reason=finish_reason_stop eligible=true`。
...
- 2026-05-31 stage: runtime response-mappers import detachment。断开 `src/` runtime 对旧 TS `response-mappers.ts` 的类型/import 依赖；相关 stage/shell 改用 `JsonObject`/局部接口。Residue gate 新增扫描：除 `response-mappers.ts` 自身外，`sharedmodule/llmswitch-core/src/**/*.ts` 禁止出现 `response-mappers` import。验证：provider-response rust plan + residue + resp outbound monitoring 40/40 passed；Rust `hub_pipeline_lib` 7/7 passed；`sharedmodule/llmswitch-core npm run build` passed。
