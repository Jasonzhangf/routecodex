...
1. 从当前 main HEAD 创建干净 worktree，先合并独立 runtime worktree中已被验证的 runtime-bin/provider/router/server/config 接线，形成可复现基线。
...
| 类别 | 最小验证 |
|---|---|
| CLI 定向 | `cargo test -p routecodex-v4-cli --locked`；任意 cwd help/version/config check |
