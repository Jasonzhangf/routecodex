...
| `finish-reason.ts` visible-success/tool-call fallback | native finish reason derivation | TS fallback 已删除，只保留 native 委托 | 已改 | `deriveFinishReasonWithVisibleSuccessFallback(...)` 现在只返回 `deriveFinishReason(body)` | 验证：`finish-reason.spec.ts`、`finish-reason.visible-success.spec.ts`、residue audit |
