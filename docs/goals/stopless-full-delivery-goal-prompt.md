...
- stopless TS/native/HTTP 黑盒定向测试。
- rebuild + `routecodex restart --port 5555`。
- 5555 live probe：首轮 continue、恢复轮次再次 continue、allow-stop、budget exhausted、missing-schema budget、reset、`needs_user_input=true`、`stream=true/false` 对照。
