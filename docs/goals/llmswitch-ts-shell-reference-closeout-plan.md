...
### 2026-07-11 handler response-store integration root bridge mock removed

- `tests/server/handlers/handler-response-utils.responses-store-integration.spec.ts` no longer mocks or imports the root `src/modules/llmswitch/bridge.js` barrel and no longer declares legacy `importCoreDist` / `requireCoreDist` / `resolveImplForSubpath` / `resolveBaseDir` helpers.
- The spec now mocks only active leaf bridge surfaces used by the handler path: `responses-sse-bridge.js`, `responses-response-bridge.js`, and `runtime-integrations.js`. The remaining request-to-chat conversion assertions use the existing direct-native helper instead of the retired root bridge facade.
...
### 2026-07-11 core-loader dist subpath API and tool-mapping script reference closed

- Removed the now-unused `resolveCoreModulePath`, `resolveCoreModuleUrl`, and `importCoreModule` exports from `src/modules/llmswitch/core-loader.ts`; the host core loader now only owns `resolveCorePackageDir()` for active native bridge loading.
- `scripts/verify-apply-patch.mjs` no longer imports `dist/modules/llmswitch/core-loader.js` or the deleted/absent `conversion/shared/tool-canonicalizer` dist subpath. The verifier now consumes the active host native text normalizer and direct native apply_patch validator only.
- `sharedmodule/llmswitch-core/scripts/tests/apply-patch-freeform-tool-schema-passthrough.mjs` now calls Rust/NAPI `mapBridgeToolsToChatWithOptionsJson` / `mapBridgeToolsToChatJson` directly instead of falling back to deleted `src/conversion/shared/tool-mapping.ts`.
- Fixed the Rust apply_patch owner so artifact repair writes equivalent canonical `patch` and `input` values, and fixed Rust text harvesting so fenced structured apply_patch JSON is harvested by `normalize_assistant_text_to_tool_calls_json` without JS fallback.
- Exact source scans show no active `resolveCoreModulePath` / `resolveCoreModuleUrl` / `importCoreModule` consumers outside audit patterns and historical docs, and no active `conversion/shared/tool-mapping` consumers outside residue/red-test forbidden-path assertions.
- Verification passed: `npm run build:native-hotpath`; `npm run verify:apply-patch`; `npm run verify:apply-patch-freeform-contract`; strict shell reference audit (`prodTsShellCount=0`, `shellsWithProdImporters=0`, `shellsWithHostTextRefs=0`, `coreModuleSubpathRefs=2` note-only); `npm run verify:architecture-deleted-path`; `npm run verify:architecture-thin-wrapper-only`; `npm run verify:function-map-compile-gate`; Rust `apply_patch_arguments` tests; Rust `normalize_assistant_text_accepts_fenced_structured_apply_patch_payload`.

### 2026-07-11 VR bootstrap dist wrapper script refs closed

- Added `sharedmodule/llmswitch-core/scripts/helpers/virtual-router-bootstrap-direct-native.mjs` so llmswitch-core scripts can call Rust/NAPI `bootstrapVirtualRouterConfigJson` directly instead of importing the retired `dist/native/router-hotpath/native-virtual-router-bootstrap-config.js` wrapper.
- `virtual-router-pool-mode.mjs`, `virtual-router-capability-default-pool.mjs`, and `virtual-router-direct-model.mjs` now use the direct native bootstrap helper. The direct-engine helper now maps thrown native `VIRTUAL_ROUTER_ERROR:*` payloads into `VirtualRouterError`, so script assertions no longer depend on the retired wrapper's error projection.
- Deleted three stale standalone scripts (`hub-has-image-attachment-single-source.mjs`, `hub-normalize-process-mode-route-hint-single-source.mjs`, `hub-pipeline-history-media-placeholder.mjs`) after exact file-name scans found no active caller and focused execution proved they depended on the already-missing `dist/conversion/hub/pipeline/hub-pipeline.js` TS runtime surface.
- Exact source-tracked script scans now return zero active references to `native-virtual-router-bootstrap-config` and zero `scripts/tests` references to the deleted dist HubPipeline runtime surface.
- Verification passed: three focused VR script tests; helper syntax checks; strict shell reference audit (`prodTsShellCount=0`, `shellsWithProdImporters=0`, `shellsWithHostTextRefs=0`, `coreModuleSubpathRefs=2` note-only); `npm run verify:architecture-deleted-path`; `npm run verify:architecture-thin-wrapper-only`; `npm run verify:function-map-compile-gate`.

### 2026-07-11 llmswitch-core TS-like authoring surface closed

- Physically deleted remaining llmswitch-core declaration/type mirrors (`virtual-router-contracts.d.ts`, `servertool/types.d.ts`, `types/servertool-wrapper.d.ts`) and host bridge type-only mirrors (`bridge-types.ts`, `snapshot-recorder-types.ts`) after source-tracked exact scans found no active import/package consumers outside residue/docs/fixtures.
