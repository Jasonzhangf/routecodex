...
- **Provider 特例只能在 Provider runtime**：Hub Pipeline / Virtual Router / RequestExecutor 不得写 provider key 字符串特判。`health.rs` persisted 503 cleanup 必须保持通用 provider-family 语义。
...
- **Live smoke**：`routecodex restart --port 5555` + `--port 5520` 走至少两个真实 provider 路径，provider-family cooldown cleanup 行为不变
...
4. `base-provider.ts` / `responses-handler.ts` 静默 catch 全部改显式 warn。
...
4. **Phase 4 静默 catch 清理**：base-provider / responses-handler / oauth-header-preflight / provider-http-executor-utils 改显式 warn
