...
- Fix: deleted these helpers from `responses-response-bridge` / `responses-sse-bridge` TS plus checked-in `.js/.d.ts` mirrors and bridge index exports. `handler-response-utils.ts` now reads `forceSSE || result.sseStream !== undefined` and `options.responsesRequestContext` directly. `buildClientSseKeepaliveFrameForHttp` remains only on the dedicated SSE bridge facade path.
...
- Red evidence: focused `hub-pipeline-stage-residue-audit` failed because the SSE bridge/transport facade still exposed `updateResponsesContractProbeFromSseChunkForHttp` and imported native probe helpers; `handler-response-sse.ts` still kept TS `responsesSseBlockCarry` / `responsesContractProbe` / terminal booleans.
- Fix: added Rust/NAPI `updateResponsesSseTransportTerminalStateJson`; Rust now owns partial SSE block carry, probe update, and terminal observation. The TS handler stores only opaque transport state plus a boolean action result; the SSE bridge facade no longer imports or exports probe semantics.
...
- Boundary: `sharedmodule/llmswitch-core/src/sse/index.ts` must be a public barrel only. It must not own converter factories, default converter singletons, roundTrip helpers, or runtime registry indirection.
- Red evidence: source audit showed `sse/index.ts` still exported `createChatConverters`, `createResponsesConverters`, `createAnthropicConverters`, `createGeminiConverters`, `roundTrip()` helpers, and `chatConverters/responsesConverters/anthropicConverters/geminiConverters` default singletons. `sse-codec-registry.ts` depended on those singletons through the index barrel, creating a reverse public-surface dependency.
- Fix: `sse-codec-registry.ts` now imports concrete converter constructors directly from their owning modules and creates codec-local converter instances. `sse/index.ts` is reduced to converter/type/registry exports only. `HubPipeline` imports `defaultSseCodecRegistry` directly from the registry owner instead of the public barrel.
- Positive tests: codec registry still resolves `openai-chat` missing-model fail-fast and HubPipeline SSE protocol materialization still routes through `defaultSseCodecRegistry.get(protocol)` into the mocked codec path.
