...
2. 唯一允许进入 `ErrorErr06ClientProjected` 的停止条件是：
   `optionalEligibleCandidates == 0 && (defaultEligibleCandidates == 0 || lastDefaultAttemptBudgetExhausted == true)`；当前 V3 last-default budget 为同 provider 第 3 次连续失败。
3. `defaultPoolAvailable` 只能作为 VR/default-pool truth 的输入之一；Host / RequestExecutor / direct lane 只消费完整 `ErrorErr05ExecutionDecision`，不得只看 `defaultPoolAvailable` 本地合成停止条件或 default provider 链。
4. 每个 routing group 必须有显式非空 `routing.default` skeleton；default 最后一个 provider 不得被移出成空池。
5. `router-direct` 必须消费 `ErrorErr05ExecutionDecision`。当当前池已空但仍存在 default 候选且 last-default attempt budget 未耗尽时，必须继续执行决策链，禁止 rethrow。
...
1. relay/request-executor 主路径已接入 default-pool truth；V3 进一步要求 `ErrorErr05` 表达 `lastDefaultAttemptBudgetExhausted`，禁止只靠 `defaultPoolAvailable` 投影。
