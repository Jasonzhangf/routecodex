...
5. 支持 category 固定为：`global_error`、`session_storm`、`provider_recoverable`、`provider_transport`、`provider_traffic_saturated`、`servertool_followup`。
6. waiter cap 固定在统一队列内；超过 cap 显式抛 `PROVIDER_TRAFFIC_SATURATED`，不得吞异常。

并发/RPM 饱和规则：

- `provider-traffic-governor` 发现当前 runtime 满并发或 RPM 饱和时，必须先释放文件锁，记录 `provider_traffic_saturated`，阻塞等待统一队列一次。
- 醒来后重查容量；如果仍满，直接抛 `ProviderTrafficSaturatedError`，由上层错误链/Virtual Router 切 provider。priority mode 也不得死等当前 provider。
...
- provider failure delay projection：`provider-failure-policy-backoff.ts` 只产出固定 1/2/3 attempt delay
