...
- Global codex-review profile order is `codex -p asxs review`, then `codex -p tcm review` only if asxs is unavailable or lacks a final verdict. Stale `-p cc` skill examples are invalid for current review gates.
