...
## 2026-06-09 Servertool pending-session Rust owner
- Servertool pending-session file-name sanitization, pending max-age parsing, save payload coercion, and load stale/malformed/drop decisions are Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/servertool-core/src/pending_session_contract.rs::{resolve_pending_file_name,resolve_pending_max_age_ms,plan_pending_session_save,plan_pending_session_load}`.
- TS `sharedmodule/llmswitch-core/src/servertool/pending-session.ts` may only read env/session dir, perform file IO, and call native wrappers. It must not restore local segment sanitization, numeric parsing/defaulting, payload validation/coercion, stale-drop policy, or malformed-load drop decisions.
- Native bridge chain uses `native-servertool-core-semantics.ts::{resolvePendingSessionFileNameWithNative,resolvePendingSessionMaxAgeMsWithNative,planPendingSessionSaveWithNative,planPendingSessionLoadWithNative}` and required exports `resolvePendingSessionFileNameJson`, `resolvePendingSessionMaxAgeMsJson`, `planPendingSessionSaveJson`, `planPendingSessionLoadJson`.
- Verification note: if pending-session Jest reports native unavailable, rebuild native hotpath; do not add TS fallback. The failure means installed `.node` lacks the new Rust exports.
- Verified on 2026-06-09: Rust fmt check; `cargo test -p servertool-core pending_session --lib -- --nocapture` 3/3; `cargo test -p router-hotpath-napi pending_session --lib -- --nocapture` 2/2; `node sharedmodule/llmswitch-core/scripts/build-native-hotpath.mjs`; direct `.node` export probe for `resolvePendingSessionFileNameJson`, `resolvePendingSessionMaxAgeMsJson`, `planPendingSessionSaveJson`, and `planPendingSessionLoadJson`; `npm run verify:servertool-rust-only`; llmswitch-core/root tsc; focused pending-session Jest 5/5; `cargo test -p servertool-core --lib -- --nocapture` 176/176; `cargo test -p servertool-cli -- --nocapture` 8/8; and `git diff --check`.
Tags: servertool, pending-session, rust-owner, native-bridge, thin-shell, 2026-06-09
