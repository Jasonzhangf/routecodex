...
- SSE wrapper files `native-chat-sse-event-payload.ts`, `native-anthropic-sse-event-payload.ts`, and `native-gemini-sse-event-payload.ts` are not yet safe deletion targets despite zero prod importers, because function/verification maps and tests still treat their dist/source wrappers as verification owners. First migrate those gates/tests to Rust NAPI direct evidence, then delete.
