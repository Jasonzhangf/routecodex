...
## 2026-07-28 - V3 OpenAI Chat SSE tool-call identity contract
- OpenAI Chat-compatible SSE provider `tool_calls[].index` is a delta-stream cursor, not a globally unique call identity. ProviderRespCompat02 must preserve encounter order and resolve explicit identity by `(wire index, non-empty call id)`: a new ID on a reused index creates a new call; an already-seen ID on that index resumes its original call; an id-less delta appends to the currently active call for that index. The same ID on different indices remains distinct so the downstream duplicate-identity validator can reject it at its owning node.
- Never repair this failure by truncating malformed `}{` arguments, mapping the downstream 400, or adding a provider-specific branch. The corruption source is the OpenAI Chat SSE accumulator in `v3/crates/routecodex-v3-runtime/src/hub_v1/responses_relay_runtime.rs` at `V3ProviderRespInbound01Raw -> ProviderRespCompat02ProviderCompat`.
- Live regression shape is proven by 5555 sample `openai-responses-router-glm-5.2-20260728T073341848-654001-736`: upstream reused index 0 for two call IDs and used index 1 for a third; installed V3 returned exactly three unique calls with independently valid JSON arguments, no concatenation, no provider switch, and HTTP 200 `requires_action/tool_calls`.
