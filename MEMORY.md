...
- 2026-06-08: Anthropic response runtime closeout: `sharedmodule/llmswitch-core/src/conversion/hub/response/response-runtime-anthropic.ts::buildOpenAIChatFromAnthropicMessage` is now a native shell over Rust `hub_resp_outbound_client_semantics_blocks::anthropic_chat_response::build_openai_chat_from_anthropic_message_full`. TS must not restore Responses reasoning/payload snapshots/passthrough/semantics for this path; `tests/sharedmodule/hub-pipeline-stage-residue-audit.spec.ts` gates this by banning `responses-reasoning-registry`, `buildChatResponseFromResponsesWithNative`, and TS restore helpers in that runtime.

- 2026-06-08: Servertool stop-message routing state key is Rust-owned by `servertool-core::persisted_lookup::resolve_servertool_state_key` and exposed through native `resolveServertoolStateKeyJson`. TS `handlers/stop-message-auto/runtime-utils.ts::resolveStateKey` may only merge metadata and call `resolveServertoolStateKeyWithNative`; do not restore TS ordering for continuation `request_chain`, stop-message session scope, requestId, or `default`. `verify-servertool-rust-only` gates this owner.
