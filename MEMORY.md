...
- 2026-06-08: 5555 image payload preservation root cause was capability materialization, not config. Rust VR target metadata previously exposed only `supportsMultimodal`; vision-only providers therefore looked non-visual to req_outbound and latest user images could be placeholdered. Durable rule: `supportsMultimodal=false` does not imply non-visual; check `supportsVision` before stripping latest media. Routing must prefer multimodal/vision-capable targets for image requests, and image placeholdering is allowed only when no multimodal target and no vision route exists. Verification: `cargo test -p router-hotpath-napi image_request --lib -- --nocapture` (3 tests), `cargo test -p router-hotpath-napi image_attachment --lib -- --nocapture` (5 tests), native hotpath build, root `tsc --noEmit`.
