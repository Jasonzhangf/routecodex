...
- 2026-06-27: provider failure reroute blackbox 已收口为“替代 provider 真接管 + 旧 provider 只失败一次 + 无本地 wait/backoff”三件套；`blockingRecoverable` 这类内部壳字段不能再作为验收目标。验证通过 `npx tsc -p tsconfig.json --noEmit --pretty false` 和 `npx jest --runInBand tests/server/runtime/http-server/request-executor.spec.ts tests/providers/core/runtime/provider-failure-policy.spec.ts tests/server/runtime/http-server/executor/request-executor-provider-failure-plan.spec.ts`。

- 2026-06-27: provider failure reroute / retry 收口已验证通过。`request-executor.ts` 不再记录 provider-error scoped global backoff waits，`blockingRecoverable` 相关的残余壳链已从 executor / telemetry / direct decision 消费面清掉。黑盒新增了“替代 provider 真实接管成功、旧 provider 只打一次、成功路径无 `provider.route_pool_cooldown_wait` / `server.global_error_backoff_wait`”的断言。验证：`npx tsc -p tsconfig.json --noEmit --pretty false` 与 `npx jest --runInBand tests/server/runtime/http-server/request-executor.spec.ts tests/server/runtime/http-server/request-executor.no-response-regression.spec.ts tests/server/runtime/http-server/provider-direct-pipeline.candidate-exhaustion.spec.ts tests/server/runtime/http-server/router-direct-pipeline.candidate-exhaustion.spec.ts`。

- 2026-06-27: provider failure reroute blackbox slice confirmed that provider errors should cut to an alternative provider without inserting local wait/backoff on the reroute path. `request-executor.ts` no longer records provider-error scoped global backoff waits after send/resolve failures, and the existing 429 reroute blackbox now asserts that `provider.route_pool_cooldown_wait` and `server.global_error_backoff_wait` do not appear on the successful reroute path. Verified by `npx tsc -p tsconfig.json --noEmit --pretty false` and `npx jest --runInBand tests/server/runtime/http-server/request-executor.spec.ts`.
...
- 2026-06-22: `servertool/server-side-tools-impl.ts` 的 `bindServertoolContractWithNative<T>(value: T): T` identity wrapper 已物理删除，并从 `server-side-tools.ts` 的导入/导出链路移除。当前真实仓库消费只剩测试 mock 占位，不再有 runtime owner。验证通过 `server-side-tools.cli-projection-guard`、`server-side-tools.failfast`、`servertool-active-orchestration-audit`、`servertool-cli-native-bridge`、`execution-shell.auto-hook-failfast`、`verify-servertool-rust-only` 和 `build-core`。

- 2026-06-22: `servertool/server-side-tools.ts` 里的 `bindResponseStageGateNativeShell(...)` 死壳已物理删除，连带删掉其 `JsonObject` / `respStageGateNative` import。`verify-servertool-rust-only.mjs` 也同步从“要求 alias 存在”改成“禁止该 wrapper 复活”。验证通过 `server-side-tools.cli-projection-guard`、`servertool-active-orchestration-audit`、`servertool-cli-native-bridge`、`execution-shell.auto-hook-failfast`、`verify-servertool-rust-only` 和 `build-core`。

- 2026-06-22: `servertool/server-side-tools-impl.ts` 里的纯 alias wrapper `runServertoolAutoHookCallerViaImplThinShell(...)` 已物理删除，`runServertoolAutoHookCallerImpl` 直接指向 `runServertoolAutoHookCallerViaThinShell`。`scripts/verify-servertool-rust-only.mjs` 也已同步从“要求旧 wrapper 调用”改成“禁止 wrapper 复活”。验证通过 `verify-servertool-rust-only` 与 `build-core`。

- 2026-06-22: servertool request-side / response-side hook rewriting is a paired boundary across continuation, not two independent rewrites. Response side projects `ServertoolRespHook03HookResponseInjected -> client-visible exec_command`; request side restores the ordinary tool result back into built-in tool shape in `ServertoolReqHook01ResultParsed -> 02 -> 03 -> 04` before `HubReqChatProcess03Governed`. Responses continuation owner only consumes the finalized request shape after `ServertoolReqHook04RequestFinalized`, and must not reconcile shell projection into built-in tool shape itself. Verified by `tests/servertool/stopless-cli-continuation.spec.ts` and `tests/servertool/servertool-cli-result-restore.spec.ts` PASS.

- 2026-06-22: `hasServertoolCliProjectionContext` 这个废弃兼容字段已被物理删除：Rust `engine_runtime_action_contract.rs` 不再持有该字段，TS `native-servertool-core-semantics.ts` 不再序列化该字段，`servertool-cli-native-bridge.spec.ts` 不再显式传该字段。当前 cli projection runtime action 只依赖 `executionContext.servertoolCliProjection` 推导。验证通过 `build-native-hotpath`、focused Jest 38/38、`verify-servertool-rust-only` 和 `build-core`。

- 2026-06-22: `engine.ts` 的 `hasServertoolCliProjectionContext: false` 已从主线调用删除，Rust `engine_runtime_action_contract` 通过 `#[serde(default)]` 保持兼容输入；native wrapper 的同字段改为可选。验证通过 `build-native-hotpath`、focused Jest（`servertool-cli-native-bridge` / `engine.stopless-session-thin-shell` / `servertool-active-orchestration-audit` / `execution-shell.auto-hook-failfast`）、`verify-servertool-rust-only` 和 `build-core`。可复用规则：TS 不要继续自写“能力不存在”的硬编码，兼容默认值应由 Rust contract 承接。

- 2026-06-22: `sharedmodule/llmswitch-core/src/servertool/auto-hook-caller.ts` 里的纯命名壳 `runServertoolAutoHookCallerViaThinShell` 已改成直接导出 `runServertoolAutoHookCaller`，`server-side-tools-impl.ts` 和相关 tests/gate 同步改名。验证通过 `servertool-auto-hook-trace`、`execution-shell.auto-hook-failfast`、`server-side-tools.cli-projection-guard`、`server-side-tools.auto-hook-caller-guard`、`servertool-active-orchestration-audit`、`verify-servertool-rust-only` 和 `build-core`。可复用规则：对纯命名 alias，先保留唯一真函数名，再让 gate 反向锁死旧名。

- 2026-06-22: `execution-dispatch-outcome-shell.ts` 里的 `buildServertoolDispatchPlanInputThinShell` / `buildServertoolOutcomePlanInputThinShell` 已改成直接导出 `buildServertoolDispatchPlanInput` / `buildServertoolOutcomePlanInput`。这两个函数仍是必要 bridge，但 `ThinShell` 只是历史命名壳；调用点、focused Jest 和 `verify-servertool-rust-only` 已同步改到直接名。验证通过 `server-side-tools.dispatch-native`、`server-side-tools.failfast`、`servertool-active-orchestration-audit`、`verify-servertool-rust-only` 和 `build-core`。可复用规则：bridge 仍需要时删的是“命名壳”，不是盲删 bridge 本体。
...
- 2026-06-19: Historical `__routecodex_stream_*` test/fixture residues are removed from server handler/runtime tests and conversion-matrix fixtures. Standard stream semantics are now modeled only as `PipelineExecutionResult.sseStream` plus standard Responses SSE frames (`response.required_action`, `response.output_item.done`, `response.completed`, `[DONE]`) or standard JSON response bodies with `output` / `required_action`. `handler-response-sse-wrapper-contract.spec.ts` statically gates `__routecodex_stream_contract_probe_body` and `__routecodex_stream_finish_reason` against reappearing in server tests/runtime tests/conversion-matrix fixtures. Production fix: force-SSE JSON responses without a result-level stream now enter the standard JSON-to-SSE bridge instead of falling through to missing-stream handling. Verified by focused handler Jest 34/34, `tsc --noEmit --skipLibCheck`, `audit:custom-payload-carriers`, owner-queryability, runtime-manifest, containment, `verify:function-map-compile-gate`, `rg "__routecodex_stream"` showing only the deny gate script, and `git diff --check`. Remaining separate lane: legacy `__sse_responses` residues still exist in tests/scripts/docs with runtime files=0.

- 2026-06-18: Stopless/servertool followup runtime control is now partially migrated to MetadataCenter. `stop-message-auto.ts` writes `runtime_control.stopless` through the bound request-local `MetadataCenter` side-channel and fails fast if the binding is absent; `runtime-metadata.ts::ensureRuntimeMetadata(...)` preserves the `Symbol.for('routecodex.metadataCenter')` binding across native metadata normalization. Rust followup readers in `chat_node_result_semantics.rs` and `chat_servertool_orchestration.rs` now use `runtime_control.serverToolFollowup` / `runtime_control.serverToolFollowupSource` instead of legacy flat metadata, `requestSemantics.__routecodex`, or `__rt` for stopless/followup control activation. Negative tests prove legacy flat/`__rt` markers no longer activate followup tool merge or reasoning-stop suppression. Verified by focused Rust/Jest, metadata carrier audits, metadata-center manifest/code sync, function-map gate, full `verify:architecture-ci`, root `tsc`, `build:min`, and live 5555 `/v1/responses` sample scan with provider request/response free of internal control carriers. Remaining partial lane: request-route `retryProviderKey` / `preselectedRoute` writer migration is not closed by this slice.

- 2026-06-18: Hub Pipeline architecture review surface closure is now build/CI/install-gated. `verify:architecture-review-surface` covers mainline call map, node-id consistency, pending-binding budget, wiki Markdown sync, wiki HTML sync, metadata manifest sync, metadata-center code sync, mainline manifest sync, topology doc sync, and browser smoke over 14 HTML pages. `verify:function-map-compile-gate` covers 87 function-map features, source anchors, verification-map coverage, required tests/gates, owner uniqueness, and build wiring. `verify:architecture-ci-longtail` locks deleted-path non-resurrection, duplicate owners, TS owner ban, build tiering, custom payload carrier containment, owner-queryability, and runtime manifest. Current installed runtime proof is `routecodex/rcc 0.90.3131`, snapshot `routecodex-0.90.3131-2026-06-18T074631Z`, and health ready on ports `5555` and `5520`. Important boundary: this closure does not mean `__routecodex*` runtime is fully migrated; current manifest baseline still has 25 allowlisted runtime files, including 10 `payload_side_channel` files that must be migrated later to MetadataCenter/runtime side-channel without payload semantic cropping.

- 2026-06-18: MetadataCenter manifest must be code-bound, not documentation-only. `docs/architecture/metadata-center-manifest.yml` family/slot declarations are now gated by `verify:architecture-metadata-center-manifest-code-sync`, which checks `MetadataCenter` types, state buckets, writers, readers, and release coverage. `client_attachment_scope` and `debug_snapshot` are implemented as separate families and must not be used to synthesize request truth. The gate is wired into `verify:architecture-review-surface-light` and protected by `verify-function-map-build-wiring`. Verified by the new manifest/code gate, focused request-truth Jest 8/8, `verify:architecture-review-surface-light`, `verify:function-map-compile-gate`, root `tsc`, and `git diff --check`.

- 2026-06-18: Host/executor-side `MetadataCenterWriter.stage` is a local observation label, not a pipeline canonical node-id owner surface. Reusing canonical node ids such as `VrRoute04SelectedTarget` inside forbidden host paths (for example `src/server/runtime/http-server/executor/request-executor-pipeline-attempt.ts`) is treated as architecture owner leakage by `verify:architecture-forbidden-path-growth`. Keep canonical node ids in function-map/mainline/wiki/contract truth only; local MetadataCenter writer tags in host/executor must use local labels such as `request_executor_pipeline_target_observation`. Verified by `npm run verify:architecture-forbidden-path-growth`, `npm run verify:architecture-ci`, and `git diff --check`.

- 2026-06-18: MetadataCenter phase-1 request-truth executor followup is now locked through `request-executor-request-state.ts` and `goal-state-persistence.ts`. Request log color/session binding and stopless goal-state persistence must read `sessionId/conversationId` only from `MetadataCenter.request_truth` via `readRuntimeRequestTruthIdentifiers(...)`; flat `metadata.sessionId/session_id/conversationId/conversation_id` no longer count as request truth and must not be backfilled. Negative rule is also locked: if request truth is absent and there is no explicit inject scope, these paths must not synthesize or persist session scope from flat metadata. Verified by focused Jest (`request-executor-request-state.spec.ts`, `goal-state-persistence.spec.ts`, plus 7 request-truth regression suites), `npm run build:min`, global install to `routecodex 0.90.3100`, `routecodex restart --port 5555`, health ready on `http://127.0.0.1:5555/health`, and live replay `node scripts/tests/stopless-5555-live-probe.mjs` with first turn `requires_action + exec_command`, resumed `submit_tool_outputs` `completed`, and final `finalStatus=completed`.

- 2026-06-16: Stopless followup must route to `thinking` is locked. VR classifier now reads `serverToolFollowup` from request `metadata` and promotes that turn into thinking eligibility so stopless CLI continuation cannot drift to `tools` after routeHint stripping. Source: `sharedmodule/llmswitch-core/rust-core/crates/router-hotpath-napi/src/virtual_router_engine/classifier.rs`. Verified by `cargo test -p router-hotpath-napi --lib -- 'virtual_router_engine::classifier::tests::' --nocapture`, `tests/servertool/stopless-vr-route-hint.spec.ts`, root `tsc`, install/restart to `routecodex/rcc 0.90.3074`, health PASS on `5555/5520/10000`, and local continuity proof with `CODEX_THREAD_ID=stopless-vr-fresh-3074` showing first/middle/final prompts then `stopless budget exhausted` on the fourth call; persisted file `~/.rcc/state/routing/session-codex_stopless-vr-fresh-3074.json` recorded `stopMessageUsed=3`.

- 2026-06-14: Stopless double-收口 is locked. (1) Routing state key is strictly `sessionId` via `resolveStateKey` in `sharedmodule/llmswitch-core/src/servertool/handlers/stop-message-auto/runtime-utils.ts`; `tmuxSessionId` / `conversationId` / `stopMessageClientInjectScope` are no longer accepted as fallback. (2) stopless is client-transparent: it never projects a `stop_message_auto` / `exec_command` / `routecodex servertool run` shell; the model only sees a normal `user` role follow-up message through `reenterPipeline`. (3) `StoplessOrchestrationAction` is collapsed to `terminal_final` | `followup_mainline`; `cli_projection` is physically removed from `servertool-core::stopless_orchestration_contract`, `router-hotpath-napi::servertool_core_blocks`, `router-hotpath-napi::lib`, `cli_contract.rs` (with `StopMessageCliProjectionSeedInput` / `plan_stop_message_cli_projection_seed` and 6 dead seed tests), and all TS wrappers (`StopMessageCliProjectionSeed*`, `planStopMessageCliProjectionSeedJson`, `planStopMessageCliProjectionSeedWithNative`, `buildStopMessageCliProjectionResult`, `planStopMessageCliProjectionSeedWithNative` import in `servertool/engine.ts`). (4) `tests/servertool/stop-message-auto.spec.ts` is physically removed and replaced by `tests/servertool/stopless-sessionid-transparent.spec.ts` (5/5 PASS) which locks: `resolveStateKey` sessionId-only, reenterPipeline as normal user input, `result.chat` never contains `exec_command` / `stop_message_auto` / `routecodex servertool run`, different `sessionId` do not share state, and nested rounds stay transparent. (5) Generic CLI projection (`hub.servertool_cli_projection` / `servertool_fixture`) is kept intact and unrelated. (6) Gates: `cargo test -p servertool-core stopless --lib -- --nocapture` (29 PASS), `cargo test -p servertool-core persisted_lookup --lib -- --nocapture` (37 PASS), `cargo test -p router-hotpath-napi --lib` (compile clean), `verify:servertool-rust-only` ALL PASS, `verify:function-map-compile-gate` ALL PASS, llmswitch-core tsc clean, all servertool TS tests 83/83 PASS, `git diff --check` clean. Live install/restart/live `/v1/responses` probe is the next step before claiming closure.

- 2026-06-14: Direct-path error SSOT correction is locked. The only policy center remains `Virtual Router policy + ProviderFailurePolicy + request-executor error action queue`; do not restore a second ErrorHandlingCenter. `router-direct` / `provider-direct` may keep payload/response passthrough, but provider execution errors must return to the unified ErrorErr05 decision path and may become client-visible only after candidate/policy exhaustion. `client_disconnect` (including upstream `HTTP_499` + `client abort request`) must be classified in `src/providers/core/runtime/provider-failure-policy-impl.ts` before projection, with `affectsHealth=false`, no cooldown/health penalty, and no provider-visible 4xx projection. Any `secondary/default pool` expansion must be explicitly modeled in VR; host/http-server/RequestExecutor local fallback remains forbidden.

# RouteCodex Memory

- 2026-06-12: Responses continuation history image lifecycle rule is now locked at the store release boundary. `captureResponsesRequestContext` / in-flight request state must preserve full current+historical image/media payloads for the active send and any same-attempt retry/error analysis; only `sharedmodule/llmswitch-core/src/conversion/shared/responses-conversation-store.ts::releaseRequestPayload()` may rewrite persisted history. The actual scrub logic is Rust-owned by `chat_process_media_semantics::strip_responses_stored_context_input_media` and exported as native `stripResponsesStoredContextInputMediaJson`; TS store code is a thin bridge only. Verified runtime truth: pre-release materialized continuation still contains raw historical image, post-release materialized continuation replaces historical image with `[Image omitted]` while preserving non-media text and assistant output.

- 2026-06-09: Error module contract is now function-map/gate owned by `error.pipeline_contract`, `error.execution_decision_consumer`, and `error.client_projection` in addition to existing `error.provider_failure_policy` and `error.backoff_action_queue`. Provider-local `autoRetry` is physically removed: `src/providers/core/runtime/auto-retry-error-codes.ts` is deleted, runtime/profile `autoRetry` fields are gone, and `BaseProvider` may only call `sendRequestInternal(processedRequest)` once before failures enter awaited `handleRequestError -> emitProviderErrorAndWait -> ErrorErr01-06` and rethrow the original error. `ResponsesProvider` and startup init also use awaited provider error reporting; production `src` paths are gated from calling fire-and-forget `emitProviderError` outside the ErrorErr02 owner. Provider-direct now matches router-direct with an awaited ErrorErr hook and HTTP server `source: 'provider-direct'` reporting through `resolveRequestExecutorProviderFailurePlan`, while preserving direct payload passthrough and rethrowing original errors. Gate baseline: `npm run verify:error-pipeline-contract`, `npm run verify:function-map-compile-gate`, `npm run verify:architecture-error-chain-bypass`, focused direct/error/provider catalog Jest, and `npm run verify:provider-failure-ban-blackbox` all passed. Do not restore `[provider.autoRetry]`, provider-local retry, request-path fire-and-forget provider reporting, raw router-policy event construction, or ErrorHandlingCenter provider-policy dependencies.

- 2026-06-09: Release install/runtime truth for RouteCodex CLI is `~/.rcc/install/current` (this machine resolves to `/Volumes/extension/.rcc/install/current` because `~/.rcc` is a symlink). `scripts/install-release.sh` is the only approved release installer: it builds in an isolated temp tree, installs an immutable snapshot under `install/releases/<releaseId>`, rewrites `~/.local/bin/{rcc,routecodex}` to prefer the current snapshot, restarts the server, and requires `/health` to return `status=ok ready=true pipelineReady=true`. Node runtime must satisfy `>=20 <26`; this machine's default `node` is v26 and release install was verified with `/opt/homebrew/opt/node@22/bin/node` (v22.22.2). `install-manifest.json` must carry stable release metadata (`releaseId`, `sourceRepoRoot`, `buildRepoRoot`, `installRoot`, `distCli`) and `sourceRepoRoot` must point to the real repo root, not the isolated temp build root. Verification: real `scripts/install-release.sh` on 2026-06-09 installed `routecodex-0.90.3044-2026-06-09T143731Z`, `routecodex restart --port 5520` succeeded from snapshot dist, and `/health` returned `{"status":"ok","ready":true,"pipelineReady":true,"server":"routecodex","version":"0.90.3044"}`.

- 2026-06-09: Error storm backoff is unified under host-side `src/server/runtime/http-server/executor/request-executor-error-action-queue.ts` (`feature_id: error.backoff_action_queue`). The only storm-prevention wait cycle is blocking `1s -> 2s -> 3s -> repeat`, with categories `global_error`, `session_storm`, `provider_recoverable`, `provider_transport`, `provider_traffic_saturated`, and `servertool_followup`; hooks are `record` / `wait_start` / `wait_end`, waiter cap is centralized, and per-call/env/jitter/Retry-After/exponential/soft-wait loops must stay absent. Provider HTTP local retry deps (`getHttpRetryLimit`, `shouldRetryHttpError`, `delayBeforeHttpRetry`) were removed; provider HTTP failures return to the upper ErrorErr chain. Capacity errors (`429` / `503` / `PROVIDER_TRAFFIC_SATURATED`) wait through the queue then reroute/exclude; ordinary recoverable transport/5xx may same-provider retry once through the queue, then reroute or fail-fast. Direct wrapper consumes only recoverable direct retry plans and does not route direct 401 through the standard executor. Verification: residual backoff scan clean except docs/tests forbidden mentions; focused backoff/provider/direct/executor Jest stacks passed; server module help tests passed; `verify:function-map-compile-gate`, llmswitch-core/root tsc, Rust `failure_policy` tests, and `git diff --check` passed.

- 2026-06-09: Servertool backend-route vision eligibility is Rust-owned by `servertool-core::backend_route_contract::plan_vision_eligibility` and exposed through native `planVisionEligibilityJson` / `planVisionEligibilityWithNative`. TS `sharedmodule/llmswitch-core/src/servertool/handlers/vision-eligibility.ts` may only consume the native plan for `shouldRunVisionFlowForAdapterContext` and `shouldBypassStopMessageForMediaContext`; do not restore TS `containsImageAttachment`, runtime metadata walking, captured seed extraction, video URL regex, inline multimodal support checks, image-generation flag checks, routeHint/forceVision/hasVideoAttachment policy, or local media scanners. Verification: native rebuild, direct `.node` export probe, focused Rust/NAPI/Jest gates, `verify:servertool-rust-only`, llmswitch-core/root tsc, servertool-core 196/196, servertool-cli 8/8, `build:min`, `install:global`, `routecodex restart --port 5520`, health/models, and real `/v1/responses` completed on version `0.90.3032` with output `routecodex-e2e-3032`.

- 2026-06-09: Servertool backend-route followup error envelope / terminal classification and bootstrap preflight/replay policy are Rust-owned by `servertool-core::backend_route_contract::{plan_followup_error_envelope,plan_bootstrap_replay}` and exposed through native `planFollowupErrorEnvelopeJson` / `planBootstrapReplayJson`. TS `backend-route-reenter-block.ts` / `backend-route-bootstrap-replay-block.ts` may only call native plans, perform reenter/file IO, and construct existing error projection shells; do not restore TS `extractFollowupErrorEnvelope`, `isTerminalFollowupError`, `readPreflightStatus`, `buildReplayPayload`, local 4xx/400/429/code/message substring classification, or captured seed payload assembly. Verification: Rust focused tests, NAPI bridge focused tests, native build, direct `.node` export probe, `verify:servertool-rust-only`, llmswitch-core/root tsc, focused servertool Jest 5/5, servertool-core 173/173, servertool-cli 8/8, and `git diff --check`.

- 2026-06-09: Responses route-aware continuation old TS owner is deleted and must not be restored. Deleted paths: `sharedmodule/llmswitch-core/src/conversion/hub/pipeline/route-aware-responses-continuation.ts`, `tests/sharedmodule/route-aware-responses-continuation.spec.ts`, and ignored dist outputs `dist/conversion/hub/pipeline/route-aware-responses-continuation.{js,d.ts}`. Current continuation trigger truth is Rust `sharedmodule/llmswitch-core/rust-core/crates/router-hotpath-napi/src/hub_pipeline_blocks/responses_resume.rs`; store restore/materialize truth is Rust `sharedmodule/llmswitch-core/rust-core/crates/router-hotpath-napi/src/shared_responses_conversation_utils.rs`. Active docs must describe old TS route-aware continuation only as forbidden legacy. Verified with residue Jest 110/110, Rust `responses_resume` 10/10, llmswitch-core tsc/build, root tsc, bridge blackbox 10/10, and diff check.

- 2026-06-09: 5555 forwarder/VR empty-pool root cause and fix: router-direct protocol compatibility is a preference, not provider availability. Rust `selection.rs` now keeps original route candidates when no same-protocol target exists, and forwarder real-target scanning keeps cross-protocol available targets instead of consuming the whole pool. This preserves the invariant that a non-empty `default` pool cannot collapse to `PROVIDER_NOT_AVAILABLE` due to protocol filtering. Verified with focused Rust/Jest gates, global install `0.90.3023`, `routecodex restart --port 5555`, health `ready:true`, and live request `openai-responses-router-gpt-5.5-20260609T083545816-325158-7041` on `[port:5555 group:gateway_priority_5555]` selecting `default/gateway-priority-5555-priority-default -> 1token.key1.gpt-5.5`, completing `200 finish_reason=stop`, body `pong`.

- 2026-06-09: Hub Pipeline Phase 0 cleanup rule: stale matrix scripts importing deleted `dist/conversion/hub/pipeline/stages/req_inbound/req_inbound_stage3_context_capture/*` must be deleted, not migrated back to TS wrappers. Current request context capture and shell/tool-output normalization truth is Rust-owned by `hub_req_inbound_context_capture.rs` and `hub_req_inbound_tool_call_normalization.rs`; `run-matrix-ci.mjs` must only reference existing scripts, and `hub-pipeline-stage-residue-audit.spec.ts` gates these stale scripts as forbidden files.

- 2026-06-09: 5555 stopless live validation has a hard entry precondition. If `/v1/responses` is handled by router-mode `sameProtocolBehavior=direct`, fresh user probes can be classified as `thinking:user-input` and bypass stopless entirely via `router-direct:* -> 1token.*`; raw stop schema text then reaches the client unchanged. Treat these samples as invalid live-entry evidence, not stopless regressions. For stopless acceptance on 5555, first prove the request entered the relay/managed path from `server-5555.log`.

- 2026-06-09: Windsurf/Cascade provider has been removed from RouteCodex source. Deleted code/docs/tests/scripts/fixtures must stay absent; active source/package/test/script scans for `windsurf`/`cascade` should remain clean. Older Windsurf/Cascade MEMORY entries are historical only and must not be used to restore runtime, contract, probe, harness, compat profile, fixture, script, test, doc, or config entry without Jason's explicit new-provider authorization. Gate evidence: `verify:architecture-deleted-path` now covers the removed paths and `verify:architecture-provider-specific-leaks` still blocks provider-specific logic in Hub/VR owners.
...
- 2026-06-08: Servertool backend-route followup runtime action is Rust-owned by `servertool-core::backend_route_contract::plan_followup_runtime_action` and exposed through native `planFollowupRuntimeActionJson`. TS `backend-route-runtime-block.ts` may only consume the plan to perform IO side effects; do not restore TS decisions for loop payload source, auto-limit threshold/error contract, or client-inject metadata force/source. `verify-servertool-rust-only` gates this owner.

- 2026-06-08: Servertool backend-route followup runtime metadata is Rust-owned by `servertool-core::backend_route_contract::plan_followup_runtime_metadata` and exposed through native `planFollowupRuntimeMetadataJson`. TS `backend-route-runtime-block.ts::applyFollowupRuntimeMetadata` may only apply Rust-planned root/runtime metadata mutations; do not restore TS decisions for routeHint preservation/deletion, `__hubEntry`, stream forcing, original entry endpoint, or loop-state merge order.

- 2026-06-08: Legacy `review` servertool orchestration is removed. `tests/servertool/review-followup.spec.ts`, `servertool/handlers/review*.ts`, Rust skeleton `"review"`, `review_flow`, and `servertool.review` must stay absent; `verify-servertool-rust-only` gates this via `legacy-review-tool-deleted`. Older MEMORY entries mentioning `review_flow` as active are historical and superseded.

- 2026-06-08 correction: `empty_reply_continue` / empty-reply auto-followup is forbidden and physically removed. Empty assistant/empty response output is a malformed request/response contract case; fix the request/provider contract or fail fast, do not auto-continue or bypass via servertool. Deleted paths include `servertool-core::empty_reply_continue_contract`, `planEmptyReply*` native exports, `handlers/empty-reply-continue.ts`, `tests/servertool/gemini-empty-reply-continue.spec.ts`, and `servertool-empty-responses-continue.mjs`; `verify-servertool-rust-only` gates non-resurrection.

- 2026-06-08: 5555 provider/account auth/quota errors must enter the unified provider failure execution plan, not direct-return before reroute. `401/402/403` are still classified as current-provider `unrecoverable`, but ErrorErr05 must `exclude_and_reroute` when the standard executor route pool has an alternative; deterministic request/model/protocol errors such as `400`/`special_400` and `404` still fail fast and must not be hidden by provider switching. Fix owner is standard executor ErrorErr05 / provider-failure plan only. The older direct retry-metadata clause from commit `58c88f04a` is superseded by the direct correction above.

- 2026-06-08: `127.0.0.1:10000` can return `Empty reply from server` even when RouteCodex 10000 is healthy, because macOS may route loopback to a more specific third-party listener (`netdisk_service` on `127.0.0.1:10000`) while RouteCodex listens on `*.10000`. Validate with `netstat -anv -p tcp` and compare a non-loopback local IP such as `172.30.215.14:10000/health`; do not misdiagnose this as RouteCodex handler failure or restart-loop damage.

- 2026-06-08: 5555 image payload preservation root cause was req_outbound latest-turn media stripping based on only the currently selected target metadata, not config. Outbound does not know whether provider-switch can still reroute to a multimodal or vision target, so it must not placeholder current-turn chat/responses images. Durable rule: routing must prefer multimodal/vision-capable targets for image requests; current-turn image placeholdering is allowed only from explicit VR policy after proving no multimodal target and no vision route exists. Verification: `cargo test -p router-hotpath-napi image_request --lib -- --nocapture` (3 tests), `cargo test -p router-hotpath-napi test_preserve_latest --lib -- --nocapture` (4 tests), native hotpath build, root `tsc --noEmit`.
...
- 2026-06-08: 根目录生成物治理 Phase 1/2/3/4 的 root-layout contract 与代码状态已更新：pack/install tarball 输出统一到 `artifacts/pack/`，root `*.tgz` 和 `.install-pack/` 禁止；tracked legacy `package/`（qodercli residue）和 root `rcc` symlink 已物理删除；`webui/` 不得 local-exclude，必须作为源码纳入 root contract；根目录 tool/index/model state 迁到 `.agent-state/`、`.local-index/`、`.cache/model-cache/`。`repo-sanity` 现在扫描 ignored root entries，防止 ignored 散件绕过 root-layout gate；但当前完整 `repo-sanity` 不能作为全绿证据，因为它被无关 untracked VR 文件阻断。
  Evidence: `.gitignore:10` / `.gitignore:13` / `.gitignore:83` / `.gitignore:84`; `scripts/ci/repo-sanity.mjs:110` / `scripts/ci/repo-sanity.mjs:137` / `scripts/ci/repo-sanity.mjs:148` / `scripts/ci/repo-sanity.mjs:167`; `scripts/install-global.sh:228`; `package.json:37` / `package.json:78`; `docs/goals/root-generated-artifacts-governance-plan.md:17` / `docs/goals/root-generated-artifacts-governance-plan.md:18` / `docs/goals/root-generated-artifacts-governance-plan.md:19` / `docs/goals/root-generated-artifacts-governance-plan.md:131`; staged deletion evidence from `git diff --cached --name-status -- package rcc`.
  Verification: `rg -n "artifacts/pack|\\.agent-state|\\.local-index|\\.cache/model-cache|\\.install-pack|\\*.tgz|package/|webui/" .gitignore scripts/ci/repo-sanity.mjs scripts/install-global.sh package.json docs/goals/root-generated-artifacts-governance-plan.md` confirms the current root-layout contract and owner paths. `node scripts/ci/repo-sanity.mjs` currently fails on unrelated untracked `sharedmodule/llmswitch-core/src/native/router-hotpath/virtual-router-contracts.ts`, so do not cite this memory item as full repo-sanity closeout.

Tags: root-layout, generated-artifacts, repo-sanity, pack-output, local-state, webui-source, 2026-06-08

- 2026-06-08: 根目录生成物治理补充边界：tracked report 产物默认路径必须是 `docs/reports/`，不是 root `reports/`；repo-local debug log 可用 root `logs/<feature>/`，但 `.cache/` 只允许 `.cache/model-cache/`，`artifacts/` 只允许 `artifacts/pack/`。`scripts/tool-classification-report.ts` 与 `scripts/analyze-tools-fileops.mjs` 已从 root `reports/` 迁到 `docs/reports/`，`repo-sanity` 已加入旧模式与 scoped subroot gate。
  Verification: `rg -n "path\\.join\\(process\\.cwd\\(\\), 'reports'|path\\.resolve\\(process\\.cwd\\(\\), 'reports'" scripts src sharedmodule/llmswitch-core/src --glob '!**/*.d.ts' --glob '!**/*.js'` returns no matches; `find artifacts .cache logs -maxdepth 2 -mindepth 1` shows only `artifacts/pack` and `.cache/model-cache`; `npx tsc --noEmit --pretty false` and `git diff --check && git diff --cached --check` pass.

Tags: root-layout, reports, docs-reports, artifacts-pack, model-cache, repo-sanity, 2026-06-08
...
- 2026-06-09 supersession: the retry rule above is historical only. Provider internal auto-retry and `resolveAutoRetryErrorCode()` have been deleted; 2056 now belongs to provider business-error detection plus provider catalog / ErrorErr policy classification.
...
- 2026-06-09 correction: Required_action/tool_calls client-visible terminal contract is `response.required_action -> response.completed -> response.done -> [DONE]`. Codex client reports `stream closed before response.completed` when `response.completed` is missing even if `response.done` and `[DONE]` are present. The native terminal builder must dedupe each frame independently, not suppress `response.completed` for required_action.
...
- Prior 2026-06-07 verification that forbade `response.completed` for required_action is superseded by the 2026-06-09 live Codex client evidence and focused tests.
...
## 2026-06-08 ErrorErr06 public projection boundary
- ErrorErr06 client/log projection now sanitizes provider auth/quota/rate-limit payloads before client body or HTTP error logs: `mapErrorToHttp()` maps 429 to public `Rate limited by upstream provider`, 401/403 to public `Upstream authentication failed`, and `logRequestError()` builds `[http.error.meta]` from status/code/upstreamCode shell rather than raw provider token/quota text.
- Evidence: `src/server/utils/http-error-mapper.ts:118` / `src/server/utils/http-error-mapper.ts:176` / `src/server/utils/http-error-mapper.ts:186`; `src/server/handlers/handler-utils.ts:277` / `src/server/handlers/handler-utils.ts:303`; `tests/server/utils/http-error-mapper-public-leak.spec.ts`; `tests/server/handlers/request-error-log.spec.ts`.
- Verification: `npm run jest:run -- --runTestsByPath tests/server/handlers/request-error-log.spec.ts tests/server/handlers/handler-utils.error-response-shape.spec.ts tests/server/utils/http-error-mapper-public-leak.spec.ts tests/server/utils/http-error-mapper-provider-not-available-details.spec.ts tests/server/utils/http-error-mapper-timeout.spec.ts tests/server/utils/http-error-mapper-tool-validation.spec.ts tests/server/utils/http-error-mapper-malformed-request.spec.ts --runInBand --forceExit` PASS (7 suites / 18 tests).
Tags: error-chain, ErrorErr06, public-projection, auth-redaction, quota-redaction, 2026-06-08

## 2026-06-08 Virtual Router no-TS runtime current state
- `docs/goals/virtual-router-no-ts-runtime-plan.md` is stricter than the current thin-shell policy: final VR runtime owner must be Rust under `router-hotpath-napi/src/virtual_router_engine`, with no production TS files or production imports under `sharedmodule/llmswitch-core/src/router/virtual-router/**`.
- Current worktree has progressed past the old note baseline because `sharedmodule/llmswitch-core/src/router/virtual-router/engine-selection` no longer exists, but VR no-TS closeout is not complete: `find sharedmodule/llmswitch-core/src/router/virtual-router -type f -name '*.ts'` still returns 32 TS files, and production/test imports still reference VR TS paths and old engine-selection module paths.
- Evidence: `docs/goals/virtual-router-no-ts-runtime-plan.md:5` / `docs/goals/virtual-router-no-ts-runtime-plan.md:13` / `docs/goals/virtual-router-no-ts-runtime-plan.md:52`; current `find` output includes `bootstrap.ts`, `engine.ts`, `provider-registry.ts`, `routing-state-store.ts`, `routing-instructions.ts`, and state codec files; `rg "router/virtual-router/engine-selection|src/router/virtual-router"` still finds production/test/doc references.
- Verification: current focused Jest gates that depend on migrated native wrappers are not green: usage/direct-result group failed on missing `../router/virtual-router/engine-selection/native-shared-conversion-semantics.js`, and stopless focused request-executor tests failed on missing `native-chat-process-node-result-semantics`. Treat these as unclosed migration evidence, not as completed VR no-TS proof.
Tags: virtual-router, rustification, no-ts-runtime, engine-selection, migration-incomplete, 2026-06-08

## 2026-06-08 apply_patch freeform client/provider boundary
- Current `apply_patch` truth is not the older servertool/line-edit dialect: client-visible `/v1/responses` projection must convert apply_patch function-call frames into `custom_tool_call` with raw freeform patch input when the original client tool is `{type:"custom", name:"apply_patch", format:{type:"grammar"}}`, and live SSE must suppress JSON-wrapped argument deltas/done frames before writing to the client.
- Provider-side Anthropic wire history must still preserve the same freeform patch as structured provider input `{patch: rawPatch}`; raw freeform `function.arguments` must not collapse to empty `{}` provider `tool_use.input`.
- Evidence: `src/server/handlers/handler-response-utils.ts:1094` / `src/server/handlers/handler-response-utils.ts:1120` / `src/server/handlers/handler-response-utils.ts:1270` / `src/server/handlers/handler-response-utils.ts:1357`; `sharedmodule/llmswitch-core/rust-core/crates/router-hotpath-napi/src/anthropic_openai_codec.rs:282` / `sharedmodule/llmswitch-core/rust-core/crates/router-hotpath-napi/src/anthropic_openai_codec.rs:1711`; `sharedmodule/llmswitch-core/rust-core/crates/router-hotpath-napi/src/hub_resp_outbound_client_semantics_tests.rs:830`.
- Verification: `npm run jest:run -- --runTestsByPath tests/server/handlers/handler-response-utils.apply-patch-freeform-sse.spec.ts --runInBand --forceExit` PASS (1 suite / 6 tests); `cargo test -p router-hotpath-napi normalize_responses_tool_call_arguments_for_client_projects_freeform_apply_patch_as_raw_patch --lib` PASS; `cargo test -p router-hotpath-napi build_anthropic_from_openai_chat_wraps_freeform_apply_patch_history_as_patch_input --lib` PASS.
Tags: apply_patch, freeform, custom_tool_call, responses-sse, anthropic-wire, provider-history, 2026-06-08

## 2026-06-08 provider status 2013 classification split
- Provider status `2013` is not a single durable class. Current policy keeps context-overflow-style 2013 as health-neutral `special_400`, but `MALFORMED_RESPONSE + PROVIDER_STATUS_2013` traffic saturation text such as `Token Plan 当前请求量较高，请稍后重试` is `recoverable`, health-affecting, blocking-recoverable, and retryable.
- This entry proves classification truth only. The request-executor provider-failure-plan focused gate is red in the current worktree, so do not claim full retry-plan closeout from this memory item.
- Evidence: `src/providers/core/runtime/provider-failure-policy-impl.ts:85` / `src/providers/core/runtime/provider-failure-policy-impl.ts:328` / `src/providers/core/runtime/provider-failure-policy-impl.ts:418`; `tests/providers/core/runtime/provider-failure-policy.spec.ts:324` / `tests/providers/core/runtime/provider-failure-policy.spec.ts:425` / `tests/providers/core/runtime/provider-failure-policy.spec.ts:458`.
- Verification: `npm run jest:run -- --runTestsByPath tests/providers/core/runtime/provider-failure-policy.spec.ts --runInBand --forceExit` PASS (1 suite / 26 tests). RED gate retained as open evidence: `npm run jest:run -- --runTestsByPath tests/server/runtime/http-server/executor/request-executor-provider-failure-plan.spec.ts --runInBand --forceExit` failed 3 tests in current worktree, including special_400 empty-pool exclude behavior and 2013 `backoffScope` expectation.
Tags: provider-failure-policy, provider-status-2013, special_400, recoverable, health-affecting, retry-plan-gap, 2026-06-08

## 2026-06-08 Responses started-SSE error projection boundary
- `/v1/responses` stream intent is driven by `payload.stream`, not by the `Accept` header alone. If `stream=true` fails before headers are sent, the handler must return `text/event-stream` with `event:error`; if SSE headers were already sent, it must append `event:error` to the started stream instead of degrading to JSON.
- Evidence: `src/server/handlers/responses-handler.ts:211` / `src/server/handlers/responses-handler.ts:593` / `src/server/handlers/responses-handler.ts:594`; `src/server/handlers/handler-utils.ts:436` / `src/server/handlers/handler-utils.ts:483`; `tests/server/handlers/responses-handler.started-sse-error.spec.ts:21` / `tests/server/handlers/responses-handler.started-sse-error.spec.ts:59`.
- Verification: `npm run jest:run -- --runTestsByPath tests/server/handlers/responses-handler.started-sse-error.spec.ts --runInBand --forceExit` PASS (1 suite / 2 tests).
Tags: responses-sse, started-stream, error-projection, stream-intent, no-json-degrade, 2026-06-08

## 2026-06-08 provider runtime ingress native owner
- `provider-runtime-ingress` owner is native/Rust: TS callers must use `sharedmodule/llmswitch-core/src/native/router-hotpath/native-provider-runtime-ingress.ts`; the old TS hook fanout file under `src/router/virtual-router/provider-runtime-ingress.ts` is physically deleted and must not be restored.
- Superseded direct-path note: standard executor provider errors (429/5xx/524) may delegate to ErrorErr05 for switch/reroute, but direct/router-direct/provider-direct errors must not enter relay or the standard executor. Direct only reports provider errors to the unified error chain for health/telemetry hooks and then fail-fast projects the provider error. ErrorHandlingCenter remains ErrorErr06 projection only.
...
## 2026-06-08 servertool CLI projection Responses replay-safe shell
- Root cause: same-protocol direct 400 `input[n].content array too long` was caused by RouteCodex servertool CLI projection generating client-visible Responses history with `type=reasoning` and non-empty `content`; direct passthrough correctly forwarded the client history and must not sanitize or route to relay.
- Fix owner: Rust `servertool-core::cli_contract::build_client_visible_projection_shell` now owns the client-visible `exec_command` projection shell. TS `servertool/cli-projection.ts` is a thin native wrapper and no longer hand-builds `exec_command` tool_call / reasoning shape.
...
- Deterministic request/model errors such as `404` remain direct-return even if alternatives exist; do not mask request-shape/model-support bugs by provider switching. Superseded direct-path clause: router-direct does not delegate provider errors via retry metadata anymore; it remains same-protocol passthrough, reports provider errors for unified error-chain telemetry/health hooks, and fails fast without relay/executor reentry.
...
## 2026-06-09 Hub Pipeline stale matrix deletion rule
- If a matrix script imports deleted Hub Pipeline TS stage wrappers or asserts behavior no longer owned by the Rust contract, delete the stale script and move feature-map coverage to the current Rust/native owner; do not restore `runRespProcessStage1ToolGovernance` / deleted `resp_process` TS stage wrappers to satisfy old gates.
- Current apply_patch contract owner is Rust/native freeform grammar + schema normalization. `apply-patch-native-regression-matrix.mjs` covers native normalize/validate compatibility; old add/delete oscillation filtering and servertool `APPLY_PATCH_REQUIRES_READ_BEFORE_RETRY` followup-shape assertions are not current owner truth.
Tags: hub-pipeline, rustification, dead-code-cleanup, apply-patch, matrix-gates, no-ts-wrapper-resurrection, 2026-06-09

## 2026-06-09 Removed provider code cleanup verification
- Windsurf/Cascade repository cleanup is code-only unless explicitly authorized otherwise: do not stop workers/processes and do not edit runtime config to satisfy cleanup.
- Verified active non-doc code/package/script/test scope has no removed-provider identifiers after excluding generated/vendor/history scopes. Provider fixtures/tests must use generic providers; do not restore Windsurf provider factory/runtime/profile/auth tests.
- Error-chain cleanup rule discovered during this pass: executor error reporting must pass `outcome.recoverable` and `outcome.affectsHealth` directly from provider failure policy; do not add local `affectsHealthOverride` or terminal reroute naming/policy wrappers in executor.
- Verified gates: architecture deleted-path/provider-specific-leaks, focused provider/runtime setup Jest, error-chain red Jest, llmswitch-core tsc, root tsc, and diff check passed on 2026-06-09.
Tags: removed-provider, windsurf, cascade, no-resurrection, error-chain, provider-factory, 2026-06-09

## 2026-06-09 Req outbound compat matrix Rust-owner rule
- Active llmswitch-core matrix scripts must not import deleted `dist/conversion/hub/pipeline/stages/*` wrappers or the deleted old `dist/router/virtual-router/engine-selection/native-hub-pipeline-req-outbound-semantics.js` path; use `dist/native/router-hotpath/native-hub-pipeline-req-outbound-semantics.js` or delete the stale script.
- `chat:iflow` reasoning/thinking auto-repair and `chat:gemini-cli` compat assertions were old TS wrapper expectations, not current Rust `req_outbound_stage3_compat` contract. Current iflow request compat coverage is historical-media stripping via Rust generic `strip_historical_media`, with `[Image omitted]` / `[Video omitted]` placeholders and latest-turn media preserved.
- Verified gates: native compat matrix scripts, Hub residue audit, Rust `req_outbound_stage3_compat` tests, llmswitch-core tsc, root tsc, rustification audit, llmswitch-core build, and diff checks passed on 2026-06-09.
Tags: req_outbound_stage3_compat, hub-pipeline, rust-owner, matrix-gates, dead-code-cleanup, no-ts-wrapper-resurrection, 2026-06-09

## 2026-06-09 Hub matrix deleted dist-owner cleanup
- Active llmswitch-core matrix/scripts must not restore deleted dist owners such as `conversion/hub/node-support.js`, `conversion/hub/policy/policy-engine.js`, `conversion/hub/process/chat-process.js`, `conversion/hub/response/provider-response.js`, or `servertool/handlers/followup-request-builder.js`. If a script depends on those owners, migrate it to the current Rust/native owner or physically delete it and remove it from `run-matrix-ci.mjs`.
- Deletion evidence: old hub-policy enforce scripts, old servertool followup scripts, old web_search route/backend scripts, `servertool-followup-history-media-single-source.mjs`, and `coverage-hub-chat-process-entry-client-inject.mjs` were deleted; `loop-rt-gemini.mjs` now keeps only the live Gemini SSE roundtrip. `hub-pipeline-stage-residue-audit.spec.ts` blocks the old dist-owner imports and deleted script entrypoints.
Tags: hub-pipeline, matrix-gates, deleted-dist-owner, servertool, web-search, no-ts-wrapper-resurrection, 2026-06-09

## 2026-06-09 Servertool runtime stop-message state Rust owner
- `resolveStateKey` 的 state key 顺序、runtime `stopMessageState/serverToolLoopState` snapshot 读取、`stopMessageStageMode` normalize、以及 followup `serverToolLoopState.flowId` 读取已迁到 `sharedmodule/llmswitch-core/rust-core/crates/servertool-core/src/persisted_lookup.rs`。
- Native bridge 链为 `servertool_core_blocks.rs -> router-hotpath-napi/src/lib.rs -> native-servertool-core-semantics.ts`，新增 required exports: `resolveServertoolStateKeyJson`, `resolveRuntimeStopMessageStateJson`, `readRuntimeStopMessageStageModeJson`, `readServertoolFollowupFlowIdJson`。
- `sharedmodule/llmswitch-core/src/servertool/handlers/stop-message-auto/runtime-utils.ts` 中 `resolveStateKey`, `resolveRuntimeStopMessageState`, `readRuntimeStopMessageStageMode`, `readServerToolFollowupFlowId` 只能调用 native wrapper；禁止恢复 TS `continuationScope/stickyScope/requestId/default`、`serverToolLoopState/maxRepeats/flowId`、`stopMessageState/stopMessageText/继续执行` 语义。
- Gate: `npm run verify:servertool-rust-only` now blocks TS owner revival for these three functions and asserts Rust owner/native export presence.
- Verified: servertool-core state_key/runtime_stop/followup_flow focused tests, router-hotpath-napi state_key/runtime_stop/followup_flow focused tests, native build, `npm run verify:servertool-rust-only`, llmswitch-core tsc, root tsc, focused servertool Jest, servertool-core full lib, servertool-cli blackbox, and `git diff --check` all passed on 2026-06-09.
Tags: servertool, stop-message, runtime-state, rust-owner, native-bridge, 2026-06-09

## 2026-06-09 Servertool CLI-result stopless state Rust owner
- Stopless client `exec_command` CLI-result request-record scanning is Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/servertool-core/src/persisted_lookup.rs::resolve_runtime_stop_message_state_from_adapter_context`.
- Rust owns collection from `__raw_request_body` / `capturedEntryRequest` / `capturedChatRequest`, stop_message_auto CLI command seed parsing from POSIX single-quoted `--input-json`, call_id/tool_call_id/id seed matching, CLI output validation via Rust `cli_contract::validate_client_exec_command_result`, and `source: "client_exec_result"` snapshot construction.
- Native bridge chain adds required export `resolveRuntimeStopMessageStateFromAdapterContextJson`; TS `runtime-utils.ts::resolveRuntimeStopMessageStateFromAdapterContext` may only call `readRuntimeMetadata` then `resolveRuntimeStopMessageStateFromAdapterContextWithNative`.
- Gate: `npm run verify:servertool-rust-only` blocks TS revival of `parseStopMessageCliInputFromCommand`, POSIX quote decode, CLI command maps, request-record restoration helpers, `validateClientExecCommandResultWithNative` loops, and TS `client_exec_result` construction.
- Verified on 2026-06-09: servertool-core adapter_context/runtime_stop tests, router-hotpath-napi adapter_context test, native build and direct export load, `npm run verify:servertool-rust-only`, llmswitch-core/root tsc, focused stop-message + CLI/shadow Jest, servertool-core full lib, servertool-cli blackbox, and `git diff --check` all passed.
Tags: servertool, stopless, cli-result, runtime-state, rust-owner, native-bridge, 2026-06-09

## 2026-06-09 Servertool record metadata resolver Rust owner
- BD working directory resolution and stop-message followup provider key resolution are Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/servertool-core/src/persisted_lookup.rs::{resolve_bd_working_directory_for_record,resolve_stop_message_followup_provider_key}`.
- Rust owns request-record + runtime metadata traversal for these fields. Working directory preserves the old key-priority contract across locations: all `workdir` candidates first, then `cwd`, then `workingDirectory`; therefore runtime `workdir` can beat metadata `cwd`.
- Native bridge chain adds required exports `resolveBdWorkingDirectoryForRecordJson` and `resolveStopMessageFollowupProviderKeyJson`; TS `runtime-utils.ts::{resolveBdWorkingDirectoryForRecord,resolveStopMessageFollowupProviderKey}` may only call the native wrappers after `readRuntimeMetadata`.
- Gate: `npm run verify:servertool-rust-only` blocks TS revival of `readSessionScopeValue`, `readHubCaptureContextValue`, `readProviderKeyFromMetadata`, `toNonEmptyText`, and local TS semantic resolver bodies for these functions.
- Verified on 2026-06-09: native build and direct export load, `npm run verify:servertool-rust-only`, llmswitch-core/root tsc, focused stop-message + CLI/shadow Jest, servertool-core full lib 137/137, servertool-cli blackbox 8/8, and `git diff --check` all passed.
Tags: servertool, stop-message, metadata-resolver, working-directory, provider-key, rust-owner, native-bridge, 2026-06-09

## 2026-06-09 Servertool loop warning duplicate TS owner removed
- Stop-message loop warning text and repeat-count clamp policy are Rust/native-owned by `sharedmodule/llmswitch-core/rust-core/crates/followup-core/src/lib.rs::inject_loop_warning`, surfaced through `injectLoopWarningWithNative`.
- `sharedmodule/llmswitch-core/src/servertool/stop-message-loop-payload-block.ts` may only build the seed loop payload via `buildServertoolReq04FollowupPayloadWithNative`; it must not export or restore local `appendStopMessageLoopWarning`, `warningText`, repeat count parsing/clamping, or `messages.push` warning injection.
- `appendStopMessageLoopWarning` as a callback property name in backend-route client-inject/reenter blocks is not the semantic owner; the mainline callback must continue to call `injectLoopWarningWithNative`.
- Gate: `npm run verify:servertool-rust-only` locks this with `stop-message-loop-warning-no-ts-duplicate`.
- Verified on 2026-06-09: direct `rg` showed no old export use; `npm run verify:servertool-rust-only`; llmswitch-core/root tsc; `git diff --check`.
Tags: servertool, stop-message, loop-warning, dead-code, rust-owner, native-bridge, 2026-06-09

## 2026-06-09 Servertool orchestration policy Rust owner
- Servertool orchestration policy parsing is Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/servertool-core/src/orchestration_policy_contract.rs`.
- Rust owns the preserved contracts: servertool timeout env value parsing/flooring and invalid positive checks; `clientInjectOnly` boolean/string `true|false` parsing; followup text sanitization for stopmessage markers, `[Time/Date]`, `[Image omitted]`, whitespace and blank-line collapse; followup error reason compaction to `HTTP_<code>`, `UPSTREAM_HTML_ERROR`, or 220-byte truncated reason with `...`; adapter provider key resolution order `target.providerKey -> target.providerId -> targetProviderKey -> providerKey`.
- TS `sharedmodule/llmswitch-core/src/servertool/orchestration-policy-block.ts` may only read process env, accept metadata/value arguments, call native wrappers from `native-servertool-core-semantics.ts`, and keep `containsSyntheticRouteCodexControlText` on the existing native chat-process wrapper. It must not restore local `parseTimeoutMs`, `parseBooleanLike`, `sanitizeFollowupText`, regex compaction, providerKey walker, or stop-gateway wrapper.
- Native bridge chain adds required exports `parseServertoolTimeoutMsJson`, `readClientInjectOnlyJson`, `normalizeClientInjectTextJson`, `compactFollowupErrorReasonJson`, and `resolveAdapterContextProviderKeyJson`; direct `.node` probe confirmed all five exports.
- Gate: `npm run verify:servertool-rust-only` locks Rust owner/native bridge/TS thin-shell boundaries with `servertool-orchestration-policy-rust-owner`.
- Verified on 2026-06-09: `cargo fmt --package servertool-core --package router-hotpath-napi --check`; `cargo test -p servertool-core orchestration_policy --lib -- --nocapture` 5/5; `cargo test -p router-hotpath-napi plans_orchestration_policy_via_servertool_core_bridge --lib -- --nocapture` 1/1; native build; direct `.node` export probe; focused Jest `tests/servertool/followup-runtime-provider-pin.spec.ts` 4/4; `npm run verify:servertool-rust-only`; llmswitch-core/root tsc; `cargo test -p servertool-core --lib -- --nocapture` 165/165; `cargo test -p servertool-cli -- --nocapture` 8/8; `git diff --check`.
Tags: servertool, orchestration-policy, timeout, client-inject, error-compaction, provider-key, rust-owner, native-bridge, 2026-06-09

## 2026-06-09 Servertool timeout/disconnect policy Rust owner
- Servertool timeout watcher planning, client disconnect detection/watcher planning, client-disconnected error payloads, timeout error payloads, and stop-message fetch-failed error payloads are Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/servertool-core/src/orchestration_policy_contract.rs::{plan_servertool_timeout_watcher,is_adapter_client_disconnected,plan_client_disconnect_watcher,plan_servertool_client_disconnected_error,plan_servertool_timeout_error,plan_stop_message_fetch_failed_error}`.
- TS `sharedmodule/llmswitch-core/src/servertool/timeout-error-block.ts` may only execute timer/AbortController/Error-object glue from native plans. It must not restore local disconnected-state scanning, poll interval floor/max policy, timeout arming policy, timeout/disconnect message building, status/code/category selection, or numeric clamp policy.
- Native bridge chain uses `native-servertool-core-semantics.ts::{planServertoolTimeoutWatcherWithNative,isAdapterClientDisconnectedWithNative,planClientDisconnectWatcherWithNative,planServertoolClientDisconnectedErrorWithNative,planServertoolTimeoutErrorWithNative,planStopMessageFetchFailedErrorWithNative}` and required exports `planServertoolTimeoutWatcherJson`, `isAdapterClientDisconnectedJson`, `planClientDisconnectWatcherJson`, `planServertoolClientDisconnectedErrorJson`, `planServertoolTimeoutErrorJson`, `planStopMessageFetchFailedErrorJson`.
- Gate: `npm run verify:servertool-rust-only` locks Rust owner/native bridge/required exports and blocks TS timeout-error semantic revival.
- Verified on 2026-06-09: Rust fmt check; `cargo test -p servertool-core orchestration_policy --lib -- --nocapture` 7/7; `cargo test -p router-hotpath-napi orchestration_policy --lib -- --nocapture` 1/1; native build; direct `.node` export probe for six timeout/disconnect exports; `npm run verify:servertool-rust-only`; `npm run verify:function-map-compile-gate`; focused Jest `timeout-error-block`, `followup-runtime-provider-pin`, and `followup-bootstrap-replay` 10/10; root `npx tsc --noEmit --pretty false`; `git diff --check`.
Tags: servertool, timeout, disconnect, error-payload, rust-owner, native-bridge, 2026-06-09

## 2026-06-09 Servertool backend-route materialization Rust owner
- Backend-route followup materialization is Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/servertool-core/src/backend_route_contract.rs::plan_followup_materialization`.
- Rust owns the preserved contracts: `followupPlan.entryEndpoint` wins after trim, then current request entry endpoint, then `/v1/chat/completions`; `payload` property wins over `injection`; only object payload/injection values are materialized; invalid payload/injection values remain null and must fail through the existing missing-payload/error path instead of being repaired in TS.
- Native bridge chain adds required export `planFollowupMaterializationJson`; TS `backend-route-runtime-block.ts` may only call `planFollowupMaterializationWithNative`, execute the injection IO builder from the native plan, and consume other Rust runtime action/metadata plans.
- Gate: `npm run verify:servertool-rust-only` blocks TS revival of local `resolveFollowupEntryEndpoint`, `resolveFollowupPayloadFromPlan`, `resolveFollowupPayloadSource`, direct `followupPlan.payload/injection/entryEndpoint` scanning, and TS chat endpoint defaults in `backend-route-runtime-block.ts`.
- Feature map: `hub.servertool_backend_route_runtime` maps this owner to the required servertool backend-route tests and gates.
- Verified on 2026-06-09: Rust fmt check, servertool-core materialization focused 4/4, router-hotpath-napi materialization bridge 1/1, native build and direct `.node` export probe, `npm run verify:servertool-rust-only`, `npm run verify:function-map-compile-gate`, llmswitch-core/root tsc, servertool-core full lib 169/169, servertool-cli blackbox 8/8, web_search/vision focused Jest 25/25, and `git diff --check` all passed.
Tags: servertool, backend-route, followup-materialization, rust-owner, native-bridge, function-map, 2026-06-09

## 2026-06-09 Servertool runtime context resolver Rust owner
- Captured request selection, client connection state normalization, compaction flag detection, and entry endpoint resolution are Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/servertool-core/src/persisted_lookup.rs::{get_captured_request,resolve_client_connection_state,has_compaction_flag,resolve_entry_endpoint}`.
- Rust owns the preserved contracts: captured request prefers `capturedEntryRequest` before `capturedChatRequest`; client connection state only accepts object values; compaction flag accepts boolean `true` or string `"true"` after trim/case normalization; entry endpoint prefers record `entryEndpoint`, then `metadata.entryEndpoint`, then `/v1/chat/completions`.
- Native bridge chain adds required exports `getCapturedRequestJson`, `resolveClientConnectionStateJson`, `hasCompactionFlagJson`, and `resolveEntryEndpointJson`; TS `runtime-utils.ts::{getCapturedRequest,resolveClientConnectionState,hasCompactionFlag,resolveEntryEndpoint}` may only call native wrappers.
- Gate: `npm run verify:servertool-rust-only` blocks TS revival of local captured-request, connection-state, compaction-flag, and entry-endpoint semantics in `runtime-utils.ts`.
- Verified on 2026-06-09: `cargo fmt`, servertool-core persisted_lookup focused 22/22, router-hotpath-napi runtime-context focused 1/1, native build and direct export load, `npm run verify:servertool-rust-only`, llmswitch-core/root tsc, focused stop-message/hub residue Jest 163 passed + 16 skipped, CLI/shadow Jest 19/19, servertool-core full lib 140/140, servertool-cli blackbox 8/8, and `git diff --check` all passed.
Tags: servertool, stop-message, runtime-context, captured-request, compaction, entry-endpoint, rust-owner, native-bridge, 2026-06-09

## 2026-06-09 Servertool followup tool content limit Rust owner
- Stop-message followup tool content max chars policy is Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/servertool-core/src/persisted_lookup.rs::resolve_stop_message_followup_tool_content_max_chars`.
- Rust owns the preserved contract: non-empty env string parses finite positive number, floors it, clamps to minimum 64, and invalid env returns no value without model fallback; absent env with `kimi-k2.5` / `kimi-k2.5-*` model returns 1200; other models return no value.
- Native bridge chain adds required export `resolveStopMessageFollowupToolContentMaxCharsJson`; TS `runtime-utils.ts::resolveStopMessageFollowupToolContentMaxChars` may only read `ROUTECODEX_STOPMESSAGE_FOLLOWUP_TOOL_CONTENT_MAX_CHARS` and call the native wrapper.
- Gate: `npm run verify:servertool-rust-only` blocks TS revival of numeric parsing/clamping and `kimi-k2.5` model policy in `runtime-utils.ts`.
- Verified on 2026-06-09: Rust fmt check, servertool-core focused/full lib 141/141, router-hotpath-napi focused export test 1/1, native build and direct export load, `npm run verify:servertool-rust-only`, llmswitch-core/root tsc, focused runtime-utils Jest 6/6, stop-message/hub residue Jest 164 passed + 16 skipped, CLI/shadow Jest 19/19, servertool-cli blackbox 8/8, and `git diff --check` all passed.
Tags: servertool, stop-message, content-limit, rust-owner, native-bridge, 2026-06-09

## 2026-06-09 Servertool persist stop-message state Rust owner
- Stop-message persisted-state clear/save planning is Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/servertool-core/src/persisted_lookup.rs::plan_persist_stop_message_state`.
- Rust owns the preserved contract: clear only when stop-message text/mode/count fields are blank or non-finite, no lifecycle stamp exists, and no non-stop routing or pre-command state exists; save when any stop-message, lifecycle, provider allow/disable, forced/prefer target, stopless goal, or pre-command state exists.
- Native bridge chain adds required export `planPersistStopMessageStateJson`; TS `runtime-utils.ts::persistStopMessageState` may only convert Set/Map fields to JSON for native input and then call `saveRoutingInstructionStateSync(stickyKey, null | state)`.
- Gate: `npm run verify:servertool-rust-only` blocks TS revival of empty-state semantics inside `persistStopMessageState`.
- Verified on 2026-06-09: Rust fmt check, servertool-core focused/full lib 142/142, router-hotpath-napi focused bridge test 1/1, native build and direct export probe, `npm run verify:servertool-rust-only`, llmswitch-core/root tsc, runtime-utils Jest 7/7, stop-message/hub residue Jest 165 passed + 16 skipped, CLI/shadow Jest 19/19, servertool-cli blackbox 8/8, and `git diff --check` all passed.
Tags: servertool, stop-message, persist-state, rust-owner, native-bridge, 2026-06-09

## 2026-06-09 Servertool default/implicit snapshot Rust owner
- Stop-message default snapshot and implicit Gemini empty-response snapshot planning are Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/servertool-core/src/persisted_lookup.rs::{resolve_default_stop_message_snapshot,resolve_implicit_gemini_stop_message_snapshot}`.
- Rust owns the preserved contracts: adapter `__rt.stopGatewayContext` eligibility can override direct base inspection; default snapshot uses trimmed custom text or `继续执行` and floors positive finite numeric maxRepeats else 1; implicit Gemini requires `gemini-chat`, `/v1/responses`, stop eligibility, and empty assistant reply; Responses tool/function/call output with extractable text or tool-like output is not empty.
- Native bridge chain adds required exports `resolveDefaultStopMessageSnapshotJson` and `resolveImplicitGeminiStopMessageSnapshotJson`; TS `runtime-utils.ts::{resolveDefaultStopMessageSnapshot,resolveImplicitGeminiStopMessageSnapshot}` may only call native wrappers and must not import stop-gateway or ai-followup semantic helpers.
- Gate: `npm run verify:servertool-rust-only` blocks TS revival of `isStopEligibleForServerTool`, `extractResponsesOutputText`, `hasToolLikeOutput`, `isEmptyAssistantReply`, Gemini `/v1/responses` checks, Responses output/tool checks, default `继续执行`, and local maxRepeats parsing inside `runtime-utils.ts`.
- Verified on 2026-06-09: Rust fmt check, servertool-core full lib 149/149, router-hotpath-napi snapshot bridge 2/2, native build and direct `.node` export probe, `npm run verify:servertool-rust-only`, llmswitch-core/root tsc, runtime-utils Jest 9/9, stop-message/hub residue Jest 168 passed + 16 skipped, and `git diff --check` all passed.
Tags: servertool, stop-message, default-snapshot, implicit-gemini, empty-reply-detection, rust-owner, native-bridge, 2026-06-09

## 2026-06-09 Servertool loop state Rust owner
- Servertool loop-state read/plan policy is Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/servertool-core/src/loop_state_contract.rs::{read_servertool_loop_state,plan_servertool_loop_state}`.
- Rust owns the preserved contracts: runtime `serverToolLoopState` normalization requires non-empty `payloadHash`; numeric counters are floored and clamped at 0; non-stop flow tracks payload hash unless `flowOnlyLoopLimit`; stop-message/flow-only uses `__servertool_auto__`; repeat count increments only on same flow and same selected payload mode; `startedAtMs` is reused for same flow; stop pair repeat count increments only on same pair and preserves `stopPairWarned` only for same pair.
- TS `sharedmodule/llmswitch-core/src/servertool/loop-state-block.ts` may only compute stable SHA1 payload/pair hashes, read runtime metadata, and call native wrappers. It must not restore `sameFlow`, `samePayload`, previous-count math, `__servertool_auto__`, stop-message flow branching, or local `Number.isFinite` / `Math.floor` / `Math.max` policy.
- Native bridge chain adds required exports `readServertoolLoopStateJson` and `planServertoolLoopStateJson`; `npm run verify:servertool-rust-only` locks Rust owner/native bridge and TS thin-shell boundaries.
- Verified on 2026-06-09: Rust fmt check, `cargo test -p servertool-core --lib -- --nocapture` 154/154, router-hotpath-napi loop-state bridge 2/2, native build and direct `.node` export probe, `npm run verify:servertool-rust-only`, llmswitch-core/root tsc, `tests/servertool/loop-state-block.spec.ts` 3/3, `tests/servertool/stop-message-flow-followup-reentry.spec.ts` 9/9, and `git diff --check` all passed.
Tags: servertool, loop-state, stop-message-flow, rust-owner, native-bridge, 2026-06-09

## 2026-06-09 Servertool stop-gateway Rust owner
- Stop gateway response inspect, stop eligibility, embedded tool marker detection, reasoning-only empty assistant detection, Responses tool-like output detection, and metadata `StopGatewayContext` normalization are Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/servertool-core/src/stop_gateway_context.rs::{inspect,is_stop_eligible,normalize_stop_gateway_context}`.
- Rust preserves the real last matching chat choice index, accepts spaced/case-varied tool markers such as `<|  TOOL_CALL_BEGIN  |>`, floors/clamps optional `choiceIndex`, normalizes source to `chat|responses|none`, and rejects invalid metadata context by returning null/undefined through native.
- TS `sharedmodule/llmswitch-core/src/servertool/stop-gateway-context.ts` may only call `inspectStopGatewaySignalWithNative` / `normalizeStopGatewayContextWithNative` plus runtime metadata IO. It must not restore fallback inspect, local marker scans, reasoning-only detection, Responses tool-like scans, local context normalization, or swallowed metadata write failure.
- Native bridge chain adds required export `normalizeStopGatewayContextJson`; `npm run verify:servertool-rust-only` locks Rust owner/native bridge and TS thin-shell boundaries.
- Verified on 2026-06-09: Rust fmt check, servertool-core stop_gateway 12/12, router-hotpath-napi stop_gateway bridge 1/1, native build and direct `.node` export probe, `tests/servertool/stop-gateway-context.spec.ts` 5/5, `npm run verify:servertool-rust-only`, llmswitch-core/root tsc, `cargo test -p servertool-core --lib -- --nocapture` 157/157, and `git diff --check` all passed.
Tags: servertool, stop-gateway, stop-eligibility, rust-owner, native-bridge, 2026-06-09

## 2026-06-09 Servertool stop-message compare context Rust owner
- Stop-message compare context normalization and progress summary formatting are Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/servertool-core/src/stop_message_compare_context.rs::{normalize_stop_message_compare_context,format_stop_message_compare_context}`.
- Rust owns the preserved contracts: `decision` must normalize to `trigger|skip`; invalid decision rejects context; `mode` normalizes to `on|auto|off` with invalid mode becoming `off`; numeric fields floor and clamp to non-negative i32; missing `remaining` becomes `maxRepeats - used` clamped at 0; optional stage/bd/observation/tool-signature fields are trimmed.
- TS `sharedmodule/llmswitch-core/src/servertool/stop-message-compare-context.ts` may only call native wrappers plus runtime metadata IO. It must not restore local normalize/format logic, local numeric floor/clamp, Boolean truthiness policy, or swallowed metadata write failure.
- Native bridge chain adds required exports `normalizeStopMessageCompareContextJson` and `formatStopMessageCompareContextJson`; `npm run verify:servertool-rust-only` locks Rust owner/native bridge and TS thin-shell boundaries.
- Verified on 2026-06-09: Rust fmt check, servertool-core stop_message_compare 3/3, router-hotpath-napi compare bridge 1/1, native build and direct `.node` export probe, `tests/servertool/stop-message-compare-context.spec.ts` 5/5, `npm run verify:servertool-rust-only`, llmswitch-core/root tsc, `cargo test -p servertool-core --lib -- --nocapture` 160/160, `cargo test -p servertool-cli -- --nocapture` 8/8 blackbox, and `git diff --check` all passed.
Tags: servertool, stop-message, compare-context, progress-log, rust-owner, native-bridge, 2026-06-09

## 2026-06-09 Servertool skeleton-config Rust owner
- Servertool skeleton derived config, registration spec normalization, tool name aliasing, auto-hook phase normalization, priority integer parsing, tool spec lookup, auto hook queue, pending injection kinds, progress config, followup flow profiles, and state config are Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/router-hotpath-napi/src/servertool_skeleton_config.rs::{plan_servertool_skeleton_derived_config_json,normalize_servertool_registration_spec_json,resolve_servertool_tool_spec_json}`.
- TS `sharedmodule/llmswitch-core/src/servertool/skeleton-config.ts` may only call native wrappers and return typed projections. It must not restore `normalizeServerToolName`, `normalizeAutoHookPhase`, `normalizeInteger`, `Number.isFinite`, `Math.floor`, alias checks for `websearch` / `web-search`, trigger-to-execution-mode policy, `Object.fromEntries` config builders, or followup profile normalization.
- Native bridge chain uses `native-chat-process-servertool-orchestration-semantics.ts::{planServertoolSkeletonDerivedConfigWithNative,normalizeServertoolRegistrationSpecWithNative,resolveServertoolToolSpecWithNative}` and required exports `planServertoolSkeletonDerivedConfigJson`, `normalizeServertoolRegistrationSpecJson`, `resolveServertoolToolSpecJson`.
- Gate: `npm run verify:servertool-rust-only` locks the Rust owner/native bridge/required export chain and blocks TS skeleton-config semantic revival.
- Verified on 2026-06-09: router-hotpath-napi fmt check; focused Rust tests for `derived_config`, `registration_spec`, and `resolves_tool_spec`; native build; direct `.node` export probe; `npm run verify:servertool-rust-only`; llmswitch-core/root tsc; focused skeleton Jest 4/4; and `git diff --check` all passed.
Tags: servertool, skeleton-config, rust-owner, native-bridge, thin-shell, 2026-06-09

## 2026-06-09 Servertool pending-session and pending-injection Rust owner
- Servertool pending-session file-name sanitization, pending max-age parsing, save payload coercion, load stale/malformed/drop decisions, pending injection session dedupe, persist record planning, and persistence error envelope construction are Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/servertool-core/src/pending_session_contract.rs::{resolve_pending_file_name,resolve_pending_max_age_ms,plan_pending_session_save,plan_pending_session_load,plan_pending_injection_persist,plan_pending_injection_persist_error}`.
- TS `sharedmodule/llmswitch-core/src/servertool/pending-session.ts` may only read env/session dir, perform file IO, and call native wrappers. TS `sharedmodule/llmswitch-core/src/servertool/pending-injection-block.ts` may only call native persist/error planners, execute `savePendingServerToolInjection`, and instantiate the Rust-planned `ProviderProtocolError`.
- Do not restore local segment sanitization, numeric parsing/defaulting, payload validation/coercion, stale-drop policy, malformed-load drop decisions, `aliasSessionIds` dedupe, pending injection payload construction, or `SERVERTOOL_PENDING_INJECTION_FAILED` envelope assembly in TS.
- Native bridge chain uses `native-servertool-core-semantics.ts::{resolvePendingSessionFileNameWithNative,resolvePendingSessionMaxAgeMsWithNative,planPendingSessionSaveWithNative,planPendingSessionLoadWithNative,planPendingInjectionPersistWithNative,planPendingInjectionPersistErrorWithNative}` and required exports `resolvePendingSessionFileNameJson`, `resolvePendingSessionMaxAgeMsJson`, `planPendingSessionSaveJson`, `planPendingSessionLoadJson`, `planPendingInjectionPersistJson`, `planPendingInjectionPersistErrorJson`.
- Verification note: if pending-session or pending-injection Jest reports native unavailable, rebuild native hotpath; do not add TS fallback. The failure means installed `.node` lacks the new Rust exports.
- Function/verification map entry: `hub.servertool_pending_session`.
- Verified on 2026-06-09: Rust fmt check; `cargo test -p servertool-core pending_session --lib -- --nocapture` 5/5; `cargo test -p router-hotpath-napi pending_session --lib -- --nocapture` 2/2; `node sharedmodule/llmswitch-core/scripts/build-native-hotpath.mjs`; direct `.node` export probe for pending-injection and pre-command exports; `npm run verify:servertool-rust-only`; `npm run verify:function-map-compile-gate`; focused pending-session/pending-injection/pre-command Jest 12/12; root `npx tsc --noEmit --pretty false`; and `git diff --check`.
Tags: servertool, pending-session, pending-injection, rust-owner, native-bridge, thin-shell, 2026-06-09

## 2026-06-09 Servertool pre-command hooks Rust owner
- Servertool pre-command hook config enable/disable, hook rule normalization, hook id sanitization, tool-set default/dedupe/normalize, regex source/flags planning, timeout clamp/default, priority parsing, and runtime pre-command rule planning are Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/servertool-core/src/pre_command_hook_contract.rs::{plan_pre_command_hooks_config,plan_runtime_pre_command_rule}`.
- TS `sharedmodule/llmswitch-core/src/servertool/pre-command-hooks.ts` may only read config/env/cache, materialize Rust regex plans to `RegExp`, parse current tool arguments, and execute jq/shell/runtime script IO. It must not restore local rule normalization, hook id sanitize, tool-set policy, regex parsing policy, timeout parsing/clamp, or priority parsing.
- Native bridge chain uses `native-servertool-core-semantics.ts::{planPreCommandHooksConfigWithNative,planRuntimePreCommandRuleWithNative}` and required exports `planPreCommandHooksConfigJson` / `planRuntimePreCommandRuleJson`.
- Function/verification map entry: `hub.servertool_pre_command_hooks`; `npm run verify:servertool-rust-only` blocks TS semantic revival.
- Verified on 2026-06-09: Rust fmt check; `cargo test -p servertool-core pre_command --lib -- --nocapture` 3/3; `cargo test -p router-hotpath-napi pre_command --lib -- --nocapture` 1/1; native hotpath build; direct `.node` export probe for pending-injection and pre-command exports; `npm run verify:servertool-rust-only`; `npm run verify:function-map-compile-gate`; focused pending-session/pending-injection/pre-command Jest 12/12; root `npx tsc --noEmit --pretty false`; and `git diff --check`.
Tags: servertool, pre-command-hooks, rust-owner, native-bridge, thin-shell, 2026-06-09

## 2026-06-09 Servertool flow presentation and fixture handler Rust owner
- Servertool progress presentation policy is Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/router-hotpath-napi/src/servertool_skeleton_config.rs::{resolve_servertool_progress_tool_name_json,should_use_servertool_gold_progress_highlight_json}`.
- TS `sharedmodule/llmswitch-core/src/servertool/flow-presentation-block.ts` may only call native wrappers. TS `sharedmodule/llmswitch-core/src/servertool/skeleton-config.ts` must not expose progress presentation projection helpers. Neither file may restore local flow-id normalization, `buildServertoolProgressConfig`, `toolNameByFlowId`, `goldHighlightFlowIds`, or Set-based highlight policy.
- `servertool_fixture` CLI projection dispatch is Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/router-hotpath-napi/src/chat_servertool_orchestration.rs`; the old empty TS handler `sharedmodule/llmswitch-core/src/servertool/handlers/fixture.ts` was physically deleted and must not be restored through TS registry.
- Function/verification map entry: `hub.servertool_flow_presentation`; `hub.servertool_cli_projection` notes now lock deleted TS fixture handler.
- Verified on 2026-06-09: `cargo fmt --package router-hotpath-napi --check`; router-hotpath-napi fixture dispatch Rust unit 1/1; router-hotpath-napi flow-presentation Rust unit 1/1; `node sharedmodule/llmswitch-core/scripts/build-native-hotpath.mjs`; `npm run verify:servertool-rust-only`; focused Jest servertool CLI/mixed-tools/dispatch/progress/CLI command suites 28 passed + 3 skipped; root `npx tsc --noEmit --pretty false`; `git diff --check`.
Tags: servertool, flow-presentation, cli-projection, fixture-handler, dead-code, rust-owner, native-bridge, 2026-06-09

## 2026-06-09 Servertool backend-route runtime/materialization Rust owner
- Backend-route followup materialization and runtime planning are Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/servertool-core/src/backend_route_contract.rs::{plan_followup_materialization,plan_followup_runtime_action,plan_followup_runtime_metadata}`.
- Rust owns the preserved contracts: followup `entryEndpoint` prefers plan `entryEndpoint`, then request entry endpoint, then `/v1/chat/completions`; `payload` property wins over `injection` even when invalid; only plain object `payload` / `injection` values materialize; runtime action and metadata plans remain in the same Rust owner.
- TS `sharedmodule/llmswitch-core/src/servertool/backend-route-runtime-block.ts` may only call native wrappers, execute the injection IO builder, mutate metadata from Rust plans, and throw Rust-described errors. It must not restore `resolveFollowupEntryEndpoint`, `resolveFollowupPayloadFromPlan`, `resolveFollowupPayloadSource`, local `followupPlan.payload/injection/entryEndpoint` scanning, or local chat endpoint defaults.
- Native bridge chain adds required export `planFollowupMaterializationJson`; `npm run verify:servertool-rust-only` locks Rust owner/native bridge and TS no-local-materialization boundaries.
- Function/verification map entry: `hub.servertool_backend_route_runtime`.
- Verified on 2026-06-09: `npm run verify:function-map-compile-gate`, `npm run verify:servertool-rust-only`, Rust fmt check, servertool-core materialization 4/4, router-hotpath-napi materialization bridge 1/1, native build, direct `.node` export probe, llmswitch-core/root tsc, servertool web_search + vision Jest 25/25, focused hub residue audit 111/111, and `git diff --check` all passed.
Tags: servertool, backend-route, followup-materialization, runtime-action, runtime-metadata, rust-owner, native-bridge, 2026-06-09

## 2026-06-09 Hub types zero-consumer barrel removed
- `sharedmodule/llmswitch-core/src/conversion/hub/types/index.ts` was verified as a 0-consumer pure barrel and has been physically deleted with ignored stale dist outputs `sharedmodule/llmswitch-core/dist/conversion/hub/types/index.{js,d.ts}`.
- Keep importing concrete Hub type modules directly (`chat-envelope`, `json`, `standardized`, `errors`, `node`, etc.); do not restore the aggregate `conversion/hub/types/index` barrel as a compatibility surface.
- Boundary note: `sharedmodule/llmswitch-core/src/conversion/hub/snapshot-recorder.ts` is not a 0-consumer deletion candidate because `src/modules/llmswitch/bridge/snapshot-recorder.ts` dynamically loads `importCoreDist('conversion/hub/snapshot-recorder')`.
- Gate: `tests/sharedmodule/hub-pipeline-stage-residue-audit.spec.ts` now keeps the deleted source barrel and ignored dist artifacts absent.
- Verified on 2026-06-09: strict import/dynamic scan only finds the residue gate itself; source/dist absent check; focused residue Jest 110/110; llmswitch-core tsc/build; root tsc; bridge protocol blackbox 10/10; and `git diff --check` all passed.
Tags: hub-pipeline, dead-code, zero-consumer, barrel-deletion, residue-gate, 2026-06-09

## 2026-06-09 VR native wrapper zero-consumer deletion
- `sharedmodule/llmswitch-core/src/native/router-hotpath/native-virtual-router-bootstrap-routing.ts` and `sharedmodule/llmswitch-core/src/native/router-hotpath/native-virtual-router-stop-message-state-semantics.ts` were verified as 0-consumer TS native wrappers and physically deleted with ignored dist outputs `dist/native/router-hotpath/native-virtual-router-bootstrap-routing.{js,d.ts}` and `dist/native/router-hotpath/native-virtual-router-stop-message-state-semantics.{js,d.ts}`.
- Do not restore these TS wrappers as compatibility surfaces. The Rust native capabilities remain Rust-owned; if they are consumed again, add/route through a current owner bridge rather than reviving old wrapper files.
- Current static 0-consumer exceptions remain deliberate: `snapshot-recorder.ts` is dynamically loaded through `importCoreDist('conversion/hub/snapshot-recorder')`, `native-failure-policy.ts` is dynamically loaded by bridge/runtime failure-policy owners, and `native-hub-vr-node-contracts.ts` is consumed by contract/help tests.
- Gate: `tests/sharedmodule/hub-pipeline-stage-residue-audit.spec.ts` now keeps both deleted source wrappers and ignored dist artifacts absent.
- Follow-up: `sharedmodule/llmswitch-core/config/rust-migration-modules.json` is broadly stale and references many already-deleted old paths; clean it as a separate manifest-governance slice rather than one-off string patching.
- Verified on 2026-06-09: source/dist absent check; focused residue Jest 110/110; llmswitch-core tsc/build; root tsc; bridge protocol blackbox 10/10; and `git diff --check` all passed.
Tags: virtual-router, native-wrapper, dead-code, zero-consumer, rust-owner, residue-gate, 2026-06-09

## 2026-06-09 Legacy shadow-gate migration manifest removed
- `sharedmodule/llmswitch-core/config/rust-migration-modules.json` and its shadow-gate support scripts/docs were verified as a dead migration system: the manifest listed many already-deleted old paths, and root/core `package.json` expose no active `verify:shadow-gate:*`, `check-shadow-coverage-gate`, `promote-shadow-module`, or `run-ci-coverage` scripts.
- Physically deleted: `config/rust-migration-modules.json`, `docs/rust-migration-gates.md`, `scripts/check-shadow-coverage-gate.mjs`, `scripts/lib/rust-migration-manifest.mjs`, `scripts/promote-shadow-module.mjs`, `scripts/run-ci-coverage.mjs`, and `scripts/verify-shadow-gate-all.mjs` under `sharedmodule/llmswitch-core/`.
- Do not use old shadow-gate manifest entries as evidence for Rust closeout. Current closeout evidence must come from active residue gates, Rust/native tests, TypeScript build checks, blackbox/live smoke, and current function/verification maps.
...
## 2026-06-09 Servertool CLI projection output Rust owner
- Client exec CLI projection output planning is Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/servertool-core/src/cli_contract.rs::plan_client_exec_cli_projection_output`. Rust owns stopless `flowId=stop_message_flow -> toolName=stop_message_auto`, explicit wrong tool-name rejection, input object/null validation, repeatCount/maxRepeats extraction and validation, canonical stopless CLI input construction, optional stdoutPreview merge, and final `routecodex servertool run <toolName> --input-json <json>` command construction through `build_client_exec_cli_projection_output`.
- TS `sharedmodule/llmswitch-core/src/servertool/cli-projection.ts` may only pass `flowId`, `input`, and optional `stdoutPreview` into `buildClientExecCliProjectionOutputWithNative`, then consume the native projection shell. Do not restore TS `args.flowId === 'stop_message_flow'`, local toolName mapping, local repeat/max parsing, or local stopless payload merge.
- Native bridge chain uses `servertool_core_blocks.rs::build_client_exec_cli_projection_output_json`, `router-hotpath-napi/src/lib.rs::build_client_exec_cli_projection_output_json`, required export `buildClientExecCliProjectionOutputJson`, and wrapper `buildClientExecCliProjectionOutputWithNative`. `npm run verify:servertool-rust-only` locks the owner/native export/no-TS-owner boundary.
- Verified on 2026-06-09: Rust fmt check; `cargo test -p servertool-core projection --lib -- --nocapture` 40/40; router-hotpath bridge test 1/1; native hotpath build; direct `.node` probe for stopless projection output; focused Jest `servertool-cli-projection` + `stop-message-auto` 52 passed / 16 skipped; `npm run verify:servertool-rust-only`; llmswitch-core/root tsc; servertool-core full 194/194; servertool-cli 8/8; `npm run verify:function-map-compile-gate`; and `git diff --check`.
Tags: servertool, cli-projection, stopless, rust-owner, native-bridge, thin-shell, 2026-06-09

## 2026-06-09 Servertool AI followup branch removed
- The old stopless AI followup branch is physically deleted: `sharedmodule/llmswitch-core/src/servertool/handlers/stop-message-auto/ai-followup.ts`, `ai-followup-pure-blocks.ts`, `tests/servertool/stopmessage-response-snapshot.spec.ts`, and `tests/servertool/stop-message-auto-followup-extraction.spec.ts`.
- `stop-message-auto/config.ts` no longer parses or exposes `aiFollowup` config or `resolveStopMessageAi*` helpers. `docs/stop-message-auto.md` no longer documents `aiFollowup`; `src/index.ts` no longer preserves old `[servertool][ai-followup]` logs.
- Do not restore `aiFollowup`, `ROUTECODEX_STOPMESSAGE_AI_FOLLOWUP*`, `ROUTECODEX_STOPMESSAGE_AUTOMESSAGE*`, `renderStopMessageAutoFollowupViaAi`, or `buildStopMessageAutoMessagePrompt*`. Stopless must continue through Rust-owned client-visible CLI projection.
...
- `src/modules/pipeline/utils/oauth-helpers.ts` and `src/modules/pipeline/validation/**` were also verified as zero-consumer legacy utilities and physically deleted. Do not restore the old TS pipeline config validator; config/routing truth belongs to current config loader, llmswitch-core Hub Pipeline, and Rust/native contracts.
- `src/modules/pipeline/types/common-types.ts`, `module.types.ts`, and `shared-dtos.ts` were pure one-line aliases to `src/types/**` and have been physically deleted. Import the concrete `src/types/**` modules directly instead of restoring pipeline type barrels.
- `src/modules/pipeline/types/{base-types,provider-config-types,provider-types,transformation-types}.ts` were verified as legacy type-cluster residue and physically deleted. `pipeline-interfaces.ts` is intentionally narrowed to the still-live provider/runtime compatibility surface and must not regrow old PipelineManager/LLMSwitch/Compatibility/Transformation interfaces.
...
## 2026-06-09 Legacy Hub Pipeline util wrappers removed
- `src/modules/pipeline/utils/{preflight-validator,tool-result-text,transformation-engine}.ts` and their provider re-export wrappers under `src/providers/core/utils/` were verified as zero-consumer legacy TS pipeline semantic residues and physically deleted.
- Do not restore generic TS Hub preflight/sanitizer, tool-result text degradation, or transformation-engine compatibility surfaces. Request/response/tool semantics belong in `sharedmodule/llmswitch-core` Rust/native contracts; provider-specific wire behavior belongs in provider runtime owners.
...
## 2026-06-09 SSE function-map architecture ownership
- SSE ownership is now split into six project-level function-map features: `sse.codec_registry_surface`, `sse.stream_parse_boundary`, `sse.responses_decode_projection`, `sse.responses_encode_projection`, `sse.chat_stream_projection`, and `sse.anthropic_gemini_stream_projection`.
- SSE source owners under `sharedmodule/llmswitch-core/src/sse` must keep `feature_id:` anchors, and architecture CI must keep `npm run verify:sse-architecture-boundary` wired before the function-map compile gate.
- SSE shared registry/writer/serializer owners must not add provider-specific branches, hidden protocol fallback, `any` stream aliases, fallback serializer paths, or client-visible internal carriers. Plain provider `metadata` in direct passthrough SSE is not itself a leak; internal metadata controls such as `routeHint`, `__routecodex*`, `__rt*`, or other carrier fields must fail fast.
- Direct passthrough SSE metadata guards must validate complete SSE frames before emitting bytes to the client, with both LF and CRLF frame boundaries supported; chunk-level JSON checks are insufficient because a split `data:` frame can leak before the second chunk fails.
Tags: sse, function-map, architecture-gate, no-fallback, metadata-boundary, 2026-06-09

## 2026-06-10 Responses SSE backpressure hang truth
- Verified hang root cause: `StreamWriter.handleBackpressure()` in `sharedmodule/llmswitch-core/src/sse/shared/writer.ts` must not only wait for future `drain`; after registering listeners it must immediately recheck `stream.writableLength < maxBufferSize`, and resolve on close / reject on error. Otherwise `PassThrough({ objectMode: true })` backpressure can leave JSON-to-SSE terminal projection waiting forever even though provider response completed.
- Runtime closeout proof for the fix: `npm run build:min`, `npm run install:global`, and `npm run install:release` produced/installed `0.90.3048`; `routecodex/rcc --version` both `0.90.3048`; release current symlink `releases/routecodex-0.90.3048-2026-06-09T161413Z`; live 5520/5555 `/v1/responses` stream smokes both emitted `response.completed` and `response.done` with no `event: error`.
Tags: sse, responses, backpressure, stream-writer, live-smoke, 2026-06-10

## 2026-06-10 Responses SSE client-close cleanup
- Verified hang slice in `src/server/handlers/handler-response-utils.ts`: for non-continuation `entryEndpoint === '/v1/responses'` client-close cleanup, clearing abandoned Responses store state must not wait for `clientWriteQueue`; only continuation responses keep the queue wait before persistence.
- Verification: focused `tests/server/handlers/handler-response-utils.sse-finish-reason.spec.ts` PASS 19/19, `npx tsc --noEmit --pretty false` PASS, global runtime at `0.90.3053` passed live 5555/5520 `/v1/responses` smokes with `response.completed` + `response.done` and no `event:error`.
Tags: sse, responses, client-close, cleanup, store-release, live-smoke, 2026-06-10
...
## 2026-06-10 Req inbound reasoning / semantic-lift public bridges removed
- Exact scan found `normalizeReqInboundReasoningPayloadWithNative`, `normalizeReasoningPayloadV2WithNative`, `shouldNormalizeReasoningPayloadWithNative`, `applyReqInboundSemanticLiftWithNative` and matching JSON exports `normalize_req_inbound_reasoning_payload_json`, `should_normalize_reasoning_payload_json`, `normalize_reasoning_payload_v2_json`, `apply_req_inbound_semantic_lift_json` had no live TS/runtime consumer. Active req_inbound semantics remain `apply_req_inbound_semantic_lift` (Rust-internal, called by `hub_pipeline_lib::engine`) and the lower-level `normalize_reasoning_in_{chat,responses,gemini,anthropic}_payload` / `normalize_resp_inbound_reasoning_payload_json` normalizers.
...
- 2026-06-13: stopless stop-schema gate contract is Rust-owned by `sharedmodule/llmswitch-core/rust-core/crates/stop-message-core/src/lib.rs`. Durable rules: (1) system/continuation guidance must always teach stop schema first; (2) gate must enumerate only missing fields; (3) `stopreason=0/1` with complete terminal fields stops immediately; (4) `stopreason=2` without `next_step` must request `next_step`; (5) 3 rounds are loop guard only and must key off `no_change_count`, not ordinary `used` progression. Verified by `cargo test --manifest-path .../stop-message-core/Cargo.toml --lib` (51/51) and `cargo test --manifest-path .../servertool-core/Cargo.toml --lib` (252/252), plus runtime install/health on 5555/5520/10000 version 0.90.3064.

- 2026-06-13 update: request/handler-level stopless blackbox proof now uses the repository's ESM-safe Jest entrypoint `npm run jest:run` (node `--experimental-vm-modules`), not raw `npx jest`. Verified stopless suites: `tests/servertool/stop-message-auto.spec.ts` passed 51 tests (8 skipped) and `tests/servertool/stop-message-compare-context.spec.ts` passed 6 tests. The stop_message_auto TS thin shell now computes observationHash/toolSignatureHash and derives `schemaGate.no_change_count` from previous compare-context stable observations, completing the upstream half of the “3 rounds only for no-change loop guard” contract.

- 2026-06-13: Function-map audit baseline is 62 features in `docs/architecture/function-map.yml` and 62 matching features in `docs/architecture/verification-map.yml`; `npm run verify:function-map-compile-gate`, `npm run verify:architecture-owner-queryability`, `npm run verify:architecture-feature-map-growth-discipline`, `npm run verify:architecture-provider-specific-leaks`, `npm run verify:architecture-thin-wrapper-only`, `npm run verify:architecture-error-chain-bypass`, `npm run verify:architecture-metadata-leak-boundary`, `npm run verify:architecture-nonadjacent-conversion`, and `npm run verify:architecture-forbidden-path-growth` all passed. Current audit gap: `tool.apply_patch_freeform_contract` has a map entry and test anchors but no `src/sharedmodule` source anchor; `verify:function-map-boundary-mentions` still warns on `server.responses_request_handler_bridge_surface` because `clearResponsesConversationByRequestIdForHttp` appears in a forbidden path. Server boundary rule confirmed by user: server handlers must not own protocol parsing; parsing/normalization stays in bridge/native owners.

## 2026-06-14 note / memory / skill routing truth
- `note.md` remains append-only raw exploration. When the same theme appears multiple times, the newest verified timestamp wins for current truth, but older lines stay as evidence; the active consolidation index now lives at the top of `note.md`.
- Promotion boundary is fixed: durable project facts go to `MEMORY.md`; reusable workflows/anti-patterns go to local skills; short-lived execution context stays in `CACHE.md`. This matches `docs/agent-routing/40-task-memory-routing.md` and `~/.codex/skills/conversation-memory/SKILL.md`.
- Current reusable skill-writing rule for RouteCodex is routed-entry only: main skill file stays short and points to small reference files; details do not accumulate back into one giant `SKILL.md`.
Tags: note-md, memory-md, skill-routing, latest-wins, append-only, 2026-06-14

## 2026-06-14 Function Map / Gate / Owner registry
- Current owner registry truth is `feature_id -> owner_kind -> owner_module -> owner_scope -> required_tests -> required_gates`, split across `docs/architecture/function-map.yml`, `docs/architecture/verification-map.yml`, and source `feature_id:` anchors. Updating a critical owner without synchronizing all three is a contract violation.
- Current owner-kind distribution, verified from the live function map, is `rust_ssot=29`, `ts_runtime_owner=15`, `server_projection=10`, `ts_bridge=4`, `provider_runtime=2`, `ts_entry_shell=2`.
- Minimum architecture query gates currently proven green are `npm run verify:function-map-compile-gate`, `npm run verify:architecture-owner-queryability`, and `npm run verify:architecture-feature-map-growth-discipline`. These are the required first-pass gates before claiming a new or moved owner is registered correctly.
- Canonical lookup example remains `hub.servertool_followup`: the feature row exists in both maps and must also be findable from source anchors before a change is considered queryable.
Tags: function-map, verification-map, owner-registry, feature-id, architecture-gates, 2026-06-14

## 2026-06-14 ~/.rcc config and provider truth
- RouteCodex runtime/provider truth is read from `~/.rcc`, not repo `config/` and not `~/.codex/config.toml`. First read order is `~/.rcc/config.toml`, then `~/.rcc/config.<provider>.toml` if present, then `~/.rcc/provider/<providerId>/config.v2.toml`; merged config snapshots are derived evidence only.
- On this machine `~/.rcc` is active and contains runtime state such as `install/`, `provider/`, `diag/`, `logs/`, `sessions/`, `servertool/`, and `quota/`. Provider examples verified live are `~/.rcc/provider/1token/config.v2.toml` and `~/.rcc/provider/DF/config.v2.toml`.
- Provider v2 config durable shape is: top-level `version`/`providerId`; `[provider]` with `id`, `enabled`, `type`, `baseURL`, optional `compatibilityProfile`, `transportBackend`, optional `defaultModel`; `[provider.auth]`; optional `[provider.responses]`; `[provider.concurrency]`; and `[provider.models."<modelId>"]` entries. `provider.models.<modelId>` is the upstream wire model truth; `aliases` are client-facing only.
- `~/.rcc/install/current` is the release snapshot truth for the currently installed CLI/runtime; current verified health/install notes in `MEMORY.md` use that path, not temp build artifacts.
Tags: rcc-home, provider-config, config-v2, install-current, provider-wire-model, 2026-06-14

- 2026-06-14: `apply_patch` Responses freeform contract is locked at the Rust request-governance owner. If upstream returns `Invalid lark grammar ... unknown name: "begin_patch"` on a same-protocol direct `/v1/responses` request, first inspect `req_process_stage1_tool_governance_blocks/orchestrator.rs::normalize_apply_patch_freeform_tool_schema`, not server handler, relay store, or SSE. The valid client/provider-visible tool shape is `type: custom`, `name: apply_patch`, `format.syntax: lark`, and a complete grammar containing `begin_patch`, `end_patch`, hunk rules, and `%import common.LF`; `parameters`, JSON-wrapped `{"patch":...}`, servertool execution, and handler-side shape repair must stay absent. Verified on 2026-06-14 after global install/restart `0.90.3065`: 5520 direct and 5555 route/relay online smokes both returned raw `custom_tool_call.input`, `functionArgumentPatchLeakCount=0`, `deltaStreamCount=0`, and all `5520/5555/10000` health checks were `ready=true pipelineReady=true`.

Tags: apply_patch, responses, freeform, lark-grammar, rust-ssot, direct, relay, 2026-06-14

- 2026-06-15: direct-path provider error flow is now closed around the unique center `Virtual Router policy + ProviderFailurePolicy + request-executor error action queue`; `router-direct` / `provider-direct` consume ErrorErr05 execution decisions instead of doing report-only rethrow, and `http-error-mapper` projects provider/upstream errors to clients only after `policyExhausted=true` or `candidateExhausted=true`. Verified by focused red/green suites (`provider-failure-policy-client-disconnect-499`, `http-error-mapper-499-client-disconnect`, `http-error-mapper.policy-exhausted-gate`, `router-direct-pipeline.candidate-exhaustion`, `provider-direct-pipeline.candidate-exhaustion`, `error-pipeline-contract`), by architecture gates (`verify:function-map-compile-gate`, `verify:error-pipeline-contract`, `verify:provider-failure-ban-blackbox`, `verify:architecture-error-chain-bypass`, `verify:architecture-provider-specific-leaks`, `verify:architecture-thin-wrapper-only`, `tsc --noEmit`), and by live 0.90.3068 install/restart on 5555/5520/10000.

- 2026-06-15: `client_disconnect` (including upstream HTTP_499 / `client abort request`) is health-neutral at `error.provider_failure_policy`; it does not affect cooldown/health and is not client-visible as provider 4xx. Live evidence: a 5555 SSE request aborted by the client kept HTTP 200, sent no `event:error`, and contained no `HTTP_499` / `client abort request` text; historical `response.sse.client_close` logs remain the expected server-side signal.

- 2026-06-15: `virtual_router.primary_exhausted_to_default_pool` is Rust-owned truth only. Host decision helpers may consume the plan but may not synthesize fallback lists. Runtime/native evidence: `loadNativeRouterHotpathBinding().planPrimaryExhaustedToDefaultPoolJson(...)` returned `{"status":"default_pool","defaultPoolTargets":["fwd.c"],"fromTierId":"backup","fromTierPriority":100}` when the primary tier targets were exhausted.

- 2026-06-15: the function-map boundary gate itself needed hardening to stay deterministic under active Rust builds. `scripts/architecture/verify-function-map-boundary-mentions.mjs` must scan source trees only and skip transient/generated directories (`target`, `node_modules`, `dist`, `build`, `.git`, `.cache`, `coverage`, `.rcc`, `out`, `tmp`, `logs`, `release`), otherwise ENOENT races from disappearing `target/debug/deps/rmeta*` artifacts can break `verify:function-map-compile-gate` without any real architecture violation.

- 2026-06-15 (post-build 0.90.3068) live cross-check: 5555/5520/10000 `/health` are all `ready=true pipelineReady=true` on the same version; live `/v1/responses` SSE on 10000 also reaches `response.completed + response.done` (135 events on the most recent probe, 0 errors). A 5555 client-side abort kept HTTP 200, sent no `event:error`, and emitted no `HTTP_499` / `client abort request` text. Recent server-5555/server-5520 logs show the unified `[provider-switch] ... exclude_and_reroute` chain running across HTTP_502/503/500/UPSTREAM_HEADERS_TIMEOUT at attempt 1/6 → 2/6 → 3/6 with `decision=provider_backoff_then_reroute` and `policy=existing_exclusion`, which is the running state of requirement #4 (2+ candidates + 1 provider 5xx switches to candidate 2).

- 2026-06-16: stopless relay invalid-schema closure requires two distinct persistence lanes. Routing/session truth for future turns stays in the existing routing-state store and keeps normal `used` progression (`0 -> 1 -> 2 -> 3`). Client-visible CLI stdout for the current projected `reasoning_stop` command must be seeded separately into `servertool-core` persisted-state-fs with the current `used` plus the detailed followup text; otherwise the first invalid-schema CLI result falls back to the generic invalid prompt, and if this seed overwrites routing state it corrupts the 1/2/3 progression. Verified by `tests/servertool/stopless-cli-continuation.spec.ts`, `tests/servertool/stopless-prompt.client-visible.spec.ts`, `cargo test -p servertool-core stopless_cli_invalid_schema_reuses_persisted_detail_text --lib -- --nocapture`, `cargo test -p servertool-core invalid_schema_prefers_detailed_followup_text_for_snapshot --lib -- --nocapture`, and `git diff --check`.

- 2026-06-16: Stopless CLI 不再自动生成 fake `sessionId`/`requestId`。物理删除了 `servertool-core::cli_contract.rs` 里的 `STOPLESS_ID_COUNTER` / `next_stopless_id_counter` / `current_time_ms` / `resolve_stopless_default_session_id`(含 `CODEX_THREAD_ID`/`TMUX_PANE`/`TERM_SESSION_ID`/`ITERM_SESSION_ID`/`pid:...` 全部兜底) / `resolve_stopless_default_request_id`；`validate_stopless_session_identity` 改成 fail-fast，缺 `sessionId`/`requestId` 直接 `MissingField`。同时把 `build_stop_message_auto_run_output` 的 `current_repeat_count` / `current_max_repeats` 优先级从 `[CLI arg, payload, persisted]` 改为 `[CLI arg, persisted, payload]`，避免 payload 把会话真相压回 1。Red: `servertool-cli/tests/cli_blackbox.rs::missing_session_identity_fails_with_missing_field_error` + `stopless_continuation_count_increments_with_real_session_id`；`tests/cli/servertool-command.spec.ts::omitting sessionId causes CLI to fail with missing field error` 替换了旧 auto-fill 断言。验证：`cargo test -p servertool-cli --test cli_blackbox` 21/21、`tests/cli/servertool-command.spec.ts` 19/19、`cargo build -p servertool-cli` 0 errors、本地 `target/debug/routecodex-servertool` 用真实 `session-real-verify` 三连 → `repeatCount=1→2→3(budget_exhausted)`，第 4 次仍 `repeatCount=3` 不回弹。Live 续杯要等 `ROUTECODEX_BUILD_RESTART_ONLY=1 ROUTECODEX_INSTALL_VERIFY_PORT=5555 ./scripts/install-global.sh` 重新发版后才能在 `routecodex hook run` 中体现。

- 2026-06-17: stopless 的 session truth 还要锁同一个 `sessionDir`，不能只锁 `sessionId`。live 5555 真实链路曾出现：CLI `exec_command` 带真实 `--session-id`，但 server 下一轮仍然回到 `repeatCount=1`。根因不是 CLI 不写盘，而是 CLI 用 `ROUTECODEX_SESSION_DIR=/Users/.../.rcc/sessions/127.0.0.1_5555` 写 `session:<sid>` 文件，server 侧 `loadRoutingInstructionStateSync/saveRoutingInstructionStateSync` 却默认只看 `process.env.ROUTECODEX_SESSION_DIR`，导致 server 和 CLI 读写不同目录。修复真相：`native-virtual-router-routing-state.ts` 的 load/save wrapper 必须显式吃 `sessionDir`；`stop-message-auto/runtime-utils.ts` 和 `stop-message-auto.ts` 必须从 `adapterContext.__rt.sessionDir` 透传给所有 stopless persisted state 读写；`seedStoplessCliPersistedState` 只 pre-seed `schemaUsedBeforeCount`（当前轮之前的 used），由本轮 CLI 再推进到 `N+1`。同时 `servertool-core::cli_contract.rs` 的 stopless `repeatCount/maxRepeats` 以 persisted session truth 为先，caller hint 只作首轮 bootstrap。验证：`cargo test -p servertool-core cli_contract --lib` 40/40；`node --experimental-vm-modules ./node_modules/jest/bin/jest.js --config sharedmodule/llmswitch-core/jest.config.cjs --runInBand tests/servertool/stop-message-runtime-utils.test.ts -t 'sessionDir|persists and reloads stopless state'` 2/2；`node scripts/verify-servertool-rust-only.mjs` PASS；`node scripts/build-core.mjs` PASS；重启 `5555/5520` 后 `scripts/tests/stopless-5555-final-probe.mjs` 14/14 PASS，在线证明：5555 stopless `exec_command` 带真实 `--session-id/--request-id`、同一 live execCommand 在同一 session 内收口到 `repeatCount=3 / stopless budget exhausted`、5520 direct 不触发 stopless。

- 2026-06-17: `ROUTECODEX_SESSION_DIR` 语义已收口为 runtime workdir root，而不是 session 身份真源。验证后的当前事实是：生产路径中 stopless / pending-session / routing-state 不再从 env 或顶层字段猜目录；Rust NAPI proxy 只接受 `metadata.__rt.{sessionDir,rccUserDir}`；`SessionClientRegistry` 只认 bootstrap 显式注入的 bindings store path，`conversationSessionId -> tmuxSessionId` 只是 client binding，不重定义 request `sessionId`。正式文档已同步到 `docs/design/server-runtime-lifecycle-ssot.md`、`docs/architecture/wiki/metadata-boundary-map.md`、`docs/architecture/wiki/runtime-lifecycle-call-graph.md`，并通过 repo 内 HTML render/gate + 目标 Jest 验证。长期风险仍是物理目录混放多类 state，属于后续目录拆分问题，不再是身份语义 blocker。

- 2026-06-17: Hub Pipeline “瘦身不减功能” 审计真相：核心 Rust req/resp mainline 已基本收口，但 host/bridge 侧仍有三块主要残留语义面需要继续物理收缩：`src/server/runtime/http-server/executor/request-executor-request-semantics.ts` 的 provider-native continuation 判定、`src/server/utils/finish-reason.ts` 的 tool_calls/required_action/output function-call 回填、`src/modules/llmswitch/bridge/responses-response-bridge.ts` 的 SSE terminal/probe/finish-reason/persist 逻辑。验证证据：`tests/sharedmodule/hub-pipeline-runtime-ingress.spec.ts` PASS、`tests/sharedmodule/chat-semantics-stage1.spec.ts` PASS，而 `tests/sharedmodule/hub-pipeline-stage-residue-audit.spec.ts` 当前仍因这些残留、过期测试脚本路径、以及 `sharedmodule/llmswitch-core/src/**` side-by-side TS emit artifacts 而红。下次做 Hub 精简，先砍这三处与 emit/script hygiene，不要先碰 Rust 主链。

- 2026-06-17 update: 上述审计里两块 host residue 与一块 hygiene 已闭环收口。`request-executor-request-semantics.ts` 的 provider-native continuation 判定现在走 Rust/native owner；`src/server/utils/finish-reason.ts` 已删除 TS 侧 visible-success/tool-call fallback；`package.json:test:routing-instructions` 的失效 stop-message-auto 路径已改为现存拆分用例；`sharedmodule/llmswitch-core/src/**` 的 ignored side-by-side `.js/.d.ts/.map` emit artifacts 已物理删除。验证：`node scripts/build-core.mjs` PASS，focused Jest（`request-executor-request-semantics` / `finish-reason` / `hub-pipeline-stage-residue-audit` / `chat-semantics-stage1` / `hub-pipeline-runtime-ingress`）全绿，`npx tsc --noEmit --pretty false` PASS。当前仍值得继续瘦身的主残留只剩 `src/modules/llmswitch/bridge/responses-response-bridge.ts` 这一大块 TS bridge 语义中心，以及 `hub-pipeline-execute-request-stage.ts` / `hub-pipeline-execute-chat-process-entry.ts` 的同构壳合并。

- 2026-06-17 follow-up: `hub-pipeline-execute-chat-process-entry.ts` 已物理删除，chat_process request mainline 统一复用 `hub-pipeline-execute-request-stage.ts`，通过 `entryMode: "chat_process"` 只保留错误文案与 result 投影差异，减少一层同构壳而不改变 native 入参/route 复用语义。`responses-response-bridge.ts` 也再删掉一个 zero-consumer export：`rebindResponsesConversationRequestIdsToResponseIdForHttp`. 验证：`tests/sharedmodule/hub-pipeline-preselected-route.spec.ts`、`tests/sharedmodule/hub-pipeline-stage-residue-audit.spec.ts`、`tests/sharedmodule/chat-semantics-stage1.spec.ts`、`tests/sharedmodule/hub-pipeline-runtime-ingress.spec.ts`、`tests/server/runtime/http-server/executor/request-executor-request-semantics.spec.ts`、`tests/server/utils/finish-reason.spec.ts`、`tests/server/utils/finish-reason.visible-success.spec.ts` 全绿；`npx tsc --noEmit --pretty false` 与 `npx tsc -p sharedmodule/llmswitch-core/tsconfig.json --noEmit --pretty false` 都绿。下一轮继续砍 `responses-response-bridge.ts` 时，优先找 zero-consumer helper / 可下沉到 native 的 bridge glue，不要重做 SSE 主状态机。

- 2026-06-17 audit update: `docs/goals/hub-pipeline-slimming-no-function-loss-plan.md` 现已包含 Hub Pipeline slimming 完整候选项处置表。当前复扫结论：`responses-response-bridge.ts` exported surface 没有新的 0-consumer export；`responses-sse-bridge.ts` 虽是 re-export facade，但是 function-map/verification-map 中 `server.responses_sse_bridge_surface` 的 owner module，不能按死代码删除；SSE terminal/probe/persist helper 暂缓大拆，后续只能先找更小 zero-consumer/internal helper 或补 native-downshift 红测。验证：goal 指定 Jest 7 suites 全绿、root `tsc` 全绿、sharedmodule `tsc` 全绿、`git diff --check` 全绿。

- 2026-06-18: `MetadataCenter.request_truth` is now the only trusted request session/conversation source for the current http-server executor band. Verified closeout: `buildRequestMetadata(...)` no longer backfills top-level `metadata.sessionId/conversationId`; `finalizeRequestExecutorAttemptMetadata(...)` no longer re-materializes `sessionId/session_id/conversationId/conversation_id` after merges and explicitly deletes leaked flat fields; request log binding plus provider `usageLogInfo` now read only via `readRuntimeRequestTruthIdentifiers(...)`. Focused Jest stack passed (`executor-metadata`, `request-truth-readers`, `request-executor-session-storm-backoff`, `servertool-adapter-context`, `index.request-truth-contract`, `responses-response-bridge.request-truth`, `client-injection-flow`), `npm run build:min` passed, global install/restart reached `0.90.3098`, and 5555 live stopless probe closed correctly (`requires_action` with `exec_command` on first turn, resumed `submit_tool_outputs` => `completed`, final `finalStatus=completed`). Durable rule: stopless/servertool may stay inactive when request truth is absent, but request truth must never be re-synthesized from flat metadata aliases or tmux-only scope.


- 2026-06-18: internal metadata cleanup closeout reached. Request/response/provider/client payloads no longer carry internal control truth; only `MetadataCenter` / runtime side-channel is used for necessary control writes. Verified green: `audit:custom-payload-carriers`, `audit:custom-payload-carrier-owner-queryability`, `verify:architecture-custom-payload-carrier-runtime-manifest`, `verify:architecture-custom-payload-carrier-containment`, `verify:function-map-compile-gate`, `verify:architecture-ci-longtail`, `verify:architecture-review-surface`, `verify:architecture-ci`, `tsc --noEmit`, `build:min`, `git diff --check`. Live proof on 5555: `routecodex restart --port 5555` succeeded, `/health` returned `ready=true pipelineReady=true version=0.90.3136`, and a real `/v1/responses` smoke returned `ok`. Direct router 401 now fail-fast rethrows instead of recursive reroute; 429/502 reroute behavior remains intact.

- 2026-06-18: Provider auth/quota 401/403 must not be projected to clients as provider-auth-specific errors. `error.client_projection` now maps provider 401/403 to public-safe `502 upstream_error / Upstream provider error` and omits `request_id`; handler error/SSE projection no longer backfills request id for generic upstream errors, while internal logs may keep status/code fields without raw provider body. Verified by handler blackbox, mapper leak guard, request-error-log, and direct-passthrough route-level Jest suites plus `verify:error-pipeline-contract`, `tsc --noEmit`, and `git diff --check`. `verify:provider-failure-ban-blackbox` remained blocked by test harness ENOENT for temp `provider-health.json`.

- 2026-06-18: stopMessage / servertool followup control truth is now `MetadataCenter.runtime_control`, not flat request metadata, `__rt`, or `requestSemantics.__routecodex`. Entry writes use `runtime_control.stopMessageEnabled/stopMessageExcludeDirect`; servertool followup nested retries preserve the MetadataCenter symbol binding across JSON clone; explicit followup source/flow disables stopMessage through runtime-control only, preventing recursion without leaking control fields into payload. `MetadataCenter.attach/read` uses structural side-channel detection instead of `instanceof`, and stale side-by-side generated files under `src/server/runtime/http-server/metadata-center/*.js/*.d.ts` are physically removed and locked by `verify:architecture-deleted-path`. Verified by focused Jest 50/50, `tsc --noEmit`, `git diff --check`, payload-carrier audit/owner/manifest/containment gates, `verify:function-map-compile-gate`, `build:min`, and `verify:architecture-ci`.
...
- 2026-06-21: Startup credential-skipped providers must be physically pruned from Virtual Router config after `initializeProviderRuntimes()`, not only skipped from runtime handle creation. `setupRuntime()` originally loaded `bootstrapArtifacts.config` into Hub/VR before runtime init; if a provider later hit `startupExcludedProviderKeys` (for example `ecodev.backup.GLM-5.1` with empty token file), Rust `direct_model` routing could still enumerate the stale provider from registry and fail at request time with `provider not found for runtimeKey: ecodev.backup`. The fix is to remove excluded provider keys from `currentRouterArtifacts.config.providers` and from every route tier `targets`, then refresh the primary Hub pipeline and each routing-group pipeline with the pruned config. Verified by focused setupRuntime Jest, `tsc --noEmit`, `BUILD_MODE=dev npm run build:min`, restart to `0.90.3223`, and live 5555 `chat/responses model=ecodev.GLM-5.1` both hitting `ecodev[default].GLM-5.1` with HTTP 200.

Tags: startupExcludedProviderKeys, virtual-router-config, direct_model, ecodev, runtime-init, 2026-06-21

- 2026-06-21: provider config `errorMapping.rules` is a shared provider-runtime contract, not an `HttpRequestExecutor`-only helper. `/v1/responses` direct SSE/JSON/submit_tool_outputs paths in `src/providers/core/runtime/responses-provider.ts` must consume the same shared normalizer `src/providers/core/runtime/provider-configured-error-mapping.ts`, otherwise provider-facing `/v1/responses` can still leak the original `HTTP_400` even when generic executor tests are green. Verified by focused Jest on `responses-provider.direct-passthrough`, `provider-http-executor-utils`, `provider-failure-policy`, and `request-executor-provider-failure-plan`, plus `tsc --noEmit`, build/install, and live `0.90.3233` health on 5555. Boundary: this slice did not get a fresh live exhausted-account replay; online closure still depends on a reproducible `XLC.key2.deepseek-v4-pro` sample.
## 2026-06-21 provider-configured error mapping must cover Responses failed payload chain

- 已验证：`HttpTransportProvider` 的 `ProviderContext.extensions.errorMapping` 注入修好后，还不够覆盖 `/v1/responses` 全链；`ResponsesProvider.reportResponsesFailureIfNeeded()` 之前会把 upstream failed payload 重新压成 synthetic error，跳过 provider-configured error mapping。
- 正确 owner/修复点：`src/providers/core/runtime/responses-provider-helpers.ts` 的 `detectResponsesFailure(payload, context)` 必须在基于 failed payload 构造 normalized upstream error 后，先执行 `applyProviderConfiguredErrorMapping(...)`，再进入 `resolveProviderFailureOutcome(...)`；`responses-provider.ts` 只消费该 normalized error 上报，不得自己再手拼第二份 400 shell。
- 在线证据：重放旧失败样本后，`server-5520.log` 出现 `openai-responses-XLC.key2-deepseek-v4-pro-20260621T162103748-381781-1150` 的 `provider-switch ... status=504 code=HTTP_504`，且整次 `/v1/responses` 请求最终以 `openai-responses-minimax.key1-MiniMax-M3-20260621T162103748-381781-1150 completed (status=200, finish_reason=tool_calls)` 收口；客户端不再看到 `event:error status=400 code=HTTP_400`。

- 2026-06-22: `~/.rcc/config.toml` 的 `gateway_priority_5520.routing.thinking` 已确认真源为 `fwd.paid.gpt-5.4` + `fwd.glm.glm-5.2`，权重 1:1，`thinking=high`。`fwd.glm.glm-5.2` 在 forwarder builder 中展开为 `XLC.key1.glm-5.2` / `XLC.key2.glm-5.2` 两个 providerKey。结论：5520 thinking 已经是 gpt:glm = 1:1，不需要新增路由，只需要让测试断言和后续 goal/plan 以这个真源为准，禁止“thinking only gpt”旧预期复活。

- 2026-06-22: 5520 `thinking` 1:1 (gpt-5.4 : glm-5.2) 三层口径闭环：`~/.rcc/config.toml` 真源已含 `fwd.paid.gpt-5.4` + `fwd.glm.glm-5.2` 权重 1:1 `thinking=high`；`src/config/virtual-router-builder.ts` 已展开 `fwd.glm.glm-5.2 -> XLC.key1.glm-5.2 / XLC.key2.glm-5.2`；`tests/config/virtual-router-builder.forwarder-10000.spec.ts` `5520 thinking keeps GPT and GLM balanced 1:1` 用例断言 `targets` / `weights` / `thinking=high`，focused Jest 9/9 PASS。后续禁止再回到“thinking only gpt”旧预期。

- 2026-06-22: `sharedmodule/llmswitch-core/src/servertool/direct-stopless-route-guard.ts` 已物理删除，`engine.ts` 的 synthetic/direct preflight 现在直接把 `adapterContext` 传给 Rust `planServertoolEnginePreflightWithNative(...)`，不再持有 TS direct-route guard 第二真源；同时新增 `createServertoolRequiredResponseHookEmptyError(...)`（对应 Rust `plan_servertool_required_response_hook_empty_error`），`types.ts` 同步删除已不使用的 `ProviderInvoker`。focused Jest 4 suites、verify-servertool-rust-only、build-core 全绿。

- 2026-06-22: 对 servertool response-stage 这类“单次 metadata write wrapper”，可以直接做物理内联删除：`response-stage-orchestration-shell.ts` 的 `markServertoolResponseOrchestration(...)` 已删，`writeRuntimeControlToBoundMetadataCenter(...)` 直接内联到 `runServertoolResponseStageOrchestrationShell(...)`。验证 `resp-process-stage3-reentry`、`stop-message-flow-followup-reentry`、`servertool-active-orchestration-audit`、`verify-servertool-rust-only`、`build-core` 全绿。后续遇到同类纯包装函数，默认按“内联 + gate 禁复活”处理。

- 2026-06-22: `sharedmodule/llmswitch-core/src/servertool/handlers/stop-message-auto.ts` 的 `normalizeStoplessTriggerHintForMetadata()` 已改为调用 Rust/NAPI `normalizeStoplessTriggerHintForMetadataJson`，TS 不再持有 `stop_schema_* -> hint` 的本地 switch 真源。验证通过 `cargo test -p servertool-core normalize_stopless_trigger_hint_for_metadata_maps_reason_codes -- --nocapture`、`node sharedmodule/llmswitch-core/scripts/build-native-hotpath.mjs`、`tests/servertool/stopless-metadata-center.spec.ts --runInBand`、`node scripts/build-core.mjs`、`node scripts/verify-servertool-rust-only.mjs`。

- 2026-06-22: `sharedmodule/llmswitch-core/src/servertool/flow-presentation-block.ts` 已物理删除。`progress-log-block.ts` / `followup-flow-policy.test.ts` 改为直接调用 native wrapper `resolveServertoolProgressToolNameWithNative` / `shouldUseServertoolGoldProgressHighlightWithNative`。同步把 `docs/architecture/function-map.yml` `hub.servertool_flow_presentation.owner_scope` 明确标为 file-scoped Rust owner，让 `verify-architecture-feature-anchor-coverage` 不再硬要求 2 个 builder 命中文件，避免为了凑 anchor 在 native wrapper 里塞 fake string。`scripts/verify-servertool-rust-only.mjs` 加反向锁“flow-presentation-block.ts must stay deleted after direct native import closeout”。验证通过 `npm run verify:function-map-compile-gate`、`node scripts/verify-servertool-rust-only.mjs`、`node scripts/build-core.mjs`、focused Jest（13 passed / 3 skipped）。

- 2026-06-22: `sharedmodule/llmswitch-core/src/servertool/registry.ts` 如果只是把 `registry-impl.ts` 的真实导出改名后再 `as` 一次，这层就不是 owner，只是命名壳。正确收口方式是：把 `registry-impl.ts` 直接改成最终对外名，再让 `registry.ts` 只做无别名 re-export；同时把旧 `*Impl` 导出名和 `as ...` 别名链写进 `verify-servertool-rust-only` / `servertool-active-orchestration-audit` 作为复活禁区。验证通过 `tests/servertool/servertool-active-orchestration-audit.spec.ts`、`node scripts/verify-servertool-rust-only.mjs`、`node scripts/build-core.mjs`。

- 2026-06-22: `sharedmodule/llmswitch-core/src/servertool/execution-shell.ts` 是纯 forward wrapper，物理删除后真实 owner 只剩 `execution-handler-materialization-shell.ts` 与 `pre-command-hooks.ts`。删这类 wrapper 时，focused Jest 若 mock 的是旧导出集，必须同步补齐新 owner 依赖导出；本次 `tests/servertool/server-side-tools.failfast.spec.ts` 必须补 `planServertoolHandlerContractErrorWithNative`，否则 `server-side-tools.js` 通过新直连链路加载时会先抛 ESM missing export。验证：focused Jest 4 suites / 26 tests、`node scripts/verify-servertool-rust-only.mjs`、`node scripts/build-core.mjs` 全绿。
...
- 2026-06-22: `server-side-tools-impl.ts` 里的 pre-command runtime state 选择、persisted routing-state 读取、和 state-load error 投影，不应继续混在主 engine orchestrator 里。当前合规 owner 是独立 TS IO shell `pre-command-runtime-state-shell.ts`：由它消费 native runtime action、执行 persisted read、并把 `SERVERTOOL_STATE_LOAD_FAILED` 统一投影；`server-side-tools-impl.ts` 只消费最终 `runtimePreCommandState`。同步 gate 必须改成禁止 impl 文件复活 `loadRoutingInstructionStateSync(` / `resolveServertoolPersistentScopeKey(` / `planRuntimePreCommandStateRuntimeActionWithNative(` 这些 inline owner 字符串。
- 2026-06-22: `server-side-tools-impl.ts` 里的 CLI projection 分支不应继续持有 projected index 校验、additional client tool 收集、projection chatResponse 构造、execution context 构造。当前最小合规 owner 是独立 TS IO shell `cli-projection-runtime-shell.ts`：由它消费 native branch plan 输出、执行 `buildServertoolCliProjectionForToolCall(...)` 和 `buildServertoolCliProjectionExecutionContextWithNative(...)`，并提供 `collectAdditionalClientToolCalls` / `isClientExecCliProjectionToolCall` 的对外导出；`server-side-tools-impl.ts` 只在 `client_exec_cli_projection` action 上直接 return helper 结果。

- 2026-06-22: review 确认 servertool 响应端 hook 与请求端 hook 跨 continuation 的 paired boundary 位置正确。响应端 `ServertoolRespHook03HookResponseInjected` 在 `HubRespChatProcess03Governed` 内投影 client-visible `exec_command`；请求端 `ServertoolReqHook01ResultParsed -> 02 -> 03 -> 04` 在 `HubReqInbound02Standardized` 之后、`HubReqChatProcess03Governed` 之前把同轮 `function_call_output` 还原成 built-in tool 形状；`responses` continuation owner 仅消费 `ServertoolReqHook04RequestFinalized` 之后的请求形状，不参与 hook 改写。`engine.ts` 与 `engine-postflight-shell.ts` 的 stopless projection context 仍收在 `runServertoolEnginePostflight` 内部，未跨 continuation；`sharedmodule/llmswitch-core/src/conversion/shared/responses-conversation-store.ts` + `router-hotpath-napi/src/hub_pipeline_blocks/responses_resume.rs` 仍是唯一 continuation owner，servertool hook 文件里没有 `previous_response_id` / `submit_tool_outputs` / `responses-conversation-store` 引用。验证：`tests/servertool/stopless-cli-continuation.spec.ts` + `tests/servertool/servertool-cli-result-restore.spec.ts` 共 14/14 PASS。`servertool.hook_skeleton.mainline` 仍 `binding pending`，未登记 owner feature。


## 2026-06-22 stopless session truth baseline
- stopless 内部不再保留 sessionId 字段；loop identity 只认 requestTruth.sessionId。
- 禁止从 metadata.sessionId / runtime.sessionId / adapterContext session 字段回退拼装 stopless runtime control。
- 验证门禁：servertool stopless focused Rust + Jest 通过后才能认为收口完成。
# 2026-06-23 stopless live continuation pair restore truth

- Verified owner chain for live stopless continuation loss: `hub_req_inbound_tool_call_normalization.rs::build_reasoning_stop_restored_function_call` plus `hub_req_inbound_context_capture.rs::normalize_responses_input_items`.
- If the live projected stopless shell call is only `routecodex hook run reasoningStop` without `--input-json`, request restore must still materialize `reasoningStop(arguments="{}")`; otherwise the paired `function_call_output` becomes orphan and is dropped from `context.input`.
- Responses context capture must allow restored stopless `reasoningStop` history even when the declared tools only contain `exec_command`; stopless restored pairs are request-history truth, not undeclared-tool noise.
- Verified by:
  - `cargo test -p router-hotpath-napi stopless --lib -- --nocapture`
