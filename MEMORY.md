...
- 2026-06-08 correction: direct/router-direct/provider-direct errors must never enter relay or the standard executor. The older 5555 auth-reroute note below is superseded for direct paths: direct remains provider passthrough + hooks only, reports provider errors to the unified error chain for health/telemetry, then fail-fast projects the provider error; it must not emit retry metadata, exclude-and-reroute, or call `executePipeline()`. If a standard relay request later sees valid Responses tools, it must preserve flat Responses wire shape (`tools[n].name`), not convert to Chat nested `tools[n].function.name`.
