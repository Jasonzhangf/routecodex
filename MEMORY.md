...
## 2026-07-24 V3 Relay SSE materialized snapshot truth
...
- Live closeout evidence: global `routecodex 0.90.3971` installed, V3 5555 restarted only via `routecodex restart --port 5555`, `/health` returned `server_id=responses_v3_5555` and `manifest_version=3`. New live sample `openai-responses-router-gpt-5.5-20260724T203436206-616616-7101` has `bodyKind=sse`, terminal `rawSse`, and `materializedResponse` with id/model/status/output_text/usage; parsed terminal `response.completed/response.done` equals `materializedResponse`.
