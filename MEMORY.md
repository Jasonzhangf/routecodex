...
- 2026-06-08: 5555 provider/account auth/quota errors must enter the unified provider failure execution plan, not direct-return before reroute. `401/402/403` are still classified as current-provider `unrecoverable`, but ErrorErr05 must `exclude_and_reroute` when the route pool has an alternative; deterministic request/model/protocol errors such as `400`/`special_400` and `404` still fail fast and must not be hidden by provider switching. Fix owner is executor ErrorErr05 / provider-failure plan only; direct passthrough remains payload-transparent and only hands retry metadata back to the standard pipeline. Evidence: commit `58c88f04a fix(error): reroute provider auth failures` changed provider failure policy, executor retry plan, provider response status conversion, router-direct/request-executor tests, and memory/skill notes.
