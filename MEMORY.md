...
- TS `sharedmodule/llmswitch-core/src/servertool/flow-presentation-block.ts` may only call native wrappers. It must not restore local flow-id normalization, `buildServertoolProgressConfig`, `toolNameByFlowId`, `goldHighlightFlowIds`, or Set-based highlight policy.
