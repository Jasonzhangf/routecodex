...
- Verified on 2026-06-09: `node sharedmodule/llmswitch-core/scripts/build-native-hotpath.mjs`; `cargo test -p servertool-core pending_session --lib -- --nocapture` 3/3; `cargo test -p router-hotpath-napi pending_session --lib -- --nocapture` 2/2; `npm run jest:run -- --runTestsByPath tests/sharedmodule/servertool-pending-session.spec.ts --runInBand --forceExit` 3/3; `npm run verify:servertool-rust-only`; and `npm run verify:function-map-compile-gate`.
