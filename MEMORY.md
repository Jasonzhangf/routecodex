...
- V3 route monitor shape should use V2-style `[virtual-router-hit] req=<request> sid=<session> <route> -> <provider[key].wire_model> reason=<reason>`, not the custom `🎯 [/v1/responses] route=... provider=...` shape. V3 usage line should use short req, `project=<path>:<port>`, `route=router-direct|router-relay:<reason>`, `model=<request-model>-><wire-model>`, `time=i:<ms> e:<ms> t:<ms>`, and snake-case `finish_reason`.
