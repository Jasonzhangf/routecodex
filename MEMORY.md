...
## 2026-07-26 - V3 Responses mixed provider protocol SSE compatibility
...
- Root cause fixed for 5555 `provider_response_sse_empty`: `glmrelay_openai` is an OpenAI Chat provider; sending it through Direct `/responses` produced HTTP 200 empty SSE, while `/chat/completions` returns semantic SSE. The fix is Rust execution planning (`v3_responses_direct_binding_requires_provider_protocol_relay` plus server effective-mode resolution), not SSE transport, Error06, Direct parser, or config mutation.
