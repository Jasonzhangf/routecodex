...
- Managed V3 5555 live replay blocker fixed: when the service instance ID/config path/config digest/listener set are unchanged and status is terminal `stopped|failed`, lifecycle may roll the exact launch provenance to a new release/target executable. Running, missing-terminal, foreign, or otherwise different declarations still fail without reaping state. Verified by lifecycle unit/CLI tests, V3 workspace, and live 5555 JSON/SSE `/v1/responses` returning `V3_JSON_OK` / `V3_SSE_OK` with terminal `[DONE]`.
